...
class ClienteController {
    ...
    public function getClientesForListView($name = "", $cpf = "", $venda = ""){
        $conditions  = array();
        if(!empty($name)) $conditions[] = ClienteModel::NOME." LIKE '%$name%'";
        if(!empty($cpf)) $conditions[] = ClienteModel::CPF." LIKE '%$cpf%' "; 
        if(!empty($venda)) $conditions[] = VendaModel::TABLE.".".VendaModel::ID." = ".$venda;
        return $this->modelCliente->selectOnlyList(implode(' AND ', $conditions));			
    }
