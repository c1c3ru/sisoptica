...
class Database{
    ...
    public static function connect() {
        try {
            if(is_null(self::$annalisses)){
                $dbconfig = simplexml_load_string(file_get_contents(MODELS . "dbconfig.xml"));
            ...
            // Registrar o erro ou lançar novamente para tratamento em nível superior
     ...
     * @return representação framework corrente
     */
    ...
    protected function insert($table, $fields, $values, $debug = false){
        $q = "INSERT INTO $table ";
        if(!empty($fields)) {
            $q .= '(' . (is_array($fields) ? implode(', ', $fields) : $fields) . ') ';
        }
     ...
     * @access protected
     */
    protected function select($table, $fields = "*", $condition = null, $debug = false) {
        ...
        return self::$annalisses->execute($q);
    }
    /**
     ...
     * obter melhores respostas aos usuários.A
    ...
    protected static function turnInValues($dic){
        ...
        foreach($dic as $attr){
            ...
            else if(is_numeric($attr) && !is_string($attr)) {
                $attrs[] = $attr;
            }
