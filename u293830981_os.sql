-- phpMyAdmin SQL Dump
-- version 5.1.1
-- https://www.phpmyadmin.net/
--
-- Host: 127.0.0.1:3306
-- Tempo de geração: 01/03/2024 às 12:33
-- Versão do servidor: 10.11.7-MariaDB-cll-lve
-- Versão do PHP: 7.2.34

SET SQL_MODE = "NO_AUTO_VALUE_ON_ZERO";
START TRANSACTION;
SET time_zone = "+00:00";


/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;

--
-- Banco de dados: `u293830981_os`
--

-- --------------------------------------------------------

--
-- Estrutura para tabela `caixa_diario`
--

CREATE TABLE `caixa_diario` (
  `id` int(10) UNSIGNED NOT NULL,
  `data` date NOT NULL,
  `saldo` float NOT NULL DEFAULT 0,
  `status` smallint(5) NOT NULL DEFAULT 0,
  `loja` tinyint(3) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `central_estoque`
--

CREATE TABLE `central_estoque` (
  `id` int(10) UNSIGNED NOT NULL,
  `data` date NOT NULL,
  `valor_total` float DEFAULT NULL,
  `id_usuario_estoque` smallint(5) UNSIGNED NOT NULL,
  `id_usuario_confirmacao_entrada` smallint(5) UNSIGNED DEFAULT NULL,
  `id_usuario_confirmacao_saida` smallint(5) UNSIGNED DEFAULT NULL,
  `id_loja_origem` tinyint(3) UNSIGNED DEFAULT NULL COMMENT 'Quando nulo, este atributo refere-se ao fornecedor',
  `id_loja_destino` tinyint(3) UNSIGNED NOT NULL,
  `status` tinyint(3) UNSIGNED NOT NULL DEFAULT 0 COMMENT '0: Pendente, 1: Ok, 2: Problemas',
  `observacao` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `central_estoque_produto`
--

CREATE TABLE `central_estoque_produto` (
  `id_produto` int(10) UNSIGNED NOT NULL,
  `id_central_estoque` int(10) UNSIGNED NOT NULL,
  `quantidade_entrada` int(10) UNSIGNED NOT NULL,
  `quantidade_saida` int(10) UNSIGNED NOT NULL,
  `valor` float DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `cidade`
--

CREATE TABLE `cidade` (
  `id` smallint(5) UNSIGNED NOT NULL,
  `nome_cidade` varchar(100) DEFAULT NULL,
  `id_uf` tinyint(3) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci PACK_KEYS=1;

--
-- Despejando dados para a tabela `cidade`
--

INSERT INTO `cidade` (`id`, `nome_cidade`, `id_uf`) VALUES
(883, 'Abadia de Goiás', 9),
(1565, 'Abadia dos Dourados', 11),
(884, 'Abadiânia', 9),
(1566, 'Abaeté', 11),
(2418, 'Abaetetuba', 14),
(698, 'Abaiara', 6),
(281, 'Abaíra', 5),
(282, 'Abaré', 5),
(2784, 'Abatiá', 18),
(4413, 'Abdon Batista', 24),
(2419, 'Abel Figueiredo', 14),
(4414, 'Abelardo Luz', 24),
(1567, 'Abre Campo', 11),
(3183, 'Abreu e Lima', 16),
(5426, 'Abreulândia', 27),
(1568, 'Acaiaca', 11),
(1129, 'Açailândia', 10),
(283, 'Acajutiba', 5),
(2420, 'Acará', 14),
(699, 'Acarape', 6),
(700, 'Acaraú', 6),
(3683, 'Acari', 20),
(3368, 'Acauã', 17),
(3850, 'Aceguá', 23),
(701, 'Acopiara', 6),
(1346, 'Acorizal', 13),
(79, 'Acrelândia', 1),
(885, 'Acreúna', 9),
(3684, 'Açu', 20),
(1569, 'Açucena', 11),
(4706, 'Adamantina', 26),
(886, 'Adelândia', 9),
(4707, 'Adolfo', 26),
(2785, 'Adrianópolis', 18),
(284, 'Adustina', 5),
(3184, 'Afogados da Ingazeira', 16),
(3685, 'Afonso Bezerra', 20),
(1, 'Afonso Cláudio', 8),
(1130, 'Afonso Cunha', 10),
(3185, 'Afrânio', 16),
(2421, 'Afuá', 14),
(3186, 'Agrestina', 16),
(3369, 'Agricolândia', 17),
(4415, 'Agrolândia', 24),
(4416, 'Agronômica', 24),
(2422, 'Água Azul do Norte', 14),
(1570, 'Água Boa', 11),
(1347, 'Água Boa', 13),
(101, 'Água Branca', 2),
(2561, 'Água Branca', 15),
(3370, 'Água Branca', 17),
(1487, 'Água Clara', 12),
(1571, 'Água Comprida', 11),
(4417, 'Água Doce', 24),
(1131, 'Água Doce do Maranhão', 10),
(2, 'Água Doce do Norte', 8),
(285, 'Água Fria', 5),
(887, 'Água Fria de Goiás', 9),
(888, 'Água Limpa', 9),
(3686, 'Água Nova', 20),
(3187, 'Água Preta', 16),
(3851, 'Água Santa', 23),
(4708, 'Aguaí', 26),
(1572, 'Aguanil', 11),
(3188, 'Águas Belas', 16),
(4709, 'Águas da Prata', 26),
(4418, 'Águas de Chapecó', 24),
(4710, 'Águas de Lindóia', 26),
(4711, 'Águas de Santa Bárbara', 26),
(4712, 'Águas de São Pedro', 26),
(1573, 'Águas Formosas', 11),
(4419, 'Águas Frias', 24),
(889, 'Águas Lindas de Goiás', 9),
(4420, 'Águas Mornas', 24),
(1574, 'Águas Vermelhas', 11),
(3852, 'Agudo', 23),
(4713, 'Agudos', 26),
(2786, 'Agudos do Sul', 18),
(3, 'Águia Branca', 8),
(2562, 'Aguiar', 15),
(5427, 'Aguiarnópolis', 27),
(1575, 'Aimorés', 11),
(286, 'Aiquara', 5),
(702, 'Aiuaba', 6),
(1576, 'Aiuruoca', 11),
(3853, 'Ajuricaba', 23),
(1577, 'Alagoa', 11),
(2563, 'Alagoa Grande', 15),
(2564, 'Alagoa Nova', 15),
(2565, 'Alagoinha', 15),
(3189, 'Alagoinha', 16),
(3371, 'Alagoinha do Piauí', 17),
(287, 'Alagoinhas', 5),
(4714, 'Alambari', 26),
(1578, 'Albertina', 11),
(1132, 'Alcântara', 10),
(703, 'Alcântaras', 6),
(2566, 'Alcantil', 15),
(1488, 'Alcinópolis', 12),
(288, 'Alcobaça', 5),
(1133, 'Aldeias Altas', 10),
(3854, 'Alecrim', 23),
(4, 'Alegre', 8),
(3855, 'Alegrete', 23),
(3372, 'Alegrete do Piauí', 17),
(3856, 'Alegria', 23),
(1579, 'Além Paraíba', 11),
(2423, 'Alenquer', 14),
(3687, 'Alexandria', 20),
(890, 'Alexânia', 9),
(1580, 'Alfenas', 11),
(5, 'Alfredo Chaves', 8),
(4715, 'Alfredo Marcondes', 26),
(1581, 'Alfredo Vasconcelos', 11),
(4421, 'Alfredo Wagner', 24),
(2567, 'Algodão de Jandaíra', 15),
(2568, 'Alhandra', 15),
(3190, 'Aliança', 16),
(5428, 'Aliança do Tocantins', 27),
(289, 'Almadina', 5),
(5429, 'Almas', 27),
(2424, 'Almeirim', 14),
(1582, 'Almenara', 11),
(3688, 'Almino Afonso', 20),
(2787, 'Almirante Tamandaré', 18),
(3857, 'Almirante Tamandaré do Sul', 23),
(891, 'Aloândia', 9),
(1583, 'Alpercata', 11),
(3858, 'Alpestre', 23),
(1584, 'Alpinópolis', 11),
(1348, 'Alta Floresta', 13),
(4346, 'Alta Floresta d`Oeste', 21),
(4716, 'Altair', 26),
(2425, 'Altamira', 14),
(1134, 'Altamira do Maranhão', 10),
(2788, 'Altamira do Paraná', 18),
(704, 'Altaneira', 6),
(1585, 'Alterosa', 11),
(3191, 'Altinho', 16),
(4717, 'Altinópolis', 26),
(4398, 'Alto Alegre', 22),
(3859, 'Alto Alegre', 23),
(4718, 'Alto Alegre', 26),
(1135, 'Alto Alegre do Maranhão', 10),
(1136, 'Alto Alegre do Pindaré', 10),
(4347, 'Alto Alegre dos Parecis', 21),
(1349, 'Alto Araguaia', 13),
(4422, 'Alto Bela Vista', 24),
(1350, 'Alto Boa Vista', 13),
(1586, 'Alto Caparaó', 11),
(3689, 'Alto do Rodrigues', 20),
(3860, 'Alto Feliz', 23),
(1351, 'Alto Garças', 13),
(892, 'Alto Horizonte', 9),
(1587, 'Alto Jequitibá', 11),
(3373, 'Alto Longá', 17),
(1352, 'Alto Paraguai', 13),
(2789, 'Alto Paraíso', 18),
(4348, 'Alto Paraíso', 21),
(893, 'Alto Paraíso de Goiás', 9),
(2790, 'Alto Paraná', 18),
(1137, 'Alto Parnaíba', 10),
(2791, 'Alto Piquiri', 18),
(1588, 'Alto Rio Doce', 11),
(6, 'Alto Rio Novo', 8),
(705, 'Alto Santo', 6),
(1353, 'Alto Taquari', 13),
(2792, 'Altônia', 18),
(3374, 'Altos', 17),
(4719, 'Alumínio', 26),
(219, 'Alvarães', 3),
(1589, 'Alvarenga', 11),
(4720, 'Álvares Florence', 26),
(4721, 'Álvares Machado', 26),
(4722, 'Álvaro de Carvalho', 26),
(4723, 'Alvinlândia', 26),
(1590, 'Alvinópolis', 11),
(3861, 'Alvorada', 23),
(5430, 'Alvorada', 27),
(4349, 'Alvorada d`Oeste', 21),
(1591, 'Alvorada de Minas', 11),
(3375, 'Alvorada do Gurguéia', 17),
(894, 'Alvorada do Norte', 9),
(2793, 'Alvorada do Sul', 18),
(4399, 'Amajari', 22),
(1489, 'Amambaí', 12),
(203, 'Amapá', 4),
(1138, 'Amapá do Maranhão', 10),
(2794, 'Amaporã', 18),
(3192, 'Amaraji', 16),
(3862, 'Amaral Ferrador', 23),
(895, 'Amaralina', 9),
(3376, 'Amarante', 17),
(1139, 'Amarante do Maranhão', 10),
(290, 'Amargosa', 5),
(220, 'Amaturá', 3),
(291, 'Amélia Rodrigues', 5),
(292, 'América Dourada', 5),
(4724, 'Americana', 26),
(896, 'Americano do Brasil', 9),
(4725, 'Américo Brasiliense', 26),
(4726, 'Américo de Campos', 26),
(3863, 'Ametista do Sul', 23),
(706, 'Amontada', 6),
(897, 'Amorinópolis', 9),
(2569, 'Amparo', 15),
(4727, 'Amparo', 26),
(5351, 'Amparo de São Francisco', 25),
(1592, 'Amparo do Serra', 11),
(2795, 'Ampére', 18),
(102, 'Anadia', 2),
(293, 'Anagé', 5),
(2796, 'Anahy', 18),
(2426, 'Anajás', 14),
(1140, 'Anajatuba', 10),
(4728, 'Analândia', 26),
(221, 'Anamã', 3),
(5431, 'Ananás', 27),
(2427, 'Ananindeua', 14),
(898, 'Anápolis', 9),
(2428, 'Anapu', 14),
(1141, 'Anapurus', 10),
(1490, 'Anastácio', 12),
(1491, 'Anaurilândia', 12),
(7, 'Anchieta', 8),
(4423, 'Anchieta', 24),
(294, 'Andaraí', 5),
(2797, 'Andirá', 18),
(295, 'Andorinha', 5),
(1593, 'Andradas', 11),
(4729, 'Andradina', 26),
(3864, 'André da Rocha', 23),
(1594, 'Andrelândia', 11),
(4730, 'Angatuba', 26),
(1595, 'Angelândia', 11),
(1492, 'Angélica', 12),
(3193, 'Angelim', 16),
(4424, 'Angelina', 24),
(296, 'Angical', 5),
(3377, 'Angical do Piauí', 17),
(5432, 'Angico', 27),
(3690, 'Angicos', 20),
(3591, 'Angra dos Reis', 19),
(297, 'Anguera', 5),
(2798, 'Ângulo', 18),
(899, 'Anhanguera', 9),
(4731, 'Anhembi', 26),
(4732, 'Anhumas', 26),
(900, 'Anicuns', 9),
(3378, 'Anísio de Abreu', 17),
(4425, 'Anita Garibaldi', 24),
(4426, 'Anitápolis', 24),
(222, 'Anori', 3),
(3865, 'Anta Gorda', 23),
(298, 'Antas', 5),
(2799, 'Antonina', 18),
(707, 'Antonina do Norte', 6),
(3379, 'Antônio Almeida', 17),
(299, 'Antônio Cardoso', 5),
(1596, 'Antônio Carlos', 11),
(4427, 'Antônio Carlos', 24),
(1597, 'Antônio Dias', 11),
(300, 'Antônio Gonçalves', 5),
(1493, 'Antônio João', 12),
(3691, 'Antônio Martins', 20),
(2800, 'Antônio Olinto', 18),
(3866, 'Antônio Prado', 23),
(1598, 'Antônio Prado de Minas', 11),
(2570, 'Aparecida', 15),
(4733, 'Aparecida', 26),
(4734, 'Aparecida d`Oeste', 26),
(901, 'Aparecida de Goiânia', 9),
(902, 'Aparecida do Rio Doce', 9),
(5433, 'Aparecida do Rio Negro', 27),
(1494, 'Aparecida do Taboado', 12),
(3592, 'Aperibé', 19),
(8, 'Apiacá', 8),
(1354, 'Apiacás', 13),
(4735, 'Apiaí', 26),
(1142, 'Apicum-Açu', 10),
(4428, 'Apiúna', 24),
(3692, 'Apodi', 20),
(301, 'Aporá', 5),
(903, 'Aporé', 9),
(302, 'Apuarema', 5),
(2801, 'Apucarana', 18),
(223, 'Apuí', 3),
(708, 'Apuiarés', 6),
(5352, 'Aquidabã', 25),
(1495, 'Aquidauana', 12),
(709, 'Aquiraz', 6),
(4429, 'Arabutã', 24),
(2571, 'Araçagi', 15),
(1599, 'Araçaí', 11),
(5353, 'Aracaju', 25),
(4736, 'Araçariguama', 26),
(303, 'Araças', 5),
(710, 'Aracati', 6),
(304, 'Aracatu', 5),
(4737, 'Araçatuba', 26),
(305, 'Araci', 5),
(1600, 'Aracitaba', 11),
(711, 'Aracoiaba', 6),
(3194, 'Araçoiaba', 16),
(4738, 'Araçoiaba da Serra', 26),
(9, 'Aracruz', 8),
(904, 'Araçu', 9),
(1601, 'Araçuaí', 11),
(905, 'Aragarças', 9),
(906, 'Aragoiânia', 9),
(5434, 'Aragominas', 27),
(5435, 'Araguacema', 27),
(5436, 'Araguaçu', 27),
(1355, 'Araguaiana', 13),
(5437, 'Araguaína', 27),
(1356, 'Araguainha', 13),
(1143, 'Araguanã', 10),
(5438, 'Araguanã', 27),
(907, 'Araguapaz', 9),
(1602, 'Araguari', 11),
(5439, 'Araguatins', 27),
(1144, 'Araioses', 10),
(1496, 'Aral Moreira', 12),
(306, 'Aramari', 5),
(3867, 'Arambaré', 23),
(1145, 'Arame', 10),
(4739, 'Aramina', 26),
(4740, 'Arandu', 26),
(1603, 'Arantina', 11),
(4741, 'Arapeí', 26),
(103, 'Arapiraca', 2),
(5440, 'Arapoema', 27),
(1604, 'Araponga', 11),
(2802, 'Arapongas', 18),
(1605, 'Araporã', 11),
(2803, 'Arapoti', 18),
(1606, 'Arapuá', 11),
(2804, 'Arapuã', 18),
(1357, 'Araputanga', 13),
(4430, 'Araquari', 24),
(2572, 'Arara', 15),
(4431, 'Araranguá', 24),
(4742, 'Araraquara', 26),
(4743, 'Araras', 26),
(712, 'Ararendá', 6),
(1146, 'Arari', 10),
(3868, 'Araricá', 23),
(713, 'Araripe', 6),
(3195, 'Araripina', 16),
(3593, 'Araruama', 19),
(2573, 'Araruna', 15),
(2805, 'Araruna', 18),
(307, 'Arataca', 5),
(3869, 'Aratiba', 23),
(714, 'Aratuba', 6),
(308, 'Aratuípe', 5),
(5354, 'Arauá', 25),
(2806, 'Araucária', 18),
(1607, 'Araújos', 11),
(1608, 'Araxá', 11),
(1609, 'Arceburgo', 11),
(4744, 'Arco-Íris', 26),
(1610, 'Arcos', 11),
(3196, 'Arcoverde', 16),
(1611, 'Areado', 11),
(3594, 'Areal', 19),
(4745, 'Arealva', 26),
(2574, 'Areia', 15),
(3693, 'Areia Branca', 20),
(5355, 'Areia Branca', 25),
(2575, 'Areia de Baraúnas', 15),
(2576, 'Areial', 15),
(4746, 'Areias', 26),
(4747, 'Areiópolis', 26),
(1358, 'Arenápolis', 13),
(908, 'Arenópolis', 9),
(3694, 'Arês', 20),
(1612, 'Argirita', 11),
(1613, 'Aricanduva', 11),
(1614, 'Arinos', 11),
(1359, 'Aripuanã', 13),
(4350, 'Ariquemes', 21),
(4748, 'Ariranha', 26),
(2807, 'Ariranha do Ivaí', 18),
(3595, 'Armação dos Búzios', 19),
(4432, 'Armazém', 24),
(715, 'Arneiroz', 6),
(3380, 'Aroazes', 17),
(2577, 'Aroeiras', 15),
(3381, 'Aroeiras do Itaim', 17),
(3382, 'Arraial', 17),
(3596, 'Arraial do Cabo', 19),
(5441, 'Arraias', 27),
(3870, 'Arroio do Meio', 23),
(3871, 'Arroio do Padre', 23),
(3872, 'Arroio do Sal', 23),
(3873, 'Arroio do Tigre', 23),
(3874, 'Arroio dos Ratos', 23),
(3875, 'Arroio Grande', 23),
(4433, 'Arroio Trinta', 24),
(4749, 'Artur Nogueira', 26),
(909, 'Aruanã', 9),
(4750, 'Arujá', 26),
(4434, 'Arvoredo', 24),
(3876, 'Arvorezinha', 23),
(4435, 'Ascurra', 24),
(4751, 'Aspásia', 26),
(2808, 'Assaí', 18),
(716, 'Assaré', 6),
(4752, 'Assis', 26),
(80, 'Assis Brasil', 1),
(2809, 'Assis Chateaubriand', 18),
(2578, 'Assunção', 15),
(3383, 'Assunção do Piauí', 17),
(1615, 'Astolfo Dutra', 11),
(2810, 'Astorga', 18),
(104, 'Atalaia', 2),
(2811, 'Atalaia', 18),
(224, 'Atalaia do Norte', 3),
(4436, 'Atalanta', 24),
(1616, 'Ataléia', 11),
(4753, 'Atibaia', 26),
(10, 'Atilio Vivacqua', 8),
(5442, 'Augustinópolis', 27),
(2429, 'Augusto Corrêa', 14),
(1617, 'Augusto de Lima', 11),
(3877, 'Augusto Pestana', 23),
(3695, 'Augusto Severo', 20),
(3878, 'Áurea', 23),
(309, 'Aurelino Leal', 5),
(4754, 'Auriflama', 26),
(910, 'Aurilândia', 9),
(717, 'Aurora', 6),
(4437, 'Aurora', 24),
(2430, 'Aurora do Pará', 14),
(5443, 'Aurora do Tocantins', 27),
(225, 'Autazes', 3),
(4755, 'Avaí', 26),
(4756, 'Avanhandava', 26),
(4757, 'Avaré', 26),
(2431, 'Aveiro', 14),
(3384, 'Avelino Lopes', 17),
(911, 'Avelinópolis', 9),
(1147, 'Axixá', 10),
(5444, 'Axixá do Tocantins', 27),
(5445, 'Babaçulândia', 27),
(1148, 'Bacabal', 10),
(1149, 'Bacabeira', 10),
(1150, 'Bacuri', 10),
(1151, 'Bacurituba', 10),
(4758, 'Bady Bassitt', 26),
(1618, 'Baependi', 11),
(3879, 'Bagé', 23),
(2432, 'Bagre', 14),
(2579, 'Baía da Traição', 15),
(3696, 'Baía Formosa', 20),
(310, 'Baianópolis', 5),
(2433, 'Baião', 14),
(311, 'Baixa Grande', 5),
(3385, 'Baixa Grande do Ribeiro', 17),
(718, 'Baixio', 6),
(11, 'Baixo Guandu', 8),
(4759, 'Balbinos', 26),
(1619, 'Baldim', 11),
(912, 'Baliza', 9),
(4438, 'Balneário Arroio do Silva', 24),
(4439, 'Balneário Barra do Sul', 24),
(4440, 'Balneário Camboriú', 24),
(4441, 'Balneário Gaivota', 24),
(3880, 'Balneário Pinhal', 23),
(2812, 'Balsa Nova', 18),
(4760, 'Bálsamo', 26),
(1152, 'Balsas', 10),
(1620, 'Bambuí', 11),
(719, 'Banabuiú', 6),
(4761, 'Bananal', 26),
(2580, 'Bananeiras', 15),
(1621, 'Bandeira', 11),
(1622, 'Bandeira do Sul', 11),
(4442, 'Bandeirante', 24),
(1497, 'Bandeirantes', 12),
(2813, 'Bandeirantes', 18),
(5446, 'Bandeirantes do Tocantins', 27),
(2434, 'Bannach', 14),
(312, 'Banzaê', 5),
(3881, 'Barão', 23),
(4762, 'Barão de Antonina', 26),
(1623, 'Barão de Cocais', 11),
(3882, 'Barão de Cotegipe', 23),
(1153, 'Barão de Grajaú', 10),
(1360, 'Barão de Melgaço', 13),
(1624, 'Barão de Monte Alto', 11),
(3883, 'Barão do Triunfo', 23),
(2581, 'Baraúna', 15),
(3697, 'Baraúna', 20),
(1625, 'Barbacena', 11),
(720, 'Barbalha', 6),
(4763, 'Barbosa', 26),
(2814, 'Barbosa Ferraz', 18),
(2435, 'Barcarena', 14),
(3698, 'Barcelona', 20),
(226, 'Barcelos', 3),
(4764, 'Bariri', 26),
(313, 'Barra', 5),
(4443, 'Barra Bonita', 24),
(4765, 'Barra Bonita', 26),
(3386, 'Barra d`Alcântara', 17),
(314, 'Barra da Estiva', 5),
(3197, 'Barra de Guabiraba', 16),
(2582, 'Barra de Santa Rosa', 15),
(2583, 'Barra de Santana', 15),
(105, 'Barra de Santo Antônio', 2),
(12, 'Barra de São Francisco', 8),
(106, 'Barra de São Miguel', 2),
(2584, 'Barra de São Miguel', 15),
(1361, 'Barra do Bugres', 13),
(4766, 'Barra do Chapéu', 26),
(315, 'Barra do Choça', 5),
(1154, 'Barra do Corda', 10),
(1362, 'Barra do Garças', 13),
(3884, 'Barra do Guarita', 23),
(2815, 'Barra do Jacaré', 18),
(316, 'Barra do Mendes', 5),
(5447, 'Barra do Ouro', 27),
(3597, 'Barra do Piraí', 19),
(3885, 'Barra do Quaraí', 23),
(3886, 'Barra do Ribeiro', 23),
(3887, 'Barra do Rio Azul', 23),
(317, 'Barra do Rocha', 5),
(4767, 'Barra do Turvo', 26),
(5356, 'Barra dos Coqueiros', 25),
(3888, 'Barra Funda', 23),
(1626, 'Barra Longa', 11),
(3598, 'Barra Mansa', 19),
(4444, 'Barra Velha', 24),
(2816, 'Barracão', 18),
(3889, 'Barracão', 23),
(3387, 'Barras', 17),
(721, 'Barreira', 6),
(318, 'Barreiras', 5),
(3388, 'Barreiras do Piauí', 17),
(227, 'Barreirinha', 3),
(1155, 'Barreirinhas', 10),
(3198, 'Barreiros', 16),
(4768, 'Barretos', 26),
(4769, 'Barrinha', 26),
(722, 'Barro', 6),
(319, 'Barro Alto', 5),
(913, 'Barro Alto', 9),
(3389, 'Barro Duro', 17),
(320, 'Barro Preto (antigo Gov. Lomanto Jr.)', 5),
(321, 'Barrocas', 5),
(5448, 'Barrolândia', 27),
(723, 'Barroquinha', 6),
(3890, 'Barros Cassal', 23),
(1627, 'Barroso', 11),
(4770, 'Barueri', 26),
(4771, 'Bastos', 26),
(1498, 'Bataguassu', 12),
(1499, 'Bataiporã', 12),
(107, 'Batalha', 2),
(3390, 'Batalha', 17),
(4772, 'Batatais', 26),
(724, 'Baturité', 6),
(4773, 'Bauru', 26),
(2585, 'Bayeux', 15),
(4774, 'Bebedouro', 26),
(725, 'Beberibe', 6),
(726, 'Bela Cruz', 6),
(1500, 'Bela Vista', 12),
(2817, 'Bela Vista da Caroba', 18),
(914, 'Bela Vista de Goiás', 9),
(1628, 'Bela Vista de Minas', 11),
(1156, 'Bela Vista do Maranhão', 10),
(2818, 'Bela Vista do Paraíso', 18),
(3391, 'Bela Vista do Piauí', 17),
(4445, 'Bela Vista do Toldo', 24),
(1157, 'Belágua', 10),
(108, 'Belém', 2),
(2436, 'Belém', 14),
(2586, 'Belém', 15),
(3199, 'Belém de Maria', 16),
(3200, 'Belém de São Francisco', 16),
(2587, 'Belém do Brejo do Cruz', 15),
(3392, 'Belém do Piauí', 17),
(3599, 'Belford Roxo', 19),
(1629, 'Belmiro Braga', 11),
(322, 'Belmonte', 5),
(4446, 'Belmonte', 24),
(323, 'Belo Campo', 5),
(1630, 'Belo Horizonte', 11),
(3201, 'Belo Jardim', 16),
(109, 'Belo Monte', 2),
(1631, 'Belo Oriente', 11),
(1632, 'Belo Vale', 11),
(2437, 'Belterra', 14),
(3393, 'Beneditinos', 17),
(1158, 'Benedito Leite', 10),
(4447, 'Benedito Novo', 24),
(2438, 'Benevides', 14),
(228, 'Benjamin Constant', 3),
(3891, 'Benjamin Constant do Sul', 23),
(4775, 'Bento de Abreu', 26),
(3699, 'Bento Fernandes', 20),
(3892, 'Bento Gonçalves', 23),
(1159, 'Bequimão', 10),
(1633, 'Berilo', 11),
(1634, 'Berizal', 11),
(2588, 'Bernardino Batista', 15),
(4776, 'Bernardino de Campos', 26),
(1160, 'Bernardo do Mearim', 10),
(5449, 'Bernardo Sayão', 27),
(4777, 'Bertioga', 26),
(3394, 'Bertolínia', 17),
(1635, 'Bertópolis', 11),
(229, 'Beruri', 3),
(3202, 'Betânia', 16),
(3395, 'Betânia do Piauí', 17),
(1636, 'Betim', 11),
(3203, 'Bezerros', 16),
(1637, 'Bias Fortes', 11),
(1638, 'Bicas', 11),
(4448, 'Biguaçu', 24),
(4778, 'Bilac', 26),
(1639, 'Biquinhas', 11),
(4779, 'Birigui', 26),
(4780, 'Biritiba-Mirim', 26),
(324, 'Biritinga', 5),
(2819, 'Bituruna', 18),
(4449, 'Blumenau', 24),
(13, 'Boa Esperança', 8),
(1640, 'Boa Esperança', 11),
(2820, 'Boa Esperança', 18),
(2821, 'Boa Esperança do Iguaçu', 18),
(4781, 'Boa Esperança do Sul', 26),
(3396, 'Boa Hora', 17),
(325, 'Boa Nova', 5),
(2589, 'Boa Ventura', 15),
(2822, 'Boa Ventura de São Roque', 18),
(727, 'Boa Viagem', 6),
(2590, 'Boa Vista', 15),
(4400, 'Boa Vista', 22),
(2823, 'Boa Vista da Aparecida', 18),
(3893, 'Boa Vista das Missões', 23),
(3894, 'Boa Vista do Buricá', 23),
(3895, 'Boa Vista do Cadeado', 23),
(1161, 'Boa Vista do Gurupi', 10),
(3896, 'Boa Vista do Incra', 23),
(230, 'Boa Vista do Ramos', 3),
(3897, 'Boa Vista do Sul', 23),
(326, 'Boa Vista do Tupim', 5),
(110, 'Boca da Mata', 2),
(231, 'Boca do Acre', 3),
(3397, 'Bocaina', 17),
(4782, 'Bocaina', 26),
(1641, 'Bocaina de Minas', 11),
(4450, 'Bocaina do Sul', 24),
(1642, 'Bocaiúva', 11),
(2824, 'Bocaiúva do Sul', 18),
(3700, 'Bodó', 20),
(3204, 'Bodocó', 16),
(1501, 'Bodoquena', 12),
(4783, 'Bofete', 26),
(4784, 'Boituva', 26),
(3205, 'Bom Conselho', 16),
(1643, 'Bom Despacho', 11),
(1162, 'Bom Jardim', 10),
(3206, 'Bom Jardim', 16),
(3600, 'Bom Jardim', 19),
(4451, 'Bom Jardim da Serra', 24),
(915, 'Bom Jardim de Goiás', 9),
(1644, 'Bom Jardim de Minas', 11),
(2591, 'Bom Jesus', 15),
(3398, 'Bom Jesus', 17),
(3701, 'Bom Jesus', 20),
(3898, 'Bom Jesus', 23),
(4452, 'Bom Jesus', 24),
(327, 'Bom Jesus da Lapa', 5),
(1645, 'Bom Jesus da Penha', 11),
(328, 'Bom Jesus da Serra', 5),
(1163, 'Bom Jesus das Selvas', 10),
(916, 'Bom Jesus de Goiás', 9),
(1646, 'Bom Jesus do Amparo', 11),
(1363, 'Bom Jesus do Araguaia', 13),
(1647, 'Bom Jesus do Galho', 11),
(3601, 'Bom Jesus do Itabapoana', 19),
(14, 'Bom Jesus do Norte', 8),
(4453, 'Bom Jesus do Oeste', 24),
(2825, 'Bom Jesus do Sul', 18),
(2439, 'Bom Jesus do Tocantins', 14),
(5450, 'Bom Jesus do Tocantins', 27),
(4785, 'Bom Jesus dos Perdões', 26),
(1164, 'Bom Lugar', 10),
(3899, 'Bom Princípio', 23),
(3399, 'Bom Princípio do Piauí', 17),
(3900, 'Bom Progresso', 23),
(1648, 'Bom Repouso', 11),
(4454, 'Bom Retiro', 24),
(3901, 'Bom Retiro do Sul', 23),
(1649, 'Bom Sucesso', 11),
(2592, 'Bom Sucesso', 15),
(2826, 'Bom Sucesso', 18),
(4786, 'Bom Sucesso de Itararé', 26),
(2827, 'Bom Sucesso do Sul', 18),
(4455, 'Bombinhas', 24),
(1650, 'Bonfim', 11),
(4401, 'Bonfim', 22),
(3400, 'Bonfim do Piauí', 17),
(917, 'Bonfinópolis', 9),
(1651, 'Bonfinópolis de Minas', 11),
(329, 'Boninal', 5),
(330, 'Bonito', 5),
(1502, 'Bonito', 12),
(2440, 'Bonito', 14),
(3207, 'Bonito', 16),
(1652, 'Bonito de Minas', 11),
(2593, 'Bonito de Santa Fé', 15),
(918, 'Bonópolis', 9),
(2594, 'Boqueirão', 15),
(3902, 'Boqueirão do Leão', 23),
(3401, 'Boqueirão do Piauí', 17),
(5357, 'Boquim', 25),
(331, 'Boquira', 5),
(4787, 'Borá', 26),
(4788, 'Boracéia', 26),
(232, 'Borba', 3),
(2595, 'Borborema', 15),
(4789, 'Borborema', 26),
(1653, 'Borda da Mata', 11),
(4790, 'Borebi', 26),
(2828, 'Borrazópolis', 18),
(3903, 'Bossoroca', 23),
(1654, 'Botelhos', 11),
(4791, 'Botucatu', 26),
(1655, 'Botumirim', 11),
(332, 'Botuporã', 5),
(4456, 'Botuverá', 24),
(3904, 'Bozano', 23),
(4457, 'Braço do Norte', 24),
(4458, 'Braço do Trombudo', 24),
(3905, 'Braga', 23),
(2441, 'Bragança', 14),
(4792, 'Bragança Paulista', 26),
(2829, 'Braganey', 18),
(111, 'Branquinha', 2),
(1656, 'Brás Pires', 11),
(2442, 'Brasil Novo', 14),
(1503, 'Brasilândia', 12),
(1657, 'Brasilândia de Minas', 11),
(2830, 'Brasilândia do Sul', 18),
(5451, 'Brasilândia do Tocantins', 27),
(81, 'Brasiléia', 1),
(3402, 'Brasileira', 17),
(882, 'Brasília', 7),
(1658, 'Brasília de Minas', 11),
(1364, 'Brasnorte', 13),
(1659, 'Brasópolis', 11),
(4793, 'Braúna', 26),
(1660, 'Braúnas', 11),
(919, 'Brazabrantes', 9),
(3208, 'Brejão', 16),
(15, 'Brejetuba', 8),
(3209, 'Brejinho', 16),
(3702, 'Brejinho', 20),
(5452, 'Brejinho de Nazaré', 27),
(1165, 'Brejo', 10),
(4794, 'Brejo Alegre', 26),
(3210, 'Brejo da Madre de Deus', 16),
(1166, 'Brejo de Areia', 10),
(2596, 'Brejo do Cruz', 15),
(3403, 'Brejo do Piauí', 17),
(2597, 'Brejo dos Santos', 15),
(5358, 'Brejo Grande', 25),
(2443, 'Brejo Grande do Araguaia', 14),
(728, 'Brejo Santo', 6),
(333, 'Brejões', 5),
(334, 'Brejolândia', 5),
(2444, 'Breu Branco', 14),
(2445, 'Breves', 14),
(920, 'Britânia', 9),
(3906, 'Brochier', 23),
(4795, 'Brodowski', 26),
(4796, 'Brotas', 26),
(335, 'Brotas de Macaúbas', 5),
(1661, 'Brumadinho', 11),
(336, 'Brumado', 5),
(4459, 'Brunópolis', 24),
(4460, 'Brusque', 24),
(1662, 'Bueno Brandão', 11),
(1663, 'Buenópolis', 11),
(3211, 'Buenos Aires', 16),
(337, 'Buerarema', 5),
(1664, 'Bugre', 11),
(3212, 'Buíque', 16),
(82, 'Bujari', 1),
(2446, 'Bujaru', 14),
(4797, 'Buri', 26),
(4798, 'Buritama', 26),
(1167, 'Buriti', 10),
(921, 'Buriti Alegre', 9),
(1168, 'Buriti Bravo', 10),
(922, 'Buriti de Goiás', 9),
(5453, 'Buriti do Tocantins', 27),
(3404, 'Buriti dos Lopes', 17),
(3405, 'Buriti dos Montes', 17),
(1169, 'Buriticupu', 10),
(923, 'Buritinópolis', 9),
(338, 'Buritirama', 5),
(1170, 'Buritirana', 10),
(1665, 'Buritis', 11),
(4351, 'Buritis', 21),
(4799, 'Buritizal', 26),
(1666, 'Buritizeiro', 11),
(3907, 'Butiá', 23),
(233, 'Caapiranga', 3),
(2598, 'Caaporã', 15),
(1504, 'Caarapó', 12),
(339, 'Caatiba', 5),
(2599, 'Cabaceiras', 15),
(340, 'Cabaceiras do Paraguaçu', 5),
(1667, 'Cabeceira Grande', 11),
(924, 'Cabeceiras', 9),
(3406, 'Cabeceiras do Piauí', 17),
(2600, 'Cabedelo', 15),
(4352, 'Cabixi', 21),
(3213, 'Cabo de Santo Agostinho', 16),
(3602, 'Cabo Frio', 19),
(1668, 'Cabo Verde', 11),
(4800, 'Cabrália Paulista', 26),
(4801, 'Cabreúva', 26),
(3214, 'Cabrobó', 16),
(4461, 'Caçador', 24),
(4802, 'Caçapava', 26),
(3908, 'Caçapava do Sul', 23),
(4353, 'Cacaulândia', 21),
(3909, 'Cacequi', 23),
(1365, 'Cáceres', 13),
(341, 'Cachoeira', 5),
(925, 'Cachoeira Alta', 9),
(1669, 'Cachoeira da Prata', 11),
(926, 'Cachoeira de Goiás', 9),
(1670, 'Cachoeira de Minas', 11),
(1671, 'Cachoeira de Pajeú', 11),
(2447, 'Cachoeira do Arari', 14),
(2448, 'Cachoeira do Piriá', 14),
(3910, 'Cachoeira do Sul', 23),
(2601, 'Cachoeira dos Índios', 15),
(927, 'Cachoeira Dourada', 9),
(1672, 'Cachoeira Dourada', 11),
(1171, 'Cachoeira Grande', 10),
(4803, 'Cachoeira Paulista', 26),
(3603, 'Cachoeiras de Macacu', 19),
(3215, 'Cachoeirinha', 16),
(3911, 'Cachoeirinha', 23),
(5454, 'Cachoeirinha', 27),
(16, 'Cachoeiro de Itapemirim', 8),
(2602, 'Cacimba de Areia', 15),
(2603, 'Cacimba de Dentro', 15),
(2604, 'Cacimbas', 15),
(112, 'Cacimbinhas', 2),
(3912, 'Cacique Doble', 23),
(4354, 'Cacoal', 21),
(4804, 'Caconde', 26),
(928, 'Caçu', 9),
(342, 'Caculé', 5),
(343, 'Caém', 5),
(1673, 'Caetanópolis', 11),
(344, 'Caetanos', 5),
(1674, 'Caeté', 11),
(3216, 'Caetés', 16),
(345, 'Caetité', 5),
(346, 'Cafarnaum', 5),
(2831, 'Cafeara', 18),
(2832, 'Cafelândia', 18),
(4805, 'Cafelândia', 26),
(2833, 'Cafezal do Sul', 18),
(4806, 'Caiabu', 26),
(1675, 'Caiana', 11),
(929, 'Caiapônia', 9),
(3913, 'Caibaté', 23),
(4462, 'Caibi', 24),
(2605, 'Caiçara', 15),
(3914, 'Caiçara', 23),
(3703, 'Caiçara do Norte', 20),
(3704, 'Caiçara do Rio do Vento', 20),
(3705, 'Caicó', 20),
(4807, 'Caieiras', 26),
(347, 'Cairu', 5),
(4808, 'Caiuá', 26),
(4809, 'Cajamar', 26),
(1172, 'Cajapió', 10),
(1173, 'Cajari', 10),
(4810, 'Cajati', 26),
(2606, 'Cajazeiras', 15),
(3407, 'Cajazeiras do Piauí', 17),
(2607, 'Cajazeirinhas', 15),
(4811, 'Cajobi', 26),
(113, 'Cajueiro', 2),
(3408, 'Cajueiro da Praia', 17),
(1676, 'Cajuri', 11),
(4812, 'Cajuru', 26),
(3217, 'Calçado', 16),
(204, 'Calçoene', 4),
(1677, 'Caldas', 11),
(2608, 'Caldas Brandão', 15),
(930, 'Caldas Novas', 9),
(931, 'Caldazinha', 9),
(348, 'Caldeirão Grande', 5),
(3409, 'Caldeirão Grande do Piauí', 17),
(2834, 'Califórnia', 18),
(4463, 'Calmon', 24),
(3218, 'Calumbi', 16),
(349, 'Camacan', 5),
(350, 'Camaçari', 5),
(1678, 'Camacho', 11),
(2609, 'Camalaú', 15),
(351, 'Camamu', 5),
(1679, 'Camanducaia', 11),
(1505, 'Camapuã', 12),
(3915, 'Camaquã', 23),
(3219, 'Camaragibe', 16),
(3916, 'Camargo', 23),
(2835, 'Cambará', 18),
(3917, 'Cambará do Sul', 23),
(2836, 'Cambé', 18),
(2837, 'Cambira', 18),
(4464, 'Camboriú', 24),
(3604, 'Cambuci', 19),
(1680, 'Cambuí', 11),
(1681, 'Cambuquira', 11),
(2449, 'Cametá', 14),
(729, 'Camocim', 6),
(3220, 'Camocim de São Félix', 16),
(1682, 'Campanário', 11),
(1683, 'Campanha', 11),
(114, 'Campestre', 2),
(1684, 'Campestre', 11),
(3918, 'Campestre da Serra', 23),
(932, 'Campestre de Goiás', 9),
(1174, 'Campestre do Maranhão', 10),
(2838, 'Campina da Lagoa', 18),
(3919, 'Campina das Missões', 23),
(4813, 'Campina do Monte Alegre', 26),
(2839, 'Campina do Simão', 18),
(2610, 'Campina Grande', 15),
(2840, 'Campina Grande do Sul', 18),
(1685, 'Campina Verde', 11),
(933, 'Campinaçu', 9),
(1366, 'Campinápolis', 13),
(4814, 'Campinas', 26),
(3410, 'Campinas do Piauí', 17),
(3920, 'Campinas do Sul', 23),
(934, 'Campinorte', 9),
(115, 'Campo Alegre', 2),
(4465, 'Campo Alegre', 24),
(935, 'Campo Alegre de Goiás', 9),
(352, 'Campo Alegre de Lourdes', 5),
(3411, 'Campo Alegre do Fidalgo', 17),
(1686, 'Campo Azul', 11),
(1687, 'Campo Belo', 11),
(4466, 'Campo Belo do Sul', 24),
(3921, 'Campo Bom', 23),
(2841, 'Campo Bonito', 18),
(2611, 'Campo de Santana', 15),
(5359, 'Campo do Brito', 25),
(1688, 'Campo do Meio', 11),
(2842, 'Campo do Tenente', 18),
(4467, 'Campo Erê', 24),
(1689, 'Campo Florido', 11),
(353, 'Campo Formoso', 5),
(116, 'Campo Grande', 2),
(1506, 'Campo Grande', 12),
(3412, 'Campo Grande do Piauí', 17),
(2843, 'Campo Largo', 18),
(3413, 'Campo Largo do Piauí', 17),
(936, 'Campo Limpo de Goiás', 9),
(4815, 'Campo Limpo Paulista', 26),
(2844, 'Campo Magro', 18),
(3414, 'Campo Maior', 17),
(2845, 'Campo Mourão', 18),
(3922, 'Campo Novo', 23),
(4355, 'Campo Novo de Rondônia', 21),
(1367, 'Campo Novo do Parecis', 13),
(3706, 'Campo Redondo', 20),
(1368, 'Campo Verde', 13),
(1690, 'Campos Altos', 11),
(937, 'Campos Belos', 9),
(3923, 'Campos Borges', 23),
(1369, 'Campos de Júlio', 13),
(4816, 'Campos do Jordão', 26),
(3605, 'Campos dos Goytacazes', 19),
(1691, 'Campos Gerais', 11),
(5455, 'Campos Lindos', 27),
(4468, 'Campos Novos', 24),
(4817, 'Campos Novos Paulista', 26),
(730, 'Campos Sales', 6),
(938, 'Campos Verdes', 9),
(3221, 'Camutanga', 16),
(1692, 'Cana Verde', 11),
(1693, 'Canaã', 11),
(2450, 'Canaã dos Carajás', 14),
(1370, 'Canabrava do Norte', 13),
(4818, 'Cananéia', 26),
(117, 'Canapi', 2),
(354, 'Canápolis', 5),
(1694, 'Canápolis', 11),
(355, 'Canarana', 5),
(1371, 'Canarana', 13),
(4819, 'Canas', 26),
(3415, 'Canavieira', 17),
(356, 'Canavieiras', 5),
(357, 'Candeal', 5),
(358, 'Candeias', 5),
(1695, 'Candeias', 11),
(4356, 'Candeias do Jamari', 21),
(3924, 'Candelária', 23),
(359, 'Candiba', 5),
(2846, 'Cândido de Abreu', 18),
(3925, 'Cândido Godói', 23),
(1175, 'Cândido Mendes', 10),
(4820, 'Cândido Mota', 26),
(4821, 'Cândido Rodrigues', 26),
(360, 'Cândido Sales', 5),
(3926, 'Candiota', 23),
(2847, 'Candói', 18),
(3927, 'Canela', 23),
(4469, 'Canelinha', 24),
(3707, 'Canguaretama', 20),
(3928, 'Canguçu', 23),
(5360, 'Canhoba', 25),
(3222, 'Canhotinho', 16),
(731, 'Canindé', 6),
(5361, 'Canindé de São Francisco', 25),
(4822, 'Canitar', 26),
(3929, 'Canoas', 23),
(4470, 'Canoinhas', 24),
(361, 'Cansanção', 5),
(4402, 'Cantá', 22),
(1696, 'Cantagalo', 11),
(2848, 'Cantagalo', 18),
(3606, 'Cantagalo', 19),
(1176, 'Cantanhede', 10),
(3416, 'Canto do Buriti', 17),
(362, 'Canudos', 5),
(3930, 'Canudos do Vale', 23),
(234, 'Canutama', 3),
(2451, 'Capanema', 14),
(2849, 'Capanema', 18),
(4471, 'Capão Alto', 24),
(4823, 'Capão Bonito', 26),
(3931, 'Capão Bonito do Sul', 23),
(3932, 'Capão da Canoa', 23),
(3933, 'Capão do Cipó', 23),
(3934, 'Capão do Leão', 23),
(1697, 'Caparaó', 11),
(118, 'Capela', 2),
(5362, 'Capela', 25),
(3935, 'Capela de Santana', 23),
(4824, 'Capela do Alto', 26),
(363, 'Capela do Alto Alegre', 5),
(1698, 'Capela Nova', 11),
(1699, 'Capelinha', 11),
(1700, 'Capetinga', 11),
(2612, 'Capim', 15),
(1701, 'Capim Branco', 11),
(364, 'Capim Grosso', 5),
(1702, 'Capinópolis', 11),
(4472, 'Capinzal', 24),
(1177, 'Capinzal do Norte', 10),
(732, 'Capistrano', 6),
(3936, 'Capitão', 23),
(1703, 'Capitão Andrade', 11),
(3417, 'Capitão de Campos', 17),
(1704, 'Capitão Enéas', 11),
(3418, 'Capitão Gervásio Oliveira', 17),
(2850, 'Capitão Leônidas Marques', 18),
(2452, 'Capitão Poço', 14),
(1705, 'Capitólio', 11),
(4825, 'Capivari', 26),
(4473, 'Capivari de Baixo', 24),
(3937, 'Capivari do Sul', 23),
(83, 'Capixaba', 1),
(3223, 'Capoeiras', 16),
(1706, 'Caputira', 11),
(3938, 'Caraá', 23),
(4403, 'Caracaraí', 22),
(1507, 'Caracol', 12),
(3419, 'Caracol', 17),
(4826, 'Caraguatatuba', 26),
(1707, 'Caraí', 11),
(365, 'Caraíbas', 5),
(2851, 'Carambeí', 18),
(1708, 'Caranaíba', 11),
(1709, 'Carandaí', 11),
(1710, 'Carangola', 11),
(3607, 'Carapebus', 19),
(4827, 'Carapicuíba', 26),
(1711, 'Caratinga', 11),
(235, 'Carauari', 3),
(2613, 'Caraúbas', 15),
(3708, 'Caraúbas', 20),
(3420, 'Caraúbas do Piauí', 17),
(366, 'Caravelas', 5),
(3939, 'Carazinho', 23),
(1712, 'Carbonita', 11),
(367, 'Cardeal da Silva', 5),
(4828, 'Cardoso', 26),
(3608, 'Cardoso Moreira', 19),
(1713, 'Careaçu', 11),
(236, 'Careiro', 3),
(237, 'Careiro da Várzea', 3),
(17, 'Cariacica', 8),
(733, 'Caridade', 6),
(3421, 'Caridade do Piauí', 17),
(368, 'Carinhanha', 5),
(5363, 'Carira', 25),
(734, 'Cariré', 6),
(5456, 'Cariri do Tocantins', 27),
(735, 'Caririaçu', 6),
(736, 'Cariús', 6),
(1372, 'Carlinda', 13),
(2852, 'Carlópolis', 18),
(3940, 'Carlos Barbosa', 23),
(1714, 'Carlos Chagas', 11),
(3941, 'Carlos Gomes', 23),
(1715, 'Carmésia', 11),
(3609, 'Carmo', 19),
(1716, 'Carmo da Cachoeira', 11),
(1717, 'Carmo da Mata', 11),
(1718, 'Carmo de Minas', 11),
(1719, 'Carmo do Cajuru', 11),
(1720, 'Carmo do Paranaíba', 11),
(1721, 'Carmo do Rio Claro', 11),
(939, 'Carmo do Rio Verde', 9),
(5457, 'Carmolândia', 27),
(5364, 'Carmópolis', 25),
(1722, 'Carmópolis de Minas', 11),
(3224, 'Carnaíba', 16),
(3709, 'Carnaúba dos Dantas', 20),
(3710, 'Carnaubais', 20),
(737, 'Carnaubal', 6),
(3225, 'Carnaubeira da Penha', 16),
(1723, 'Carneirinho', 11),
(119, 'Carneiros', 2),
(4404, 'Caroebe', 22),
(1178, 'Carolina', 10),
(3226, 'Carpina', 16),
(1724, 'Carrancas', 11),
(2614, 'Carrapateira', 15),
(5458, 'Carrasco Bonito', 27),
(3227, 'Caruaru', 16),
(1179, 'Carutapera', 10),
(1725, 'Carvalhópolis', 11),
(1726, 'Carvalhos', 11),
(4829, 'Casa Branca', 26),
(1727, 'Casa Grande', 11),
(369, 'Casa Nova', 5),
(3942, 'Casca', 23),
(1728, 'Cascalho Rico', 11),
(738, 'Cascavel', 6),
(2853, 'Cascavel', 18),
(5459, 'Caseara', 27),
(3943, 'Caseiros', 23),
(3610, 'Casimiro de Abreu', 19),
(3228, 'Casinhas', 16),
(2615, 'Casserengue', 15),
(1729, 'Cássia', 11),
(4830, 'Cássia dos Coqueiros', 26),
(1508, 'Cassilândia', 12),
(2453, 'Castanhal', 14),
(1373, 'Castanheira', 13),
(4357, 'Castanheiras', 21),
(940, 'Castelândia', 9),
(18, 'Castelo', 8),
(3422, 'Castelo do Piauí', 17),
(4831, 'Castilho', 26),
(2854, 'Castro', 18),
(370, 'Castro Alves', 5),
(1730, 'Cataguases', 11),
(941, 'Catalão', 9),
(4832, 'Catanduva', 26),
(2855, 'Catanduvas', 18),
(4474, 'Catanduvas', 24),
(739, 'Catarina', 6),
(1731, 'Catas Altas', 11),
(1732, 'Catas Altas da Noruega', 11),
(3229, 'Catende', 16),
(4833, 'Catiguá', 26),
(2616, 'Catingueira', 15),
(371, 'Catolândia', 5),
(2617, 'Catolé do Rocha', 15),
(372, 'Catu', 5),
(3944, 'Catuípe', 23),
(1733, 'Catuji', 11),
(740, 'Catunda', 6),
(942, 'Caturaí', 9),
(373, 'Caturama', 5),
(2618, 'Caturité', 15),
(1734, 'Catuti', 11),
(741, 'Caucaia', 6),
(943, 'Cavalcante', 9),
(1735, 'Caxambu', 11),
(4475, 'Caxambu do Sul', 24),
(1180, 'Caxias', 10),
(3945, 'Caxias do Sul', 23),
(3423, 'Caxingó', 17),
(3711, 'Ceará-Mirim', 20),
(1181, 'Cedral', 10),
(4834, 'Cedral', 26),
(742, 'Cedro', 6),
(3230, 'Cedro', 16),
(5365, 'Cedro de São João', 25),
(1736, 'Cedro do Abaeté', 11),
(4476, 'Celso Ramos', 24),
(3946, 'Centenário', 23),
(5460, 'Centenário', 27),
(2856, 'Centenário do Sul', 18),
(374, 'Central', 5),
(1737, 'Central de Minas', 11),
(1182, 'Central do Maranhão', 10),
(1738, 'Centralina', 11),
(1183, 'Centro do Guilherme', 10),
(1184, 'Centro Novo do Maranhão', 10),
(4358, 'Cerejeiras', 21),
(944, 'Ceres', 9),
(4835, 'Cerqueira César', 26),
(4836, 'Cerquilho', 26),
(3947, 'Cerrito', 23),
(2857, 'Cerro Azul', 18),
(3948, 'Cerro Branco', 23),
(3712, 'Cerro Corá', 20),
(3949, 'Cerro Grande', 23),
(3950, 'Cerro Grande do Sul', 23),
(3951, 'Cerro Largo', 23),
(4477, 'Cerro Negro', 24),
(4837, 'Cesário Lange', 26),
(2858, 'Céu Azul', 18),
(945, 'Cezarina', 9),
(3231, 'Chã de Alegria', 16),
(3232, 'Chã Grande', 16),
(120, 'Chã Preta', 2),
(1739, 'Chácara', 11),
(1740, 'Chalé', 11),
(3952, 'Chapada', 23),
(5461, 'Chapada da Natividade', 27),
(5462, 'Chapada de Areia', 27),
(1741, 'Chapada do Norte', 11),
(1374, 'Chapada dos Guimarães', 13),
(1742, 'Chapada Gaúcha', 11),
(946, 'Chapadão do Céu', 9),
(4478, 'Chapadão do Lageado', 24),
(1509, 'Chapadão do Sul', 12),
(1185, 'Chapadinha', 10),
(4479, 'Chapecó', 24),
(4838, 'Charqueada', 26),
(3953, 'Charqueadas', 23),
(3954, 'Charrua', 23),
(743, 'Chaval', 6),
(4839, 'Chavantes', 26),
(2454, 'Chaves', 14),
(1743, 'Chiador', 11),
(3955, 'Chiapeta', 23),
(2859, 'Chopinzinho', 18),
(744, 'Choró', 6),
(745, 'Chorozinho', 6),
(375, 'Chorrochó', 5),
(3956, 'Chuí', 23),
(4359, 'Chupinguaia', 21),
(3957, 'Chuvisca', 23),
(2860, 'Cianorte', 18),
(376, 'Cícero Dantas', 5),
(2861, 'Cidade Gaúcha', 18),
(947, 'Cidade Ocidental', 9),
(1186, 'Cidelândia', 10),
(3958, 'Cidreira', 23),
(377, 'Cipó', 5),
(1744, 'Cipotânea', 11),
(3959, 'Ciríaco', 23),
(1745, 'Claraval', 11),
(1746, 'Claro dos Poções', 11),
(1375, 'Cláudia', 13),
(1747, 'Cláudio', 11),
(4840, 'Clementina', 26),
(2862, 'Clevelândia', 18),
(378, 'Coaraci', 5),
(238, 'Coari', 3),
(3424, 'Cocal', 17),
(3425, 'Cocal de Telha', 17),
(4480, 'Cocal do Sul', 24),
(3426, 'Cocal dos Alves', 17),
(1376, 'Cocalinho', 13),
(948, 'Cocalzinho de Goiás', 9),
(379, 'Cocos', 5),
(239, 'Codajás', 3),
(1187, 'Codó', 10),
(1188, 'Coelho Neto', 10),
(1748, 'Coimbra', 11),
(121, 'Coité do Nóia', 2),
(3427, 'Coivaras', 17),
(2455, 'Colares', 14),
(19, 'Colatina', 8),
(1377, 'Colíder', 13),
(4841, 'Colina', 26),
(1189, 'Colinas', 10),
(3960, 'Colinas', 23),
(949, 'Colinas do Sul', 9),
(5463, 'Colinas do Tocantins', 27),
(5464, 'Colméia', 27),
(1378, 'Colniza', 13),
(4842, 'Colômbia', 26),
(2863, 'Colombo', 18),
(3428, 'Colônia do Gurguéia', 17),
(3429, 'Colônia do Piauí', 17),
(122, 'Colônia Leopoldina', 2),
(2864, 'Colorado', 18),
(3961, 'Colorado', 23),
(4360, 'Colorado do Oeste', 21),
(1749, 'Coluna', 11),
(5465, 'Combinado', 27),
(1750, 'Comendador Gomes', 11),
(3611, 'Comendador Levy Gasparian', 19),
(1751, 'Comercinho', 11),
(1379, 'Comodoro', 13),
(2619, 'Conceição', 15),
(1752, 'Conceição da Aparecida', 11),
(20, 'Conceição da Barra', 8),
(1753, 'Conceição da Barra de Minas', 11),
(380, 'Conceição da Feira', 5),
(1754, 'Conceição das Alagoas', 11),
(1755, 'Conceição das Pedras', 11),
(1756, 'Conceição de Ipanema', 11),
(3612, 'Conceição de Macabu', 19),
(381, 'Conceição do Almeida', 5),
(2456, 'Conceição do Araguaia', 14),
(3430, 'Conceição do Canindé', 17),
(21, 'Conceição do Castelo', 8),
(382, 'Conceição do Coité', 5),
(383, 'Conceição do Jacuípe', 5),
(1190, 'Conceição do Lago-Açu', 10),
(1757, 'Conceição do Mato Dentro', 11),
(1758, 'Conceição do Pará', 11),
(1759, 'Conceição do Rio Verde', 11),
(5466, 'Conceição do Tocantins', 27),
(1760, 'Conceição dos Ouros', 11),
(4843, 'Conchal', 26),
(4844, 'Conchas', 26),
(4481, 'Concórdia', 24),
(2457, 'Concórdia do Pará', 14),
(2620, 'Condado', 15),
(3233, 'Condado', 16),
(384, 'Conde', 5),
(2621, 'Conde', 15),
(385, 'Condeúba', 5),
(3962, 'Condor', 23),
(1761, 'Cônego Marinho', 11),
(1762, 'Confins', 11),
(1380, 'Confresa', 13),
(2622, 'Congo', 15),
(1763, 'Congonhal', 11),
(1764, 'Congonhas', 11),
(1765, 'Congonhas do Norte', 11),
(2865, 'Congonhinhas', 18),
(1766, 'Conquista', 11),
(1381, 'Conquista d`Oeste', 13),
(1767, 'Conselheiro Lafaiete', 11),
(2866, 'Conselheiro Mairinck', 18),
(1768, 'Conselheiro Pena', 11),
(1769, 'Consolação', 11),
(3963, 'Constantina', 23),
(1770, 'Contagem', 11),
(2867, 'Contenda', 18),
(386, 'Contendas do Sincorá', 5),
(1771, 'Coqueiral', 11),
(3964, 'Coqueiro Baixo', 23),
(123, 'Coqueiro Seco', 2),
(3965, 'Coqueiros do Sul', 23),
(1772, 'Coração de Jesus', 11),
(387, 'Coração de Maria', 5),
(2868, 'Corbélia', 18),
(3613, 'Cordeiro', 19),
(4845, 'Cordeirópolis', 26),
(388, 'Cordeiros', 5),
(4482, 'Cordilheira Alta', 24),
(1773, 'Cordisburgo', 11),
(1774, 'Cordislândia', 11),
(746, 'Coreaú', 6),
(2623, 'Coremas', 15),
(1510, 'Corguinho', 12),
(389, 'Coribe', 5),
(1775, 'Corinto', 11),
(2869, 'Cornélio Procópio', 18),
(1776, 'Coroaci', 11),
(4846, 'Coroados', 26),
(1191, 'Coroatá', 10),
(1777, 'Coromandel', 11),
(3966, 'Coronel Barros', 23),
(3967, 'Coronel Bicaco', 23),
(2870, 'Coronel Domingos Soares', 18),
(3713, 'Coronel Ezequiel', 20),
(1778, 'Coronel Fabriciano', 11),
(4483, 'Coronel Freitas', 24),
(3714, 'Coronel João Pessoa', 20),
(390, 'Coronel João Sá', 5),
(3431, 'Coronel José Dias', 17),
(4847, 'Coronel Macedo', 26),
(4484, 'Coronel Martins', 24),
(1779, 'Coronel Murta', 11),
(1780, 'Coronel Pacheco', 11),
(3968, 'Coronel Pilar', 23),
(1511, 'Coronel Sapucaia', 12),
(2871, 'Coronel Vivida', 18),
(1781, 'Coronel Xavier Chaves', 11),
(1782, 'Córrego Danta', 11),
(1783, 'Córrego do Bom Jesus', 11),
(950, 'Córrego do Ouro', 9),
(1784, 'Córrego Fundo', 11),
(1785, 'Córrego Novo', 11),
(4485, 'Correia Pinto', 24),
(3432, 'Corrente', 17),
(3234, 'Correntes', 16),
(391, 'Correntina', 5),
(3235, 'Cortês', 16),
(1512, 'Corumbá', 12),
(951, 'Corumbá de Goiás', 9),
(952, 'Corumbaíba', 9),
(4848, 'Corumbataí', 26),
(2872, 'Corumbataí do Sul', 18),
(4361, 'Corumbiara', 21),
(4486, 'Corupá', 24),
(124, 'Coruripe', 2),
(4849, 'Cosmópolis', 26),
(4850, 'Cosmorama', 26),
(4362, 'Costa Marques', 21),
(1513, 'Costa Rica', 12),
(392, 'Cotegipe', 5),
(4851, 'Cotia', 26),
(3969, 'Cotiporã', 23),
(1382, 'Cotriguaçu', 13),
(5467, 'Couto de Magalhães', 27),
(1786, 'Couto de Magalhães de Minas', 11),
(3970, 'Coxilha', 23),
(1514, 'Coxim', 12),
(2624, 'Coxixola', 15),
(125, 'Craíbas', 2),
(747, 'Crateús', 6),
(748, 'Crato', 6),
(4852, 'Cravinhos', 26),
(393, 'Cravolândia', 5),
(4487, 'Criciúma', 24),
(1787, 'Crisólita', 11),
(394, 'Crisópolis', 5),
(3971, 'Crissiumal', 23),
(1788, 'Cristais', 11),
(4853, 'Cristais Paulista', 26),
(3972, 'Cristal', 23),
(3973, 'Cristal do Sul', 23),
(5468, 'Cristalândia', 27),
(3433, 'Cristalândia do Piauí', 17),
(1789, 'Cristália', 11),
(953, 'Cristalina', 9),
(1790, 'Cristiano Otoni', 11),
(954, 'Cristianópolis', 9),
(1791, 'Cristina', 11),
(5366, 'Cristinápolis', 25),
(3434, 'Cristino Castro', 17),
(395, 'Cristópolis', 5),
(955, 'Crixás', 9),
(5469, 'Crixás do Tocantins', 27),
(749, 'Croatá', 6),
(956, 'Cromínia', 9),
(1792, 'Crucilândia', 11),
(750, 'Cruz', 6),
(3974, 'Cruz Alta', 23),
(396, 'Cruz das Almas', 5),
(2625, 'Cruz do Espírito Santo', 15),
(2873, 'Cruz Machado', 18),
(4854, 'Cruzália', 26),
(3975, 'Cruzaltense', 23),
(4855, 'Cruzeiro', 26),
(1793, 'Cruzeiro da Fortaleza', 11),
(2874, 'Cruzeiro do Iguaçu', 18),
(2875, 'Cruzeiro do Oeste', 18),
(84, 'Cruzeiro do Sul', 1),
(2876, 'Cruzeiro do Sul', 18),
(3976, 'Cruzeiro do Sul', 23),
(3715, 'Cruzeta', 20),
(1794, 'Cruzília', 11),
(2877, 'Cruzmaltina', 18),
(4856, 'Cubatão', 26),
(2626, 'Cubati', 15),
(1383, 'Cuiabá', 13),
(2627, 'Cuité', 15),
(2628, 'Cuité de Mamanguape', 15),
(2629, 'Cuitegi', 15),
(4363, 'Cujubim', 21),
(957, 'Cumari', 9),
(3236, 'Cumaru', 16),
(2458, 'Cumaru do Norte', 14),
(5367, 'Cumbe', 25),
(4857, 'Cunha', 26),
(4488, 'Cunha Porã', 24),
(4489, 'Cunhataí', 24),
(1795, 'Cuparaque', 11),
(3237, 'Cupira', 16),
(397, 'Curaçá', 5),
(3435, 'Curimatá', 17),
(2459, 'Curionópolis', 14),
(2878, 'Curitiba', 18),
(4490, 'Curitibanos', 24),
(2879, 'Curiúva', 18),
(3436, 'Currais', 17),
(3716, 'Currais Novos', 20),
(2630, 'Curral de Cima', 15),
(1796, 'Curral de Dentro', 11),
(3437, 'Curral Novo do Piauí', 17),
(2631, 'Curral Velho', 15),
(2460, 'Curralinho', 14),
(3438, 'Curralinhos', 17),
(2461, 'Curuá', 14),
(2462, 'Curuçá', 14),
(1192, 'Cururupu', 10),
(1384, 'Curvelândia', 13),
(1797, 'Curvelo', 11),
(3238, 'Custódia', 16),
(205, 'Cutias', 4),
(958, 'Damianópolis', 9),
(2632, 'Damião', 15),
(959, 'Damolândia', 9),
(5470, 'Darcinópolis', 27),
(398, 'Dário Meira', 5),
(1798, 'Datas', 11),
(3977, 'David Canabarro', 23),
(960, 'Davinópolis', 9),
(1193, 'Davinópolis', 10),
(1799, 'Delfim Moreira', 11),
(1800, 'Delfinópolis', 11),
(126, 'Delmiro Gouveia', 2),
(1801, 'Delta', 11),
(3439, 'Demerval Lobão', 17),
(1386, 'Denise', 13),
(1515, 'Deodápolis', 12),
(751, 'Deputado Irapuan Pinheiro', 6),
(3978, 'Derrubadas', 23),
(4858, 'Descalvado', 26),
(4491, 'Descanso', 24),
(1802, 'Descoberto', 11),
(2633, 'Desterro', 15),
(1803, 'Desterro de Entre Rios', 11),
(1804, 'Desterro do Melo', 11),
(3979, 'Dezesseis de Novembro', 23),
(4859, 'Diadema', 26),
(2634, 'Diamante', 15),
(2880, 'Diamante d`Oeste', 18),
(2881, 'Diamante do Norte', 18),
(2882, 'Diamante do Sul', 18),
(1805, 'Diamantina', 11),
(1387, 'Diamantino', 13),
(5471, 'Dianópolis', 27),
(399, 'Dias d`Ávila', 5),
(3980, 'Dilermando de Aguiar', 23),
(1806, 'Diogo de Vasconcelos', 11),
(1807, 'Dionísio', 11),
(4492, 'Dionísio Cerqueira', 24),
(961, 'Diorama', 9),
(4860, 'Dirce Reis', 26),
(3440, 'Dirceu Arcoverde', 17),
(5368, 'Divina Pastora', 25),
(1808, 'Divinésia', 11),
(1809, 'Divino', 11),
(1810, 'Divino das Laranjeiras', 11),
(22, 'Divino de São Lourenço', 8),
(4861, 'Divinolândia', 26),
(1811, 'Divinolândia de Minas', 11),
(1812, 'Divinópolis', 11),
(962, 'Divinópolis de Goiás', 9),
(5472, 'Divinópolis do Tocantins', 27),
(1813, 'Divisa Alegre', 11),
(1814, 'Divisa Nova', 11),
(1815, 'Divisópolis', 11),
(4862, 'Dobrada', 26),
(4863, 'Dois Córregos', 26),
(3981, 'Dois Irmãos', 23),
(3982, 'Dois Irmãos das Missões', 23),
(1516, 'Dois Irmãos do Buriti', 12),
(5473, 'Dois Irmãos do Tocantins', 27),
(3983, 'Dois Lajeados', 23),
(127, 'Dois Riachos', 2),
(2883, 'Dois Vizinhos', 18),
(4864, 'Dolcinópolis', 26),
(1388, 'Dom Aquino', 13),
(400, 'Dom Basílio', 5),
(1816, 'Dom Bosco', 11),
(1817, 'Dom Cavati', 11),
(2463, 'Dom Eliseu', 14),
(3441, 'Dom Expedito Lopes', 17),
(3984, 'Dom Feliciano', 23),
(3442, 'Dom Inocêncio', 17),
(1818, 'Dom Joaquim', 11),
(401, 'Dom Macedo Costa', 5),
(3985, 'Dom Pedrito', 23),
(1194, 'Dom Pedro', 10),
(3986, 'Dom Pedro de Alcântara', 23),
(1819, 'Dom Silvério', 11),
(1820, 'Dom Viçoso', 11),
(23, 'Domingos Martins', 8),
(3443, 'Domingos Mourão', 17),
(4493, 'Dona Emma', 24),
(1821, 'Dona Eusébia', 11),
(3987, 'Dona Francisca', 23),
(2635, 'Dona Inês', 15),
(1822, 'Dores de Campos', 11),
(1823, 'Dores de Guanhães', 11),
(1824, 'Dores do Indaiá', 11),
(24, 'Dores do Rio Preto', 8),
(1825, 'Dores do Turvo', 11),
(1826, 'Doresópolis', 11),
(3239, 'Dormentes', 16),
(1517, 'Douradina', 12),
(2884, 'Douradina', 18),
(4865, 'Dourado', 26),
(1827, 'Douradoquara', 11),
(1518, 'Dourados', 12),
(2885, 'Doutor Camargo', 18),
(3988, 'Doutor Maurício Cardoso', 23),
(4494, 'Doutor Pedrinho', 24),
(3989, 'Doutor Ricardo', 23),
(3717, 'Doutor Severiano', 20),
(2886, 'Doutor Ulysses', 18),
(963, 'Doverlândia', 9),
(4866, 'Dracena', 26),
(4867, 'Duartina', 26),
(3614, 'Duas Barras', 19),
(2636, 'Duas Estradas', 15),
(5474, 'Dueré', 27),
(4868, 'Dumont', 26),
(1195, 'Duque Bacelar', 10),
(3615, 'Duque de Caxias', 19),
(1828, 'Durandé', 11),
(4869, 'Echaporã', 26),
(25, 'Ecoporanga', 8),
(964, 'Edealina', 9),
(965, 'Edéia', 9),
(240, 'Eirunepé', 3),
(1519, 'Eldorado', 12),
(4870, 'Eldorado', 26),
(3990, 'Eldorado do Sul', 23),
(2464, 'Eldorado dos Carajás', 14),
(3444, 'Elesbão Veloso', 17),
(4871, 'Elias Fausto', 26),
(3445, 'Eliseu Martins', 17),
(4872, 'Elisiário', 26),
(402, 'Elísio Medrado', 5),
(1829, 'Elói Mendes', 11),
(2637, 'Emas', 15),
(4873, 'Embaúba', 26),
(4874, 'Embu', 26),
(4875, 'Embu-Guaçu', 26),
(4876, 'Emilianópolis', 26),
(3991, 'Encantado', 23),
(3718, 'Encanto', 20),
(403, 'Encruzilhada', 5),
(3992, 'Encruzilhada do Sul', 23),
(2887, 'Enéas Marques', 18),
(2888, 'Engenheiro Beltrão', 18),
(1830, 'Engenheiro Caldas', 11),
(4877, 'Engenheiro Coelho', 26),
(1831, 'Engenheiro Navarro', 11),
(3616, 'Engenheiro Paulo de Frontin', 19),
(3993, 'Engenho Velho', 23),
(1832, 'Entre Folhas', 11),
(404, 'Entre Rios', 5),
(4495, 'Entre Rios', 24),
(1833, 'Entre Rios de Minas', 11),
(2889, 'Entre Rios do Oeste', 18),
(3994, 'Entre Rios do Sul', 23),
(3995, 'Entre-Ijuís', 23),
(241, 'Envira', 3),
(85, 'Epitaciolândia', 1),
(3719, 'Equador', 20),
(3996, 'Erebango', 23),
(3997, 'Erechim', 23),
(752, 'Ererê', 6),
(405, 'Érico Cardoso', 5),
(4496, 'Ermo', 24),
(3998, 'Ernestina', 23),
(3999, 'Erval Grande', 23),
(4000, 'Erval Seco', 23),
(4497, 'Erval Velho', 24),
(1834, 'Ervália', 11),
(3240, 'Escada', 16),
(4001, 'Esmeralda', 23),
(1835, 'Esmeraldas', 11),
(1836, 'Espera Feliz', 11),
(2638, 'Esperança', 15),
(4002, 'Esperança do Sul', 23),
(2890, 'Esperança Nova', 18),
(3446, 'Esperantina', 17),
(5475, 'Esperantina', 27),
(1196, 'Esperantinópolis', 10),
(2891, 'Espigão Alto do Iguaçu', 18),
(4364, 'Espigão d`Oeste', 21),
(1837, 'Espinosa', 11),
(3720, 'Espírito Santo', 20),
(1838, 'Espírito Santo do Dourado', 11),
(4878, 'Espírito Santo do Pinhal', 26),
(4879, 'Espírito Santo do Turvo', 26),
(406, 'Esplanada', 5),
(4003, 'Espumoso', 23),
(4004, 'Estação', 23),
(5369, 'Estância', 25),
(4005, 'Estância Velha', 23),
(4006, 'Esteio', 23),
(1839, 'Estiva', 11),
(4880, 'Estiva Gerbi', 26),
(1197, 'Estreito', 10),
(4007, 'Estrela', 23),
(4881, 'Estrela d`Oeste', 26),
(1840, 'Estrela Dalva', 11),
(128, 'Estrela de Alagoas', 2),
(1841, 'Estrela do Indaiá', 11),
(966, 'Estrela do Norte', 9),
(4882, 'Estrela do Norte', 26),
(1842, 'Estrela do Sul', 11),
(4008, 'Estrela Velha', 23),
(407, 'Euclides da Cunha', 5),
(4883, 'Euclides da Cunha Paulista', 26),
(4009, 'Eugênio de Castro', 23),
(1843, 'Eugenópolis', 11),
(408, 'Eunápolis', 5),
(753, 'Eusébio', 6),
(1844, 'Ewbank da Câmara', 11),
(1845, 'Extrema', 11),
(3721, 'Extremoz', 20),
(3241, 'Exu', 16),
(2639, 'Fagundes', 15),
(4010, 'Fagundes Varela', 23),
(967, 'Faina', 9),
(1846, 'Fama', 11),
(1847, 'Faria Lemos', 11),
(754, 'Farias Brito', 6),
(2465, 'Faro', 14),
(2892, 'Farol', 18),
(4011, 'Farroupilha', 23),
(4884, 'Fartura', 26),
(3447, 'Fartura do Piauí', 17),
(409, 'Fátima', 5),
(5476, 'Fátima', 27),
(1520, 'Fátima do Sul', 12),
(2893, 'Faxinal', 18),
(4012, 'Faxinal do Soturno', 23),
(4498, 'Faxinal dos Guedes', 24),
(4013, 'Faxinalzinho', 23),
(968, 'Fazenda Nova', 9),
(2894, 'Fazenda Rio Grande', 18),
(4014, 'Fazenda Vilanova', 23),
(86, 'Feijó', 1),
(410, 'Feira da Mata', 5),
(411, 'Feira de Santana', 5),
(129, 'Feira Grande', 2),
(3242, 'Feira Nova', 16),
(5370, 'Feira Nova', 25),
(1198, 'Feira Nova do Maranhão', 10),
(1848, 'Felício dos Santos', 11),
(3722, 'Felipe Guerra', 20),
(1849, 'Felisburgo', 11),
(1850, 'Felixlândia', 11),
(4015, 'Feliz', 23),
(130, 'Feliz Deserto', 2),
(1389, 'Feliz Natal', 13),
(2895, 'Fênix', 18),
(2896, 'Fernandes Pinheiro', 18),
(1851, 'Fernandes Tourinho', 11),
(3243, 'Fernando de Noronha', 16),
(1199, 'Fernando Falcão', 10),
(3723, 'Fernando Pedroza', 20),
(4885, 'Fernando Prestes', 26),
(4886, 'Fernandópolis', 26),
(4887, 'Fernão', 26),
(4888, 'Ferraz de Vasconcelos', 26),
(206, 'Ferreira Gomes', 4),
(3244, 'Ferreiros', 16),
(1852, 'Ferros', 11),
(1853, 'Fervedouro', 11),
(2897, 'Figueira', 18),
(1521, 'Figueirão', 12),
(5477, 'Figueirópolis', 27),
(1390, 'Figueirópolis d`Oeste', 13),
(412, 'Filadélfia', 5),
(5478, 'Filadélfia', 27),
(413, 'Firmino Alves', 5),
(969, 'Firminópolis', 9),
(131, 'Flexeiras', 2),
(2898, 'Flor da Serra do Sul', 18),
(4499, 'Flor do Sertão', 24),
(4889, 'Flora Rica', 26),
(2899, 'Floraí', 18),
(3724, 'Florânia', 20),
(4890, 'Floreal', 26),
(3245, 'Flores', 16),
(4016, 'Flores da Cunha', 23),
(970, 'Flores de Goiás', 9),
(3448, 'Flores do Piauí', 17),
(3246, 'Floresta', 16),
(2900, 'Floresta', 18),
(414, 'Floresta Azul', 5),
(2466, 'Floresta do Araguaia', 14),
(3449, 'Floresta do Piauí', 17),
(1854, 'Florestal', 11),
(2901, 'Florestópolis', 18),
(3450, 'Floriano', 17),
(4017, 'Floriano Peixoto', 23),
(4500, 'Florianópolis', 24),
(2902, 'Flórida', 18),
(4891, 'Flórida Paulista', 26),
(4892, 'Florínia', 26),
(242, 'Fonte Boa', 3),
(4018, 'Fontoura Xavier', 23),
(1855, 'Formiga', 11),
(4019, 'Formigueiro', 23),
(971, 'Formosa', 9),
(1200, 'Formosa da Serra Negra', 10),
(2903, 'Formosa do Oeste', 18),
(415, 'Formosa do Rio Preto', 5),
(4501, 'Formosa do Sul', 24),
(972, 'Formoso', 9),
(1856, 'Formoso', 11),
(5479, 'Formoso do Araguaia', 27),
(4020, 'Forquetinha', 23),
(755, 'Forquilha', 6),
(4502, 'Forquilhinha', 24),
(756, 'Fortaleza', 6),
(1857, 'Fortaleza de Minas', 11),
(5480, 'Fortaleza do Tabocão', 27),
(1201, 'Fortaleza dos Nogueiras', 10),
(4021, 'Fortaleza dos Valos', 23),
(757, 'Fortim', 6),
(1202, 'Fortuna', 10),
(1858, 'Fortuna de Minas', 11),
(2904, 'Foz do Iguaçu', 18),
(2905, 'Foz do Jordão', 18),
(4503, 'Fraiburgo', 24),
(4893, 'Franca', 26),
(3451, 'Francinópolis', 17),
(2906, 'Francisco Alves', 18),
(3452, 'Francisco Ayres', 17),
(1859, 'Francisco Badaró', 11),
(2907, 'Francisco Beltrão', 18),
(3725, 'Francisco Dantas', 20),
(1860, 'Francisco Dumont', 11),
(3453, 'Francisco Macedo', 17),
(4894, 'Francisco Morato', 26),
(1861, 'Francisco Sá', 11),
(3454, 'Francisco Santos', 17),
(1862, 'Franciscópolis', 11),
(4895, 'Franco da Rocha', 26),
(758, 'Frecheirinha', 6),
(4022, 'Frederico Westphalen', 23),
(1863, 'Frei Gaspar', 11),
(1864, 'Frei Inocêncio', 11),
(1865, 'Frei Lagonegro', 11),
(2640, 'Frei Martinho', 15),
(3247, 'Frei Miguelinho', 16),
(5371, 'Frei Paulo', 25),
(4504, 'Frei Rogério', 24),
(1866, 'Fronteira', 11),
(1867, 'Fronteira dos Vales', 11),
(3455, 'Fronteiras', 17),
(1868, 'Fruta de Leite', 11),
(1869, 'Frutal', 11),
(3726, 'Frutuoso Gomes', 20),
(26, 'Fundão', 8),
(1870, 'Funilândia', 11),
(4896, 'Gabriel Monteiro', 26),
(2641, 'Gado Bravo', 15),
(4897, 'Gália', 26),
(1871, 'Galiléia', 11),
(3727, 'Galinhos', 20),
(4505, 'Galvão', 24),
(3248, 'Gameleira', 16),
(973, 'Gameleira de Goiás', 9),
(1872, 'Gameleiras', 11),
(416, 'Gandu', 5),
(3249, 'Garanhuns', 16),
(5372, 'Gararu', 25),
(4898, 'Garça', 26),
(4023, 'Garibaldi', 23),
(4506, 'Garopaba', 24),
(2467, 'Garrafão do Norte', 14),
(4024, 'Garruchos', 23),
(4507, 'Garuva', 24),
(4508, 'Gaspar', 24),
(4899, 'Gastão Vidigal', 26),
(1391, 'Gaúcha do Norte', 13),
(4025, 'Gaurama', 23),
(417, 'Gavião', 5),
(4900, 'Gavião Peixoto', 26),
(3456, 'Geminiano', 17),
(4026, 'General Câmara', 23),
(1392, 'General Carneiro', 13),
(2908, 'General Carneiro', 18),
(5373, 'General Maynard', 25),
(4901, 'General Salgado', 26),
(759, 'General Sampaio', 6),
(4027, 'Gentil', 23),
(418, 'Gentio do Ouro', 5),
(4902, 'Getulina', 26),
(4028, 'Getúlio Vargas', 23),
(3457, 'Gilbués', 17),
(132, 'Girau do Ponciano', 2),
(4029, 'Giruá', 23),
(1873, 'Glaucilândia', 11),
(4903, 'Glicério', 26),
(419, 'Glória', 5),
(1393, 'Glória d`Oeste', 13),
(1522, 'Glória de Dourados', 12),
(3250, 'Glória do Goitá', 16),
(4030, 'Glorinha', 23),
(1203, 'Godofredo Viana', 10),
(2909, 'Godoy Moreira', 18),
(1874, 'Goiabeira', 11),
(1875, 'Goianá', 11),
(3251, 'Goiana', 16),
(974, 'Goianápolis', 9),
(975, 'Goiandira', 9),
(976, 'Goianésia', 9),
(2468, 'Goianésia do Pará', 14),
(977, 'Goiânia', 9),
(3728, 'Goianinha', 20),
(978, 'Goianira', 9),
(5481, 'Goianorte', 27),
(979, 'Goiás', 9),
(5482, 'Goiatins', 27),
(980, 'Goiatuba', 9),
(2910, 'Goioerê', 18),
(2911, 'Goioxim', 18),
(1876, 'Gonçalves', 11),
(1204, 'Gonçalves Dias', 10),
(420, 'Gongogi', 5),
(1877, 'Gonzaga', 11),
(1878, 'Gouveia', 11),
(981, 'Gouvelândia', 9),
(1205, 'Governador Archer', 10),
(4509, 'Governador Celso Ramos', 24),
(3729, 'Governador Dix-Sept Rosado', 20),
(1206, 'Governador Edison Lobão', 10),
(1207, 'Governador Eugênio Barros', 10),
(4365, 'Governador Jorge Teixeira', 21),
(27, 'Governador Lindenberg', 8),
(1208, 'Governador Luiz Rocha', 10),
(421, 'Governador Mangabeira', 5),
(1209, 'Governador Newton Bello', 10),
(1210, 'Governador Nunes Freire', 10),
(1879, 'Governador Valadares', 11),
(760, 'Graça', 6),
(1211, 'Graça Aranha', 10),
(5374, 'Gracho Cardoso', 25),
(1212, 'Grajaú', 10),
(4031, 'Gramado', 23),
(4032, 'Gramado dos Loureiros', 23),
(4033, 'Gramado Xavier', 23),
(2912, 'Grandes Rios', 18),
(3252, 'Granito', 16),
(761, 'Granja', 6),
(762, 'Granjeiro', 6),
(1880, 'Grão Mogol', 11),
(4510, 'Grão Pará', 24),
(3253, 'Gravatá', 16),
(4034, 'Gravataí', 23),
(4511, 'Gravatal', 24),
(763, 'Groaíras', 6),
(3730, 'Grossos', 20),
(1881, 'Grupiara', 11),
(4035, 'Guabiju', 23),
(4512, 'Guabiruba', 24),
(28, 'Guaçuí', 8),
(3458, 'Guadalupe', 17),
(4036, 'Guaíba', 23),
(4904, 'Guaiçara', 26),
(4905, 'Guaimbê', 26),
(2913, 'Guaíra', 18),
(4906, 'Guaíra', 26),
(2914, 'Guairaçá', 18),
(764, 'Guaiúba', 6),
(243, 'Guajará', 3),
(4366, 'Guajará-Mirim', 21),
(422, 'Guajeru', 5),
(3731, 'Guamaré', 20),
(2915, 'Guamiranga', 18),
(423, 'Guanambi', 5),
(1882, 'Guanhães', 11),
(1883, 'Guapé', 11),
(4907, 'Guapiaçu', 26),
(4908, 'Guapiara', 26),
(3617, 'Guapimirim', 19),
(2916, 'Guapirama', 18),
(982, 'Guapó', 9),
(4037, 'Guaporé', 23),
(2917, 'Guaporema', 18),
(4909, 'Guará', 26),
(2642, 'Guarabira', 15),
(4910, 'Guaraçaí', 26),
(2918, 'Guaraci', 18),
(4911, 'Guaraci', 26),
(1884, 'Guaraciaba', 11);
INSERT INTO `cidade` (`id`, `nome_cidade`, `id_uf`) VALUES
(4513, 'Guaraciaba', 24),
(765, 'Guaraciaba do Norte', 6),
(1885, 'Guaraciama', 11),
(5483, 'Guaraí', 27),
(983, 'Guaraíta', 9),
(766, 'Guaramiranga', 6),
(4514, 'Guaramirim', 24),
(1886, 'Guaranésia', 11),
(1887, 'Guarani', 11),
(4912, 'Guarani d`Oeste', 26),
(4038, 'Guarani das Missões', 23),
(984, 'Guarani de Goiás', 9),
(2919, 'Guaraniaçu', 18),
(4913, 'Guarantã', 26),
(1394, 'Guarantã do Norte', 13),
(29, 'Guarapari', 8),
(2920, 'Guarapuava', 18),
(2921, 'Guaraqueçaba', 18),
(1888, 'Guarará', 11),
(4914, 'Guararapes', 26),
(4915, 'Guararema', 26),
(424, 'Guaratinga', 5),
(4916, 'Guaratinguetá', 26),
(2922, 'Guaratuba', 18),
(1889, 'Guarda-Mor', 11),
(4917, 'Guareí', 26),
(4918, 'Guariba', 26),
(3459, 'Guaribas', 17),
(985, 'Guarinos', 9),
(4919, 'Guarujá', 26),
(4515, 'Guarujá do Sul', 24),
(4920, 'Guarulhos', 26),
(4516, 'Guatambú', 24),
(4921, 'Guatapará', 26),
(1890, 'Guaxupé', 11),
(1523, 'Guia Lopes da Laguna', 12),
(1891, 'Guidoval', 11),
(1213, 'Guimarães', 10),
(1892, 'Guimarânia', 11),
(1395, 'Guiratinga', 13),
(1893, 'Guiricema', 11),
(1894, 'Gurinhatã', 11),
(2643, 'Gurinhém', 15),
(2644, 'Gurjão', 15),
(2469, 'Gurupá', 14),
(5484, 'Gurupi', 27),
(4922, 'Guzolândia', 26),
(4039, 'Harmonia', 23),
(986, 'Heitoraí', 9),
(1895, 'Heliodora', 11),
(425, 'Heliópolis', 5),
(4923, 'Herculândia', 26),
(4040, 'Herval', 23),
(4517, 'Herval d`Oeste', 24),
(4041, 'Herveiras', 23),
(767, 'Hidrolândia', 6),
(987, 'Hidrolândia', 9),
(988, 'Hidrolina', 9),
(4924, 'Holambra', 26),
(2923, 'Honório Serpa', 18),
(768, 'Horizonte', 6),
(4042, 'Horizontina', 23),
(4925, 'Hortolândia', 26),
(3460, 'Hugo Napoleão', 17),
(4043, 'Hulha Negra', 23),
(244, 'Humaitá', 3),
(4044, 'Humaitá', 23),
(1214, 'Humberto de Campos', 10),
(4926, 'Iacanga', 26),
(989, 'Iaciara', 9),
(4927, 'Iacri', 26),
(426, 'Iaçu', 5),
(1896, 'Iapu', 11),
(4928, 'Iaras', 26),
(3254, 'Iati', 16),
(2924, 'Ibaiti', 18),
(4045, 'Ibarama', 23),
(769, 'Ibaretama', 6),
(4929, 'Ibaté', 26),
(133, 'Ibateguara', 2),
(30, 'Ibatiba', 8),
(2925, 'Ibema', 18),
(1897, 'Ibertioga', 11),
(1898, 'Ibiá', 11),
(4046, 'Ibiaçá', 23),
(1899, 'Ibiaí', 11),
(4518, 'Ibiam', 24),
(770, 'Ibiapina', 6),
(2645, 'Ibiara', 15),
(427, 'Ibiassucê', 5),
(428, 'Ibicaraí', 5),
(4519, 'Ibicaré', 24),
(429, 'Ibicoara', 5),
(430, 'Ibicuí', 5),
(771, 'Ibicuitinga', 6),
(3255, 'Ibimirim', 16),
(431, 'Ibipeba', 5),
(432, 'Ibipitanga', 5),
(2926, 'Ibiporã', 18),
(433, 'Ibiquera', 5),
(4930, 'Ibirá', 26),
(1900, 'Ibiracatu', 11),
(1901, 'Ibiraci', 11),
(31, 'Ibiraçu', 8),
(4047, 'Ibiraiaras', 23),
(3256, 'Ibirajuba', 16),
(4520, 'Ibirama', 24),
(434, 'Ibirapitanga', 5),
(435, 'Ibirapuã', 5),
(4048, 'Ibirapuitã', 23),
(4931, 'Ibirarema', 26),
(436, 'Ibirataia', 5),
(1902, 'Ibirité', 11),
(4049, 'Ibirubá', 23),
(437, 'Ibitiara', 5),
(4932, 'Ibitinga', 26),
(32, 'Ibitirama', 8),
(438, 'Ibititá', 5),
(1903, 'Ibitiúra de Minas', 11),
(1904, 'Ibituruna', 11),
(4933, 'Ibiúna', 26),
(439, 'Ibotirama', 5),
(772, 'Icapuí', 6),
(4521, 'Içara', 24),
(1905, 'Icaraí de Minas', 11),
(2927, 'Icaraíma', 18),
(1215, 'Icatu', 10),
(4934, 'Icém', 26),
(440, 'Ichu', 5),
(773, 'Icó', 6),
(33, 'Iconha', 8),
(3732, 'Ielmo Marinho', 20),
(4935, 'Iepê', 26),
(134, 'Igaci', 2),
(441, 'Igaporã', 5),
(4936, 'Igaraçu do Tietê', 26),
(2646, 'Igaracy', 15),
(4937, 'Igarapava', 26),
(1906, 'Igarapé', 11),
(1216, 'Igarapé do Meio', 10),
(1217, 'Igarapé Grande', 10),
(2470, 'Igarapé-Açu', 14),
(2471, 'Igarapé-Miri', 14),
(3257, 'Igarassu', 16),
(4938, 'Igaratá', 26),
(1907, 'Igaratinga', 11),
(442, 'Igrapiúna', 5),
(135, 'Igreja Nova', 2),
(4050, 'Igrejinha', 23),
(3618, 'Iguaba Grande', 19),
(443, 'Iguaí', 5),
(4939, 'Iguape', 26),
(3258, 'Iguaraci', 16),
(2928, 'Iguaraçu', 18),
(1908, 'Iguatama', 11),
(1524, 'Iguatemi', 12),
(774, 'Iguatu', 6),
(2929, 'Iguatu', 18),
(1909, 'Ijaci', 11),
(4051, 'Ijuí', 23),
(4940, 'Ilha Comprida', 26),
(5375, 'Ilha das Flores', 25),
(3259, 'Ilha de Itamaracá', 16),
(3461, 'Ilha Grande', 17),
(4941, 'Ilha Solteira', 26),
(4942, 'Ilhabela', 26),
(444, 'Ilhéus', 5),
(4522, 'Ilhota', 24),
(1910, 'Ilicínea', 11),
(4052, 'Ilópolis', 23),
(2647, 'Imaculada', 15),
(4523, 'Imaruí', 24),
(2930, 'Imbaú', 18),
(4053, 'Imbé', 23),
(1911, 'Imbé de Minas', 11),
(4524, 'Imbituba', 24),
(2931, 'Imbituva', 18),
(4525, 'Imbuia', 24),
(4054, 'Imigrante', 23),
(1218, 'Imperatriz', 10),
(2932, 'Inácio Martins', 18),
(990, 'Inaciolândia', 9),
(3260, 'Inajá', 16),
(2933, 'Inajá', 18),
(1912, 'Inconfidentes', 11),
(1913, 'Indaiabira', 11),
(4526, 'Indaial', 24),
(4943, 'Indaiatuba', 26),
(775, 'Independência', 6),
(4055, 'Independência', 23),
(4944, 'Indiana', 26),
(1914, 'Indianópolis', 11),
(2934, 'Indianópolis', 18),
(4945, 'Indiaporã', 26),
(991, 'Indiara', 9),
(5376, 'Indiaroba', 25),
(1396, 'Indiavaí', 13),
(2648, 'Ingá', 15),
(1915, 'Ingaí', 11),
(3261, 'Ingazeira', 16),
(4056, 'Inhacorá', 23),
(445, 'Inhambupe', 5),
(2472, 'Inhangapi', 14),
(136, 'Inhapi', 2),
(1916, 'Inhapim', 11),
(1917, 'Inhaúma', 11),
(3462, 'Inhuma', 17),
(992, 'Inhumas', 9),
(1918, 'Inimutaba', 11),
(1525, 'Inocência', 12),
(4946, 'Inúbia Paulista', 26),
(4527, 'Iomerê', 24),
(1919, 'Ipaba', 11),
(993, 'Ipameri', 9),
(1920, 'Ipanema', 11),
(3733, 'Ipanguaçu', 20),
(776, 'Ipaporanga', 6),
(1921, 'Ipatinga', 11),
(777, 'Ipaumirim', 6),
(4947, 'Ipaussu', 26),
(4057, 'Ipê', 23),
(446, 'Ipecaetá', 5),
(4948, 'Iperó', 26),
(4949, 'Ipeúna', 26),
(1922, 'Ipiaçu', 11),
(447, 'Ipiaú', 5),
(4950, 'Ipiguá', 26),
(448, 'Ipirá', 5),
(4528, 'Ipira', 24),
(2935, 'Ipiranga', 18),
(994, 'Ipiranga de Goiás', 9),
(1397, 'Ipiranga do Norte', 13),
(3463, 'Ipiranga do Piauí', 17),
(4058, 'Ipiranga do Sul', 23),
(245, 'Ipixuna', 3),
(2473, 'Ipixuna do Pará', 14),
(3262, 'Ipojuca', 16),
(995, 'Iporá', 9),
(2936, 'Iporã', 18),
(4529, 'Iporã do Oeste', 24),
(4951, 'Iporanga', 26),
(778, 'Ipu', 6),
(4952, 'Ipuã', 26),
(4530, 'Ipuaçu', 24),
(3263, 'Ipubi', 16),
(3734, 'Ipueira', 20),
(779, 'Ipueiras', 6),
(5485, 'Ipueiras', 27),
(1923, 'Ipuiúna', 11),
(4531, 'Ipumirim', 24),
(449, 'Ipupiara', 5),
(780, 'Iracema', 6),
(4405, 'Iracema', 22),
(2937, 'Iracema do Oeste', 18),
(4953, 'Iracemápolis', 26),
(4532, 'Iraceminha', 24),
(4059, 'Iraí', 23),
(1924, 'Iraí de Minas', 11),
(450, 'Irajuba', 5),
(451, 'Iramaia', 5),
(246, 'Iranduba', 3),
(4533, 'Irani', 24),
(4954, 'Irapuã', 26),
(4955, 'Irapuru', 26),
(452, 'Iraquara', 5),
(453, 'Irará', 5),
(2938, 'Irati', 18),
(4534, 'Irati', 24),
(781, 'Irauçuba', 6),
(454, 'Irecê', 5),
(2939, 'Iretama', 18),
(4535, 'Irineópolis', 24),
(2474, 'Irituia', 14),
(34, 'Irupi', 8),
(3464, 'Isaías Coelho', 17),
(996, 'Israelândia', 9),
(4536, 'Itá', 24),
(4060, 'Itaara', 23),
(2649, 'Itabaiana', 15),
(5377, 'Itabaiana', 25),
(5378, 'Itabaianinha', 25),
(455, 'Itabela', 5),
(4956, 'Itaberá', 26),
(456, 'Itaberaba', 5),
(997, 'Itaberaí', 9),
(5379, 'Itabi', 25),
(1925, 'Itabira', 11),
(1926, 'Itabirinha de Mantena', 11),
(1927, 'Itabirito', 11),
(3619, 'Itaboraí', 19),
(457, 'Itabuna', 5),
(5486, 'Itacajá', 27),
(1928, 'Itacambira', 11),
(1929, 'Itacarambi', 11),
(458, 'Itacaré', 5),
(247, 'Itacoatiara', 3),
(3264, 'Itacuruba', 16),
(4061, 'Itacurubi', 23),
(459, 'Itaeté', 5),
(460, 'Itagi', 5),
(461, 'Itagibá', 5),
(462, 'Itagimirim', 5),
(35, 'Itaguaçu', 8),
(463, 'Itaguaçu da Bahia', 5),
(3620, 'Itaguaí', 19),
(2940, 'Itaguajé', 18),
(1930, 'Itaguara', 11),
(998, 'Itaguari', 9),
(999, 'Itaguaru', 9),
(5487, 'Itaguatins', 27),
(4957, 'Itaí', 26),
(3265, 'Itaíba', 16),
(782, 'Itaiçaba', 6),
(3465, 'Itainópolis', 17),
(4537, 'Itaiópolis', 24),
(1219, 'Itaipava do Grajaú', 10),
(1931, 'Itaipé', 11),
(2941, 'Itaipulândia', 18),
(783, 'Itaitinga', 6),
(2475, 'Itaituba', 14),
(1000, 'Itajá', 9),
(3735, 'Itajá', 20),
(4538, 'Itajaí', 24),
(4958, 'Itajobi', 26),
(4959, 'Itaju', 26),
(464, 'Itaju do Colônia', 5),
(1932, 'Itajubá', 11),
(465, 'Itajuípe', 5),
(3621, 'Italva', 19),
(466, 'Itamaraju', 5),
(1933, 'Itamarandiba', 11),
(248, 'Itamarati', 3),
(1934, 'Itamarati de Minas', 11),
(467, 'Itamari', 5),
(1935, 'Itambacuri', 11),
(2942, 'Itambaracá', 18),
(468, 'Itambé', 5),
(3266, 'Itambé', 16),
(2943, 'Itambé', 18),
(1936, 'Itambé do Mato Dentro', 11),
(1937, 'Itamogi', 11),
(1938, 'Itamonte', 11),
(469, 'Itanagra', 5),
(4960, 'Itanhaém', 26),
(1939, 'Itanhandu', 11),
(1398, 'Itanhangá', 13),
(470, 'Itanhém', 5),
(1940, 'Itanhomi', 11),
(1941, 'Itaobim', 11),
(4961, 'Itaóca', 26),
(3622, 'Itaocara', 19),
(1001, 'Itapaci', 9),
(784, 'Itapagé', 6),
(1942, 'Itapagipe', 11),
(471, 'Itaparica', 5),
(472, 'Itapé', 5),
(473, 'Itapebi', 5),
(1943, 'Itapecerica', 11),
(4962, 'Itapecerica da Serra', 26),
(1220, 'Itapecuru Mirim', 10),
(2944, 'Itapejara d`Oeste', 18),
(4539, 'Itapema', 24),
(36, 'Itapemirim', 8),
(2945, 'Itaperuçu', 18),
(3623, 'Itaperuna', 19),
(3267, 'Itapetim', 16),
(474, 'Itapetinga', 5),
(4963, 'Itapetininga', 26),
(1944, 'Itapeva', 11),
(4964, 'Itapeva', 26),
(4965, 'Itapevi', 26),
(475, 'Itapicuru', 5),
(785, 'Itapipoca', 6),
(4966, 'Itapira', 26),
(249, 'Itapiranga', 3),
(4540, 'Itapiranga', 24),
(1002, 'Itapirapuã', 9),
(4967, 'Itapirapuã Paulista', 26),
(5488, 'Itapiratins', 27),
(3268, 'Itapissuma', 16),
(476, 'Itapitanga', 5),
(786, 'Itapiúna', 6),
(4541, 'Itapoá', 24),
(4968, 'Itápolis', 26),
(1526, 'Itaporã', 12),
(5489, 'Itaporã do Tocantins', 27),
(2650, 'Itaporanga', 15),
(4969, 'Itaporanga', 26),
(5380, 'Itaporanga d`Ajuda', 25),
(2651, 'Itapororoca', 15),
(4367, 'Itapuã do Oeste', 21),
(4062, 'Itapuca', 23),
(4970, 'Itapuí', 26),
(4971, 'Itapura', 26),
(1003, 'Itapuranga', 9),
(4972, 'Itaquaquecetuba', 26),
(477, 'Itaquara', 5),
(4063, 'Itaqui', 23),
(1527, 'Itaquiraí', 12),
(3269, 'Itaquitinga', 16),
(37, 'Itarana', 8),
(478, 'Itarantim', 5),
(4973, 'Itararé', 26),
(787, 'Itarema', 6),
(4974, 'Itariri', 26),
(1004, 'Itarumã', 9),
(4064, 'Itati', 23),
(3624, 'Itatiaia', 19),
(1945, 'Itatiaiuçu', 11),
(4975, 'Itatiba', 26),
(4065, 'Itatiba do Sul', 23),
(479, 'Itatim', 5),
(4976, 'Itatinga', 26),
(788, 'Itatira', 6),
(2652, 'Itatuba', 15),
(3736, 'Itaú', 20),
(1946, 'Itaú de Minas', 11),
(1399, 'Itaúba', 13),
(207, 'Itaubal', 4),
(1005, 'Itauçu', 9),
(3466, 'Itaueira', 17),
(1947, 'Itaúna', 11),
(2946, 'Itaúna do Sul', 18),
(1948, 'Itaverava', 11),
(1949, 'Itinga', 11),
(1221, 'Itinga do Maranhão', 10),
(1400, 'Itiquira', 13),
(4977, 'Itirapina', 26),
(4978, 'Itirapuã', 26),
(480, 'Itiruçu', 5),
(481, 'Itiúba', 5),
(4979, 'Itobi', 26),
(482, 'Itororó', 5),
(4980, 'Itu', 26),
(483, 'Ituaçu', 5),
(484, 'Ituberá', 5),
(1950, 'Itueta', 11),
(1951, 'Ituiutaba', 11),
(1006, 'Itumbiara', 9),
(1952, 'Itumirim', 11),
(4981, 'Itupeva', 26),
(2476, 'Itupiranga', 14),
(4542, 'Ituporanga', 24),
(1953, 'Iturama', 11),
(1954, 'Itutinga', 11),
(4982, 'Ituverava', 26),
(485, 'Iuiú', 5),
(38, 'Iúna', 8),
(2947, 'Ivaí', 18),
(2948, 'Ivaiporã', 18),
(2949, 'Ivaté', 18),
(2950, 'Ivatuba', 18),
(1528, 'Ivinhema', 12),
(1007, 'Ivolândia', 9),
(4066, 'Ivorá', 23),
(4067, 'Ivoti', 23),
(3270, 'Jaboatão dos Guararapes', 16),
(4543, 'Jaborá', 24),
(486, 'Jaborandi', 5),
(4983, 'Jaborandi', 26),
(2951, 'Jaboti', 18),
(4068, 'Jaboticaba', 23),
(4984, 'Jaboticabal', 26),
(1955, 'Jaboticatubas', 11),
(3737, 'Jaçanã', 20),
(487, 'Jacaraci', 5),
(2653, 'Jacaraú', 15),
(137, 'Jacaré dos Homens', 2),
(2477, 'Jacareacanga', 14),
(4985, 'Jacareí', 26),
(2952, 'Jacarezinho', 18),
(4986, 'Jaci', 26),
(1401, 'Jaciara', 13),
(1956, 'Jacinto', 11),
(4544, 'Jacinto Machado', 24),
(488, 'Jacobina', 5),
(3467, 'Jacobina do Piauí', 17),
(1957, 'Jacuí', 11),
(138, 'Jacuípe', 2),
(4069, 'Jacuizinho', 23),
(2478, 'Jacundá', 14),
(4987, 'Jacupiranga', 26),
(1958, 'Jacutinga', 11),
(4070, 'Jacutinga', 23),
(2953, 'Jaguapitã', 18),
(489, 'Jaguaquara', 5),
(1959, 'Jaguaraçu', 11),
(4071, 'Jaguarão', 23),
(490, 'Jaguarari', 5),
(39, 'Jaguaré', 8),
(789, 'Jaguaretama', 6),
(4072, 'Jaguari', 23),
(2954, 'Jaguariaíva', 18),
(790, 'Jaguaribara', 6),
(791, 'Jaguaribe', 6),
(491, 'Jaguaripe', 5),
(4988, 'Jaguariúna', 26),
(792, 'Jaguaruana', 6),
(4545, 'Jaguaruna', 24),
(1960, 'Jaíba', 11),
(3468, 'Jaicós', 17),
(4989, 'Jales', 26),
(4990, 'Jambeiro', 26),
(1961, 'Jampruca', 11),
(1962, 'Janaúba', 11),
(1008, 'Jandaia', 9),
(2955, 'Jandaia do Sul', 18),
(492, 'Jandaíra', 5),
(3738, 'Jandaíra', 20),
(4991, 'Jandira', 26),
(3739, 'Janduís', 20),
(1402, 'Jangada', 13),
(2956, 'Janiópolis', 18),
(1963, 'Januária', 11),
(3740, 'Januário Cicco', 20),
(1964, 'Japaraíba', 11),
(139, 'Japaratinga', 2),
(5381, 'Japaratuba', 25),
(3625, 'Japeri', 19),
(3741, 'Japi', 20),
(2957, 'Japira', 18),
(5382, 'Japoatã', 25),
(1965, 'Japonvar', 11),
(1529, 'Japorã', 12),
(250, 'Japurá', 3),
(2958, 'Japurá', 18),
(3271, 'Jaqueira', 16),
(4073, 'Jaquirana', 23),
(1009, 'Jaraguá', 9),
(4546, 'Jaraguá do Sul', 24),
(1530, 'Jaraguari', 12),
(140, 'Jaramataia', 2),
(793, 'Jardim', 6),
(1531, 'Jardim', 12),
(2959, 'Jardim Alegre', 18),
(3742, 'Jardim de Angicos', 20),
(3743, 'Jardim de Piranhas', 20),
(3469, 'Jardim do Mulato', 17),
(3744, 'Jardim do Seridó', 20),
(2960, 'Jardim Olinda', 18),
(4547, 'Jardinópolis', 24),
(4992, 'Jardinópolis', 26),
(4074, 'Jari', 23),
(4993, 'Jarinu', 26),
(4368, 'Jaru', 21),
(1010, 'Jataí', 9),
(2961, 'Jataizinho', 18),
(3272, 'Jataúba', 16),
(1532, 'Jateí', 12),
(794, 'Jati', 6),
(1222, 'Jatobá', 10),
(3273, 'Jatobá', 16),
(3470, 'Jatobá do Piauí', 17),
(4994, 'Jaú', 26),
(5490, 'Jaú do Tocantins', 27),
(1011, 'Jaupaci', 9),
(1403, 'Jauru', 13),
(1966, 'Jeceaba', 11),
(1967, 'Jenipapo de Minas', 11),
(1223, 'Jenipapo dos Vieiras', 10),
(1968, 'Jequeri', 11),
(141, 'Jequiá da Praia', 2),
(493, 'Jequié', 5),
(1969, 'Jequitaí', 11),
(1970, 'Jequitibá', 11),
(1971, 'Jequitinhonha', 11),
(494, 'Jeremoabo', 5),
(2654, 'Jericó', 15),
(4995, 'Jeriquara', 26),
(40, 'Jerônimo Monteiro', 8),
(3471, 'Jerumenha', 17),
(1972, 'Jesuânia', 11),
(2962, 'Jesuítas', 18),
(1012, 'Jesúpolis', 9),
(4369, 'Ji-Paraná', 21),
(795, 'Jijoca de Jericoacoara', 6),
(495, 'Jiquiriçá', 5),
(496, 'Jitaúna', 5),
(4548, 'Joaçaba', 24),
(1973, 'Joaíma', 11),
(1974, 'Joanésia', 11),
(4996, 'Joanópolis', 26),
(3274, 'João Alfredo', 16),
(3745, 'João Câmara', 20),
(3472, 'João Costa', 17),
(3746, 'João Dias', 20),
(497, 'João Dourado', 5),
(1224, 'João Lisboa', 10),
(1975, 'João Monlevade', 11),
(41, 'João Neiva', 8),
(2655, 'João Pessoa', 15),
(1976, 'João Pinheiro', 11),
(4997, 'João Ramalho', 26),
(1977, 'Joaquim Felício', 11),
(142, 'Joaquim Gomes', 2),
(3275, 'Joaquim Nabuco', 16),
(3473, 'Joaquim Pires', 17),
(2963, 'Joaquim Távora', 18),
(3474, 'Joca Marques', 17),
(4075, 'Jóia', 23),
(4549, 'Joinville', 24),
(1978, 'Jordânia', 11),
(87, 'Jordão', 1),
(4550, 'José Boiteux', 24),
(4998, 'José Bonifácio', 26),
(3747, 'José da Penha', 20),
(3475, 'José de Freitas', 17),
(1979, 'José Gonçalves de Minas', 11),
(1980, 'José Raydan', 11),
(1225, 'Joselândia', 10),
(1981, 'Josenópolis', 11),
(1013, 'Joviânia', 9),
(1404, 'Juara', 13),
(2656, 'Juarez Távora', 15),
(5491, 'Juarina', 27),
(1982, 'Juatuba', 11),
(2657, 'Juazeirinho', 15),
(498, 'Juazeiro', 5),
(796, 'Juazeiro do Norte', 6),
(3476, 'Juazeiro do Piauí', 17),
(797, 'Jucás', 6),
(3276, 'Jucati', 16),
(499, 'Jucuruçu', 5),
(3748, 'Jucurutu', 20),
(1405, 'Juína', 13),
(1983, 'Juiz de Fora', 11),
(3477, 'Júlio Borges', 17),
(4076, 'Júlio de Castilhos', 23),
(4999, 'Júlio Mesquita', 26),
(5000, 'Jumirim', 26),
(1226, 'Junco do Maranhão', 10),
(2658, 'Junco do Seridó', 15),
(143, 'Jundiá', 2),
(3749, 'Jundiá', 20),
(5001, 'Jundiaí', 26),
(2964, 'Jundiaí do Sul', 18),
(144, 'Junqueiro', 2),
(5002, 'Junqueirópolis', 26),
(3277, 'Jupi', 16),
(4551, 'Jupiá', 24),
(5003, 'Juquiá', 26),
(5004, 'Juquitiba', 26),
(1984, 'Juramento', 11),
(2965, 'Juranda', 18),
(3278, 'Jurema', 16),
(3478, 'Jurema', 17),
(2659, 'Juripiranga', 15),
(2660, 'Juru', 15),
(251, 'Juruá', 3),
(1985, 'Juruaia', 11),
(1406, 'Juruena', 13),
(2479, 'Juruti', 14),
(1407, 'Juscimeira', 13),
(500, 'Jussara', 5),
(1014, 'Jussara', 9),
(2966, 'Jussara', 18),
(501, 'Jussari', 5),
(502, 'Jussiape', 5),
(252, 'Jutaí', 3),
(1533, 'Juti', 12),
(1986, 'Juvenília', 11),
(2967, 'Kaloré', 18),
(253, 'Lábrea', 3),
(4552, 'Lacerdópolis', 24),
(1987, 'Ladainha', 11),
(1534, 'Ladário', 12),
(503, 'Lafaiete Coutinho', 5),
(1988, 'Lagamar', 11),
(5383, 'Lagarto', 25),
(4553, 'Lages', 24),
(1227, 'Lago da Pedra', 10),
(1228, 'Lago do Junco', 10),
(1229, 'Lago dos Rodrigues', 10),
(1230, 'Lago Verde', 10),
(2661, 'Lagoa', 15),
(3479, 'Lagoa Alegre', 17),
(4077, 'Lagoa Bonita do Sul', 23),
(3750, 'Lagoa d`Anta', 20),
(145, 'Lagoa da Canoa', 2),
(5492, 'Lagoa da Confusão', 27),
(1989, 'Lagoa da Prata', 11),
(2662, 'Lagoa de Dentro', 15),
(3751, 'Lagoa de Pedras', 20),
(3480, 'Lagoa de São Francisco', 17),
(3752, 'Lagoa de Velhos', 20),
(3481, 'Lagoa do Barro do Piauí', 17),
(3279, 'Lagoa do Carro', 16),
(3280, 'Lagoa do Itaenga', 16),
(1231, 'Lagoa do Mato', 10),
(3281, 'Lagoa do Ouro', 16),
(3482, 'Lagoa do Piauí', 17),
(3483, 'Lagoa do Sítio', 17),
(5493, 'Lagoa do Tocantins', 27),
(3282, 'Lagoa dos Gatos', 16),
(1990, 'Lagoa dos Patos', 11),
(4078, 'Lagoa dos Três Cantos', 23),
(1991, 'Lagoa Dourada', 11),
(1992, 'Lagoa Formosa', 11),
(1993, 'Lagoa Grande', 11),
(3283, 'Lagoa Grande', 16),
(1232, 'Lagoa Grande do Maranhão', 10),
(3753, 'Lagoa Nova', 20),
(504, 'Lagoa Real', 5),
(3754, 'Lagoa Salgada', 20),
(1015, 'Lagoa Santa', 9),
(1994, 'Lagoa Santa', 11),
(2663, 'Lagoa Seca', 15),
(4079, 'Lagoa Vermelha', 23),
(4080, 'Lagoão', 23),
(5005, 'Lagoinha', 26),
(3484, 'Lagoinha do Piauí', 17),
(4554, 'Laguna', 24),
(1535, 'Laguna Carapã', 12),
(505, 'Laje', 5),
(3626, 'Laje do Muriaé', 19),
(4081, 'Lajeado', 23),
(5494, 'Lajeado', 27),
(4082, 'Lajeado do Bugre', 23),
(4555, 'Lajeado Grande', 24),
(1233, 'Lajeado Novo', 10),
(506, 'Lajedão', 5),
(507, 'Lajedinho', 5),
(3284, 'Lajedo', 16),
(508, 'Lajedo do Tabocal', 5),
(3755, 'Lajes', 20),
(3756, 'Lajes Pintadas', 20),
(1995, 'Lajinha', 11),
(509, 'Lamarão', 5),
(1996, 'Lambari', 11),
(1408, 'Lambari d`Oeste', 13),
(1997, 'Lamim', 11),
(3485, 'Landri Sales', 17),
(2968, 'Lapa', 18),
(510, 'Lapão', 5),
(42, 'Laranja da Terra', 8),
(1998, 'Laranjal', 11),
(2969, 'Laranjal', 18),
(208, 'Laranjal do Jari', 4),
(5006, 'Laranjal Paulista', 26),
(5384, 'Laranjeiras', 25),
(2970, 'Laranjeiras do Sul', 18),
(1999, 'Lassance', 11),
(2664, 'Lastro', 15),
(4556, 'Laurentino', 24),
(511, 'Lauro de Freitas', 5),
(4557, 'Lauro Muller', 24),
(5495, 'Lavandeira', 27),
(5007, 'Lavínia', 26),
(2000, 'Lavras', 11),
(798, 'Lavras da Mangabeira', 6),
(4083, 'Lavras do Sul', 23),
(5008, 'Lavrinhas', 26),
(2001, 'Leandro Ferreira', 11),
(4558, 'Lebon Régis', 24),
(5009, 'Leme', 26),
(2002, 'Leme do Prado', 11),
(512, 'Lençóis', 5),
(5010, 'Lençóis Paulista', 26),
(4559, 'Leoberto Leal', 24),
(2003, 'Leopoldina', 11),
(1016, 'Leopoldo de Bulhões', 9),
(2971, 'Leópolis', 18),
(4084, 'Liberato Salzano', 23),
(2004, 'Liberdade', 11),
(513, 'Licínio de Almeida', 5),
(2972, 'Lidianópolis', 18),
(1234, 'Lima Campos', 10),
(2005, 'Lima Duarte', 11),
(5011, 'Limeira', 26),
(2006, 'Limeira do Oeste', 11),
(3285, 'Limoeiro', 16),
(146, 'Limoeiro de Anadia', 2),
(2480, 'Limoeiro do Ajuru', 14),
(799, 'Limoeiro do Norte', 6),
(2973, 'Lindoeste', 18),
(5012, 'Lindóia', 26),
(4560, 'Lindóia do Sul', 24),
(4085, 'Lindolfo Collor', 23),
(4086, 'Linha Nova', 23),
(43, 'Linhares', 8),
(5013, 'Lins', 26),
(2665, 'Livramento', 15),
(514, 'Livramento de Nossa Senhora', 5),
(5496, 'Lizarda', 27),
(2974, 'Loanda', 18),
(2975, 'Lobato', 18),
(2666, 'Logradouro', 15),
(2976, 'Londrina', 18),
(2007, 'Lontra', 11),
(4561, 'Lontras', 24),
(5014, 'Lorena', 26),
(1235, 'Loreto', 10),
(5015, 'Lourdes', 26),
(5016, 'Louveira', 26),
(1409, 'Lucas do Rio Verde', 13),
(5017, 'Lucélia', 26),
(2667, 'Lucena', 15),
(5018, 'Lucianópolis', 26),
(1410, 'Luciára', 13),
(3757, 'Lucrécia', 20),
(5019, 'Luís Antônio', 26),
(3486, 'Luís Correia', 17),
(1236, 'Luís Domingues', 10),
(515, 'Luís Eduardo Magalhães', 5),
(3758, 'Luís Gomes', 20),
(2008, 'Luisburgo', 11),
(2009, 'Luislândia', 11),
(4562, 'Luiz Alves', 24),
(2977, 'Luiziana', 18),
(5020, 'Luiziânia', 26),
(2010, 'Luminárias', 11),
(2978, 'Lunardelli', 18),
(5021, 'Lupércio', 26),
(2979, 'Lupionópolis', 18),
(5022, 'Lutécia', 26),
(2011, 'Luz', 11),
(4563, 'Luzerna', 24),
(1017, 'Luziânia', 9),
(3487, 'Luzilândia', 17),
(5497, 'Luzinópolis', 27),
(3627, 'Macaé', 19),
(3759, 'Macaíba', 20),
(516, 'Macajuba', 5),
(4087, 'Maçambara', 23),
(5385, 'Macambira', 25),
(209, 'Macapá', 4),
(3286, 'Macaparana', 16),
(517, 'Macarani', 5),
(5023, 'Macatuba', 26),
(3760, 'Macau', 20),
(5024, 'Macaubal', 26),
(518, 'Macaúbas', 5),
(5025, 'Macedônia', 26),
(147, 'Maceió', 2),
(2012, 'Machacalis', 11),
(4088, 'Machadinho', 23),
(4370, 'Machadinho d`Oeste', 21),
(2013, 'Machado', 11),
(3287, 'Machados', 16),
(4564, 'Macieira', 24),
(3628, 'Macuco', 19),
(519, 'Macururé', 5),
(800, 'Madalena', 6),
(3488, 'Madeiro', 17),
(520, 'Madre de Deus', 5),
(2014, 'Madre de Deus de Minas', 11),
(2668, 'Mãe d`Água', 15),
(2481, 'Mãe do Rio', 14),
(521, 'Maetinga', 5),
(4565, 'Mafra', 24),
(2482, 'Magalhães Barata', 14),
(1237, 'Magalhães de Almeida', 10),
(5026, 'Magda', 26),
(3629, 'Magé', 19),
(522, 'Maiquinique', 5),
(523, 'Mairi', 5),
(5027, 'Mairinque', 26),
(5028, 'Mairiporã', 26),
(1018, 'Mairipotaba', 9),
(4566, 'Major Gercino', 24),
(148, 'Major Isidoro', 2),
(3761, 'Major Sales', 20),
(4567, 'Major Vieira', 24),
(2015, 'Malacacheta', 11),
(524, 'Malhada', 5),
(525, 'Malhada de Pedras', 5),
(5386, 'Malhada dos Bois', 25),
(5387, 'Malhador', 25),
(2980, 'Mallet', 18),
(2669, 'Malta', 15),
(2670, 'Mamanguape', 15),
(1019, 'Mambaí', 9),
(2981, 'Mamborê', 18),
(2016, 'Mamonas', 11),
(4089, 'Mampituba', 23),
(254, 'Manacapuru', 3),
(2671, 'Manaíra', 15),
(255, 'Manaquiri', 3),
(3288, 'Manari', 16),
(256, 'Manaus', 3),
(88, 'Mâncio Lima', 1),
(2982, 'Mandaguaçu', 18),
(2983, 'Mandaguari', 18),
(2984, 'Mandirituba', 18),
(5029, 'Manduri', 26),
(2985, 'Manfrinópolis', 18),
(2017, 'Manga', 11),
(3630, 'Mangaratiba', 19),
(2986, 'Mangueirinha', 18),
(2018, 'Manhuaçu', 11),
(2019, 'Manhumirim', 11),
(257, 'Manicoré', 3),
(3489, 'Manoel Emídio', 17),
(2987, 'Manoel Ribas', 18),
(89, 'Manoel Urbano', 1),
(4090, 'Manoel Viana', 23),
(526, 'Manoel Vitorino', 5),
(527, 'Mansidão', 5),
(2020, 'Mantena', 11),
(44, 'Mantenópolis', 8),
(4091, 'Maquiné', 23),
(2021, 'Mar de Espanha', 11),
(149, 'Mar Vermelho', 2),
(1020, 'Mara Rosa', 9),
(258, 'Maraã', 3),
(2483, 'Marabá', 14),
(5030, 'Marabá Paulista', 26),
(1238, 'Maracaçumé', 10),
(5031, 'Maracaí', 26),
(4568, 'Maracajá', 24),
(1536, 'Maracaju', 12),
(2484, 'Maracanã', 14),
(801, 'Maracanaú', 6),
(528, 'Maracás', 5),
(150, 'Maragogi', 2),
(529, 'Maragogipe', 5),
(3289, 'Maraial', 16),
(1239, 'Marajá do Sena', 10),
(802, 'Maranguape', 6),
(1240, 'Maranhãozinho', 10),
(2485, 'Marapanim', 14),
(5032, 'Marapoama', 26),
(4092, 'Maratá', 23),
(45, 'Marataízes', 8),
(530, 'Maraú', 5),
(4093, 'Marau', 23),
(151, 'Maravilha', 2),
(4569, 'Maravilha', 24),
(2022, 'Maravilhas', 11),
(2672, 'Marcação', 15),
(1411, 'Marcelândia', 13),
(4094, 'Marcelino Ramos', 23),
(3762, 'Marcelino Vieira', 20),
(531, 'Marcionílio Souza', 5),
(803, 'Marco', 6),
(3490, 'Marcolândia', 17),
(3491, 'Marcos Parente', 17),
(2988, 'Marechal Cândido Rondon', 18),
(152, 'Marechal Deodoro', 2),
(46, 'Marechal Floriano', 8),
(90, 'Marechal Thaumaturgo', 1),
(4570, 'Marema', 24),
(2673, 'Mari', 15),
(2023, 'Maria da Fé', 11),
(2989, 'Maria Helena', 18),
(2990, 'Marialva', 18),
(2024, 'Mariana', 11),
(4095, 'Mariana Pimentel', 23),
(4096, 'Mariano Moro', 23),
(5498, 'Marianópolis do Tocantins', 27),
(5033, 'Mariápolis', 26),
(153, 'Maribondo', 2),
(3631, 'Maricá', 19),
(2025, 'Marilac', 11),
(47, 'Marilândia', 8),
(2991, 'Marilândia do Sul', 18),
(2992, 'Marilena', 18),
(5034, 'Marília', 26),
(2993, 'Mariluz', 18),
(2994, 'Maringá', 18),
(5035, 'Marinópolis', 26),
(2026, 'Mário Campos', 11),
(2995, 'Mariópolis', 18),
(2996, 'Maripá', 18),
(2027, 'Maripá de Minas', 11),
(2486, 'Marituba', 14),
(2674, 'Marizópolis', 15),
(2028, 'Marliéria', 11),
(2997, 'Marmeleiro', 18),
(2029, 'Marmelópolis', 11),
(4097, 'Marques de Souza', 23),
(2998, 'Marquinho', 18),
(2030, 'Martinho Campos', 11),
(804, 'Martinópole', 6),
(5036, 'Martinópolis', 26),
(3763, 'Martins', 20),
(2031, 'Martins Soares', 11),
(5388, 'Maruim', 25),
(2999, 'Marumbi', 18),
(1021, 'Marzagão', 9),
(532, 'Mascote', 5),
(805, 'Massapê', 6),
(3492, 'Massapê do Piauí', 17),
(2675, 'Massaranduba', 15),
(4571, 'Massaranduba', 24),
(4098, 'Mata', 23),
(533, 'Mata de São João', 5),
(154, 'Mata Grande', 2),
(1241, 'Mata Roma', 10),
(2032, 'Mata Verde', 11),
(5037, 'Matão', 26),
(2676, 'Mataraca', 15),
(5499, 'Mateiros', 27),
(3000, 'Matelândia', 18),
(2033, 'Materlândia', 11),
(2034, 'Mateus Leme', 11),
(2035, 'Mathias Lobato', 11),
(2036, 'Matias Barbosa', 11),
(2037, 'Matias Cardoso', 11),
(3493, 'Matias Olímpio', 17),
(534, 'Matina', 5),
(1242, 'Matinha', 10),
(2677, 'Matinhas', 15),
(3001, 'Matinhos', 18),
(2038, 'Matipó', 11),
(4099, 'Mato Castelhano', 23),
(2678, 'Mato Grosso', 15),
(4100, 'Mato Leitão', 23),
(4101, 'Mato Queimado', 23),
(3002, 'Mato Rico', 18),
(2039, 'Mato Verde', 11),
(1243, 'Matões', 10),
(1244, 'Matões do Norte', 10),
(4572, 'Matos Costa', 24),
(2040, 'Matozinhos', 11),
(1022, 'Matrinchã', 9),
(155, 'Matriz de Camaragibe', 2),
(1412, 'Matupá', 13),
(2679, 'Maturéia', 15),
(2041, 'Matutina', 11),
(5038, 'Mauá', 26),
(3003, 'Mauá da Serra', 18),
(259, 'Maués', 3),
(1023, 'Maurilândia', 9),
(5500, 'Maurilândia do Tocantins', 27),
(806, 'Mauriti', 6),
(3764, 'Maxaranguape', 20),
(4102, 'Maximiliano de Almeida', 23),
(210, 'Mazagão', 4),
(2042, 'Medeiros', 11),
(535, 'Medeiros Neto', 5),
(3004, 'Medianeira', 18),
(2487, 'Medicilândia', 14),
(2043, 'Medina', 11),
(4573, 'Meleiro', 24),
(2488, 'Melgaço', 14),
(3632, 'Mendes', 19),
(2044, 'Mendes Pimentel', 11),
(5039, 'Mendonça', 26),
(3005, 'Mercedes', 18),
(2045, 'Mercês', 11),
(5040, 'Meridiano', 26),
(807, 'Meruoca', 6),
(5041, 'Mesópolis', 26),
(2046, 'Mesquita', 11),
(3633, 'Mesquita', 19),
(156, 'Messias', 2),
(3765, 'Messias Targino', 20),
(3494, 'Miguel Alves', 17),
(536, 'Miguel Calmon', 5),
(3495, 'Miguel Leão', 17),
(3634, 'Miguel Pereira', 19),
(5042, 'Miguelópolis', 26),
(537, 'Milagres', 5),
(808, 'Milagres', 6),
(1245, 'Milagres do Maranhão', 10),
(809, 'Milhã', 6),
(3496, 'Milton Brandão', 17),
(1024, 'Mimoso de Goiás', 9),
(48, 'Mimoso do Sul', 8),
(1025, 'Minaçu', 9),
(157, 'Minador do Negrão', 2),
(4103, 'Minas do Leão', 23),
(2047, 'Minas Novas', 11),
(2048, 'Minduri', 11),
(1026, 'Mineiros', 9),
(5043, 'Mineiros do Tietê', 26),
(4371, 'Ministro Andreazza', 21),
(5044, 'Mira Estrela', 26),
(2049, 'Mirabela', 11),
(5045, 'Miracatu', 26),
(3635, 'Miracema', 19),
(5501, 'Miracema do Tocantins', 27),
(1246, 'Mirador', 10),
(3006, 'Mirador', 18),
(2050, 'Miradouro', 11),
(4104, 'Miraguaí', 23),
(2051, 'Miraí', 11),
(810, 'Miraíma', 6),
(1537, 'Miranda', 12),
(1247, 'Miranda do Norte', 10),
(3290, 'Mirandiba', 16),
(5046, 'Mirandópolis', 26),
(538, 'Mirangaba', 5),
(5502, 'Miranorte', 27),
(539, 'Mirante', 5),
(4372, 'Mirante da Serra', 21),
(5047, 'Mirante do Paranapanema', 26),
(3007, 'Miraselva', 18),
(5048, 'Mirassol', 26),
(1413, 'Mirassol d`Oeste', 13),
(5049, 'Mirassolândia', 26),
(2052, 'Miravânia', 11),
(4574, 'Mirim Doce', 24),
(1248, 'Mirinzal', 10),
(3008, 'Missal', 18),
(811, 'Missão Velha', 6),
(2489, 'Mocajuba', 14),
(5050, 'Mococa', 26),
(4575, 'Modelo', 24),
(2053, 'Moeda', 11),
(2054, 'Moema', 11),
(2680, 'Mogeiro', 15),
(5051, 'Mogi das Cruzes', 26),
(5052, 'Mogi Guaçu', 26),
(1027, 'Moiporá', 9),
(5389, 'Moita Bonita', 25),
(5053, 'Moji Mirim', 26),
(2490, 'Moju', 14),
(812, 'Mombaça', 6),
(5054, 'Mombuca', 26),
(1249, 'Monção', 10),
(5055, 'Monções', 26),
(4576, 'Mondaí', 24),
(5056, 'Mongaguá', 26),
(2055, 'Monjolos', 11),
(3497, 'Monsenhor Gil', 17),
(3498, 'Monsenhor Hipólito', 17),
(2056, 'Monsenhor Paulo', 11),
(813, 'Monsenhor Tabosa', 6),
(2681, 'Montadas', 15),
(2057, 'Montalvânia', 11),
(49, 'Montanha', 8),
(3766, 'Montanhas', 20),
(4105, 'Montauri', 23),
(2491, 'Monte Alegre', 14),
(3767, 'Monte Alegre', 20),
(1028, 'Monte Alegre de Goiás', 9),
(2058, 'Monte Alegre de Minas', 11),
(5390, 'Monte Alegre de Sergipe', 25),
(3499, 'Monte Alegre do Piauí', 17),
(5057, 'Monte Alegre do Sul', 26),
(4106, 'Monte Alegre dos Campos', 23),
(5058, 'Monte Alto', 26),
(5059, 'Monte Aprazível', 26),
(2059, 'Monte Azul', 11),
(5060, 'Monte Azul Paulista', 26),
(2060, 'Monte Belo', 11),
(4107, 'Monte Belo do Sul', 23),
(4577, 'Monte Carlo', 24),
(2061, 'Monte Carmelo', 11),
(4578, 'Monte Castelo', 24),
(5061, 'Monte Castelo', 26),
(3768, 'Monte das Gameleiras', 20),
(5503, 'Monte do Carmo', 27),
(2062, 'Monte Formoso', 11),
(2682, 'Monte Horebe', 15),
(5062, 'Monte Mor', 26),
(4373, 'Monte Negro', 21),
(540, 'Monte Santo', 5),
(2063, 'Monte Santo de Minas', 11),
(5504, 'Monte Santo do Tocantins', 27),
(2064, 'Monte Sião', 11),
(2683, 'Monteiro', 15),
(5063, 'Monteiro Lobato', 26),
(158, 'Monteirópolis', 2),
(4108, 'Montenegro', 23),
(1250, 'Montes Altos', 10),
(2065, 'Montes Claros', 11),
(1029, 'Montes Claros de Goiás', 9),
(2066, 'Montezuma', 11),
(1030, 'Montividiu', 9),
(1031, 'Montividiu do Norte', 9),
(814, 'Morada Nova', 6),
(2067, 'Morada Nova de Minas', 11),
(815, 'Moraújo', 6),
(3291, 'Moreilândia', 16),
(3009, 'Moreira Sales', 18),
(3292, 'Moreno', 16),
(4109, 'Mormaço', 23),
(541, 'Morpará', 5),
(3010, 'Morretes', 18),
(816, 'Morrinhos', 6),
(1032, 'Morrinhos', 9),
(4110, 'Morrinhos do Sul', 23),
(5064, 'Morro Agudo', 26),
(1033, 'Morro Agudo de Goiás', 9),
(3500, 'Morro Cabeça no Tempo', 17),
(4579, 'Morro da Fumaça', 24),
(2068, 'Morro da Garça', 11),
(542, 'Morro do Chapéu', 5),
(3501, 'Morro do Chapéu do Piauí', 17),
(2069, 'Morro do Pilar', 11),
(4580, 'Morro Grande', 24),
(4111, 'Morro Redondo', 23),
(4112, 'Morro Reuter', 23),
(1251, 'Morros', 10),
(543, 'Mortugaba', 5),
(5065, 'Morungaba', 26),
(1034, 'Mossâmedes', 9),
(3769, 'Mossoró', 20),
(4113, 'Mostardas', 23),
(5066, 'Motuca', 26),
(1035, 'Mozarlândia', 9),
(2492, 'Muaná', 14),
(4406, 'Mucajaí', 22),
(817, 'Mucambo', 6),
(544, 'Mucugê', 5),
(4114, 'Muçum', 23),
(545, 'Mucuri', 5),
(50, 'Mucurici', 8),
(4115, 'Muitos Capões', 23),
(4116, 'Muliterno', 23),
(818, 'Mulungu', 6),
(2684, 'Mulungu', 15),
(546, 'Mulungu do Morro', 5),
(547, 'Mundo Novo', 5),
(1036, 'Mundo Novo', 9),
(1538, 'Mundo Novo', 12),
(2070, 'Munhoz', 11),
(3011, 'Munhoz de Melo', 18),
(548, 'Muniz Ferreira', 5),
(51, 'Muniz Freire', 8),
(549, 'Muquém de São Francisco', 5),
(52, 'Muqui', 8),
(2071, 'Muriaé', 11),
(5391, 'Muribeca', 25),
(159, 'Murici', 2),
(3502, 'Murici dos Portelas', 17),
(5505, 'Muricilândia', 27),
(550, 'Muritiba', 5),
(5067, 'Murutinga do Sul', 26),
(551, 'Mutuípe', 5),
(2072, 'Mutum', 11),
(1037, 'Mutunópolis', 9),
(2073, 'Muzambinho', 11),
(2074, 'Nacip Raydan', 11),
(5068, 'Nantes', 26),
(2075, 'Nanuque', 11),
(4117, 'Não-Me-Toque', 23),
(2076, 'Naque', 11),
(5069, 'Narandiba', 26),
(3770, 'Natal', 20),
(2077, 'Natalândia', 11),
(2078, 'Natércia', 11),
(3636, 'Natividade', 19),
(5506, 'Natividade', 27),
(5070, 'Natividade da Serra', 26),
(2685, 'Natuba', 15),
(4581, 'Navegantes', 24),
(1539, 'Naviraí', 12),
(552, 'Nazaré', 5),
(5507, 'Nazaré', 27),
(3293, 'Nazaré da Mata', 16),
(3503, 'Nazaré do Piauí', 17),
(5071, 'Nazaré Paulista', 26),
(2079, 'Nazareno', 11),
(2686, 'Nazarezinho', 15),
(1038, 'Nazário', 9),
(5392, 'Neópolis', 25),
(2080, 'Nepomuceno', 11),
(1039, 'Nerópolis', 9),
(5072, 'Neves Paulista', 26),
(260, 'Nhamundá', 3),
(5073, 'Nhandeara', 26),
(4118, 'Nicolau Vergueiro', 23),
(553, 'Nilo Peçanha', 5),
(3637, 'Nilópolis', 19),
(1252, 'Nina Rodrigues', 10),
(2081, 'Ninheira', 11),
(1540, 'Nioaque', 12),
(5074, 'Nipoã', 26),
(1040, 'Niquelândia', 9),
(3771, 'Nísia Floresta', 20),
(3638, 'Niterói', 19),
(1414, 'Nobres', 13),
(4119, 'Nonoai', 23),
(554, 'Nordestina', 5),
(4407, 'Normandia', 22),
(1415, 'Nortelândia', 13),
(5393, 'Nossa Senhora Aparecida', 25),
(5394, 'Nossa Senhora da Glória', 25),
(5395, 'Nossa Senhora das Dores', 25),
(3012, 'Nossa Senhora das Graças', 18),
(5396, 'Nossa Senhora de Lourdes', 25),
(3504, 'Nossa Senhora de Nazaré', 17),
(1416, 'Nossa Senhora do Livramento', 13),
(5397, 'Nossa Senhora do Socorro', 25),
(3505, 'Nossa Senhora dos Remédios', 17),
(5075, 'Nova Aliança', 26),
(3013, 'Nova Aliança do Ivaí', 18),
(4120, 'Nova Alvorada', 23),
(1541, 'Nova Alvorada do Sul', 12),
(1041, 'Nova América', 9),
(3014, 'Nova América da Colina', 18),
(1542, 'Nova Andradina', 12),
(4121, 'Nova Araçá', 23),
(1042, 'Nova Aurora', 9),
(3015, 'Nova Aurora', 18),
(1417, 'Nova Bandeirantes', 13),
(4122, 'Nova Bassano', 23),
(2082, 'Nova Belém', 11),
(4123, 'Nova Boa Vista', 23),
(1418, 'Nova Brasilândia', 13),
(4374, 'Nova Brasilândia d`Oeste', 21),
(4124, 'Nova Bréscia', 23),
(5076, 'Nova Campina', 26),
(555, 'Nova Canaã', 5),
(1419, 'Nova Canaã do Norte', 13),
(5077, 'Nova Canaã Paulista', 26),
(4125, 'Nova Candelária', 23),
(3016, 'Nova Cantu', 18),
(5078, 'Nova Castilho', 26),
(1253, 'Nova Colinas', 10),
(1043, 'Nova Crixás', 9),
(3772, 'Nova Cruz', 20),
(2083, 'Nova Era', 11),
(4582, 'Nova Erechim', 24),
(3017, 'Nova Esperança', 18),
(2493, 'Nova Esperança do Piriá', 14),
(3018, 'Nova Esperança do Sudoeste', 18),
(4126, 'Nova Esperança do Sul', 23),
(5079, 'Nova Europa', 26),
(556, 'Nova Fátima', 5),
(3019, 'Nova Fátima', 18),
(2687, 'Nova Floresta', 15),
(3639, 'Nova Friburgo', 19),
(1044, 'Nova Glória', 9),
(5080, 'Nova Granada', 26),
(1420, 'Nova Guarita', 13),
(5081, 'Nova Guataporanga', 26),
(4127, 'Nova Hartz', 23),
(557, 'Nova Ibiá', 5),
(3640, 'Nova Iguaçu', 19),
(1045, 'Nova Iguaçu de Goiás', 9),
(5082, 'Nova Independência', 26),
(1254, 'Nova Iorque', 10),
(2494, 'Nova Ipixuna', 14),
(4583, 'Nova Itaberaba', 24),
(558, 'Nova Itarana', 5),
(1421, 'Nova Lacerda', 13),
(3020, 'Nova Laranjeiras', 18),
(2084, 'Nova Lima', 11),
(3021, 'Nova Londrina', 18),
(5083, 'Nova Luzitânia', 26),
(4375, 'Nova Mamoré', 21),
(1422, 'Nova Marilândia', 13),
(1423, 'Nova Maringá', 13),
(2085, 'Nova Módica', 11),
(1424, 'Nova Monte verde', 13),
(1425, 'Nova Mutum', 13),
(5084, 'Nova Odessa', 26),
(1426, 'Nova Olímpia', 13),
(3022, 'Nova Olímpia', 18),
(819, 'Nova Olinda', 6),
(2688, 'Nova Olinda', 15),
(5508, 'Nova Olinda', 27),
(1255, 'Nova Olinda do Maranhão', 10),
(261, 'Nova Olinda do Norte', 3),
(4128, 'Nova Pádua', 23),
(4129, 'Nova Palma', 23),
(2689, 'Nova Palmeira', 15),
(4130, 'Nova Petrópolis', 23),
(2086, 'Nova Ponte', 11),
(2087, 'Nova Porteirinha', 11),
(4131, 'Nova Prata', 23),
(3023, 'Nova Prata do Iguaçu', 18),
(4132, 'Nova Ramada', 23),
(559, 'Nova Redenção', 5),
(2088, 'Nova Resende', 11),
(1046, 'Nova Roma', 9),
(4133, 'Nova Roma do Sul', 23),
(5509, 'Nova Rosalândia', 27),
(820, 'Nova Russas', 6),
(3024, 'Nova Santa Bárbara', 18),
(1427, 'Nova Santa Helena', 13),
(3506, 'Nova Santa Rita', 17),
(4134, 'Nova Santa Rita', 23),
(3025, 'Nova Santa Rosa', 18),
(2089, 'Nova Serrana', 11),
(560, 'Nova Soure', 5),
(3026, 'Nova Tebas', 18),
(2495, 'Nova Timboteua', 14),
(4584, 'Nova Trento', 24),
(1428, 'Nova Ubiratã', 13),
(2090, 'Nova União', 11),
(4376, 'Nova União', 21),
(53, 'Nova Venécia', 8),
(1047, 'Nova Veneza', 9),
(4585, 'Nova Veneza', 24),
(561, 'Nova Viçosa', 5),
(1429, 'Nova Xavantina', 13),
(5085, 'Novais', 26),
(5510, 'Novo Acordo', 27),
(262, 'Novo Airão', 3),
(5511, 'Novo Alegre', 27),
(263, 'Novo Aripuanã', 3),
(4135, 'Novo Barreiro', 23),
(1048, 'Novo Brasil', 9),
(4136, 'Novo Cabrais', 23),
(2091, 'Novo Cruzeiro', 11),
(1049, 'Novo Gama', 9),
(4137, 'Novo Hamburgo', 23),
(562, 'Novo Horizonte', 5),
(4586, 'Novo Horizonte', 24),
(5086, 'Novo Horizonte', 26),
(1430, 'Novo Horizonte do Norte', 13),
(4377, 'Novo Horizonte do Oeste', 21),
(1543, 'Novo Horizonte do Sul', 12),
(3027, 'Novo Itacolomi', 18),
(5512, 'Novo Jardim', 27),
(160, 'Novo Lino', 2),
(4138, 'Novo Machado', 23),
(1431, 'Novo Mundo', 13),
(821, 'Novo Oriente', 6),
(2092, 'Novo Oriente de Minas', 11),
(3507, 'Novo Oriente do Piauí', 17),
(1050, 'Novo Planalto', 9),
(2496, 'Novo Progresso', 14),
(2497, 'Novo Repartimento', 14),
(1432, 'Novo Santo Antônio', 13),
(3508, 'Novo Santo Antônio', 17),
(1433, 'Novo São Joaquim', 13),
(4139, 'Novo Tiradentes', 23),
(563, 'Novo Triunfo', 5),
(4140, 'Novo Xingu', 23),
(2093, 'Novorizonte', 11),
(5087, 'Nuporanga', 26),
(2498, 'Óbidos', 14),
(822, 'Ocara', 6),
(5088, 'Ocauçu', 26),
(3509, 'Oeiras', 17),
(2499, 'Oeiras do Pará', 14),
(211, 'Oiapoque', 4),
(2094, 'Olaria', 11),
(5089, 'Óleo', 26),
(2690, 'Olho d`Água', 15),
(1256, 'Olho d`Água das Cunhãs', 10),
(161, 'Olho d`Água das Flores', 2),
(162, 'Olho d`Água do Casado', 2),
(3510, 'Olho d`Água do Piauí', 17),
(163, 'Olho d`Água Grande', 2),
(3773, 'Olho-d`Água do Borges', 20),
(2095, 'Olhos-d`Água', 11),
(5090, 'Olímpia', 26),
(2096, 'Olímpio Noronha', 11),
(3294, 'Olinda', 16),
(1257, 'Olinda Nova do Maranhão', 10),
(564, 'Olindina', 5),
(2691, 'Olivedos', 15),
(2097, 'Oliveira', 11),
(5513, 'Oliveira de Fátima', 27),
(565, 'Oliveira dos Brejinhos', 5),
(2098, 'Oliveira Fortes', 11),
(164, 'Olivença', 2),
(2099, 'Onça de Pitangui', 11),
(5091, 'Onda Verde', 26),
(2100, 'Oratórios', 11),
(5092, 'Oriente', 26),
(5093, 'Orindiúva', 26),
(2500, 'Oriximiná', 14),
(2101, 'Orizânia', 11),
(1051, 'Orizona', 9),
(5094, 'Orlândia', 26),
(4587, 'Orleans', 24),
(3295, 'Orobó', 16),
(3296, 'Orocó', 16),
(823, 'Orós', 6),
(3028, 'Ortigueira', 18),
(5095, 'Osasco', 26),
(5096, 'Oscar Bressane', 26),
(4141, 'Osório', 23),
(5097, 'Osvaldo Cruz', 26),
(4588, 'Otacílio Costa', 24),
(2501, 'Ourém', 14),
(566, 'Ouriçangas', 5),
(3297, 'Ouricuri', 16),
(2502, 'Ourilândia do Norte', 14),
(5098, 'Ourinhos', 26),
(3029, 'Ourizona', 18),
(4589, 'Ouro', 24),
(165, 'Ouro Branco', 2),
(2102, 'Ouro Branco', 11),
(3774, 'Ouro Branco', 20),
(2103, 'Ouro Fino', 11),
(2104, 'Ouro Preto', 11),
(4378, 'Ouro Preto do Oeste', 21),
(2692, 'Ouro Velho', 15),
(4590, 'Ouro Verde', 24),
(5099, 'Ouro Verde', 26),
(1052, 'Ouro Verde de Goiás', 9),
(2105, 'Ouro Verde de Minas', 11),
(3030, 'Ouro Verde do Oeste', 18),
(5100, 'Ouroeste', 26),
(567, 'Ourolândia', 5),
(1053, 'Ouvidor', 9),
(5101, 'Pacaembu', 26),
(2503, 'Pacajá', 14),
(824, 'Pacajus', 6),
(4408, 'Pacaraima', 22),
(825, 'Pacatuba', 6),
(5398, 'Pacatuba', 25),
(1258, 'Paço do Lumiar', 10),
(826, 'Pacoti', 6),
(827, 'Pacujá', 6),
(1054, 'Padre Bernardo', 9),
(2106, 'Padre Carvalho', 11),
(3511, 'Padre Marcos', 17),
(2107, 'Padre Paraíso', 11),
(3512, 'Paes Landim', 17),
(2108, 'Pai Pedro', 11),
(4591, 'Paial', 24),
(3031, 'Paiçandu', 18),
(4142, 'Paim Filho', 23),
(2109, 'Paineiras', 11),
(4592, 'Painel', 24),
(2110, 'Pains', 11),
(2111, 'Paiva', 11),
(3513, 'Pajeú do Piauí', 17),
(166, 'Palestina', 2),
(5102, 'Palestina', 26),
(1055, 'Palestina de Goiás', 9),
(2504, 'Palestina do Pará', 14),
(828, 'Palhano', 6),
(4593, 'Palhoça', 24),
(2112, 'Palma', 11),
(4594, 'Palma Sola', 24),
(829, 'Palmácia', 6),
(3298, 'Palmares', 16),
(4143, 'Palmares do Sul', 23),
(5103, 'Palmares Paulista', 26),
(3032, 'Palmas', 18),
(5514, 'Palmas', 27),
(568, 'Palmas de Monte Alto', 5),
(3033, 'Palmeira', 18),
(4595, 'Palmeira', 24),
(5104, 'Palmeira d`Oeste', 26),
(4144, 'Palmeira das Missões', 23),
(3514, 'Palmeira do Piauí', 17),
(167, 'Palmeira dos Índios', 2),
(3515, 'Palmeirais', 17),
(1259, 'Palmeirândia', 10),
(5515, 'Palmeirante', 27),
(569, 'Palmeiras', 5),
(1056, 'Palmeiras de Goiás', 9),
(5516, 'Palmeiras do Tocantins', 27),
(3299, 'Palmeirina', 16),
(5517, 'Palmeirópolis', 27),
(1057, 'Palmelo', 9),
(1058, 'Palminópolis', 9),
(3034, 'Palmital', 18),
(5105, 'Palmital', 26),
(4145, 'Palmitinho', 23),
(4596, 'Palmitos', 24),
(2113, 'Palmópolis', 11),
(3035, 'Palotina', 18),
(1059, 'Panamá', 9),
(4146, 'Panambi', 23),
(54, 'Pancas', 8),
(3300, 'Panelas', 16),
(5106, 'Panorama', 26),
(4147, 'Pantano Grande', 23),
(168, 'Pão de Açúcar', 2),
(2114, 'Papagaios', 11),
(4597, 'Papanduva', 24),
(3516, 'Paquetá', 17),
(2115, 'Pará de Minas', 11),
(3641, 'Paracambi', 19),
(2116, 'Paracatu', 11),
(830, 'Paracuru', 6),
(2505, 'Paragominas', 14),
(2117, 'Paraguaçu', 11),
(5107, 'Paraguaçu Paulista', 26),
(4148, 'Paraí', 23),
(3642, 'Paraíba do Sul', 19),
(1260, 'Paraibano', 10),
(5108, 'Paraibuna', 26),
(831, 'Paraipaba', 6),
(4598, 'Paraíso', 24),
(5109, 'Paraíso', 26),
(3036, 'Paraíso do Norte', 18),
(4149, 'Paraíso do Sul', 23),
(5518, 'Paraíso do Tocantins', 27),
(2118, 'Paraisópolis', 11),
(832, 'Parambu', 6),
(570, 'Paramirim', 5),
(833, 'Paramoti', 6),
(3775, 'Paraná', 20),
(5519, 'Paranã', 27),
(3037, 'Paranacity', 18),
(3038, 'Paranaguá', 18),
(1544, 'Paranaíba', 12),
(1060, 'Paranaiguara', 9),
(1434, 'Paranaíta', 13),
(5110, 'Paranapanema', 26),
(3039, 'Paranapoema', 18),
(5111, 'Paranapuã', 26),
(3301, 'Paranatama', 16),
(1435, 'Paranatinga', 13),
(3040, 'Paranavaí', 18),
(1545, 'Paranhos', 12),
(2119, 'Paraopeba', 11),
(5112, 'Parapuã', 26),
(2693, 'Parari', 15),
(3643, 'Parati', 19),
(571, 'Paratinga', 5),
(3776, 'Paraú', 20),
(2506, 'Parauapebas', 14),
(1061, 'Paraúna', 9),
(3777, 'Parazinho', 20),
(5113, 'Pardinho', 26),
(4150, 'Pareci Novo', 23),
(4379, 'Parecis', 21),
(3778, 'Parelhas', 20),
(169, 'Pariconha', 2),
(264, 'Parintins', 3),
(572, 'Paripiranga', 5),
(170, 'Paripueira', 2),
(5114, 'Pariquera-Açu', 26),
(5115, 'Parisi', 26),
(3517, 'Parnaguá', 17),
(3518, 'Parnaíba', 17),
(3302, 'Parnamirim', 16),
(3779, 'Parnamirim', 20),
(1261, 'Parnarama', 10),
(4151, 'Parobé', 23),
(3780, 'Passa e Fica', 20),
(2120, 'Passa Quatro', 11),
(4152, 'Passa Sete', 23),
(2121, 'Passa Tempo', 11),
(2123, 'Passa-Vinte', 11),
(2122, 'Passabém', 11),
(2694, 'Passagem', 15),
(3781, 'Passagem', 20),
(1262, 'Passagem Franca', 10),
(3519, 'Passagem Franca do Piauí', 17),
(3303, 'Passira', 16),
(171, 'Passo de Camaragibe', 2),
(4599, 'Passo de Torres', 24),
(4153, 'Passo do Sobrado', 23),
(4154, 'Passo Fundo', 23),
(2124, 'Passos', 11),
(4600, 'Passos Maia', 24),
(1263, 'Pastos Bons', 10),
(2125, 'Patis', 11),
(3041, 'Pato Bragado', 18),
(3042, 'Pato Branco', 18),
(2695, 'Patos', 15),
(2126, 'Patos de Minas', 11),
(3520, 'Patos do Piauí', 17),
(2127, 'Patrocínio', 11),
(2128, 'Patrocínio do Muriaé', 11),
(5116, 'Patrocínio Paulista', 26),
(3782, 'Patu', 20),
(3644, 'Paty do Alferes', 19),
(573, 'Pau Brasil', 5),
(2507, 'Pau d`Arco', 14),
(5520, 'Pau d`Arco', 27),
(3521, 'Pau d`Arco do Piauí', 17),
(3783, 'Pau dos Ferros', 20),
(3304, 'Paudalho', 16),
(265, 'Pauini', 3),
(2129, 'Paula Cândido', 11),
(3043, 'Paula Freitas', 18),
(5117, 'Paulicéia', 26),
(5118, 'Paulínia', 26),
(1264, 'Paulino Neves', 10),
(2696, 'Paulista', 15),
(3305, 'Paulista', 16),
(3522, 'Paulistana', 17),
(5119, 'Paulistânia', 26),
(2130, 'Paulistas', 11),
(574, 'Paulo Afonso', 5),
(4155, 'Paulo Bento', 23),
(5120, 'Paulo de Faria', 26),
(3044, 'Paulo Frontin', 18),
(172, 'Paulo Jacinto', 2),
(4601, 'Paulo Lopes', 24),
(1265, 'Paulo Ramos', 10),
(2131, 'Pavão', 11),
(4156, 'Paverama', 23),
(3523, 'Pavussu', 17),
(575, 'Pé de Serra', 5),
(3045, 'Peabiru', 18),
(2132, 'Peçanha', 11),
(5121, 'Pederneiras', 26),
(3306, 'Pedra', 16),
(2133, 'Pedra Azul', 11),
(5122, 'Pedra Bela', 26),
(2134, 'Pedra Bonita', 11),
(834, 'Pedra Branca', 6),
(2697, 'Pedra Branca', 15),
(212, 'Pedra Branca do Amaparí', 4),
(2135, 'Pedra do Anta', 11),
(2136, 'Pedra do Indaiá', 11),
(2137, 'Pedra Dourada', 11),
(3784, 'Pedra Grande', 20),
(2698, 'Pedra Lavrada', 15),
(5399, 'Pedra Mole', 25),
(1436, 'Pedra Preta', 13),
(3785, 'Pedra Preta', 20),
(2138, 'Pedralva', 11),
(5123, 'Pedranópolis', 26),
(576, 'Pedrão', 5),
(4157, 'Pedras Altas', 23),
(2699, 'Pedras de Fogo', 15),
(2139, 'Pedras de Maria da Cruz', 11),
(4602, 'Pedras Grandes', 24),
(5124, 'Pedregulho', 26),
(5125, 'Pedreira', 26),
(1266, 'Pedreiras', 10),
(5400, 'Pedrinhas', 25),
(5126, 'Pedrinhas Paulista', 26),
(2140, 'Pedrinópolis', 11),
(5521, 'Pedro Afonso', 27),
(577, 'Pedro Alexandre', 5),
(3786, 'Pedro Avelino', 20),
(55, 'Pedro Canário', 8),
(5127, 'Pedro de Toledo', 26),
(1267, 'Pedro do Rosário', 10),
(1546, 'Pedro Gomes', 12),
(3524, 'Pedro II', 17),
(3525, 'Pedro Laurentino', 17),
(2141, 'Pedro Leopoldo', 11),
(4158, 'Pedro Osório', 23),
(2700, 'Pedro Régis', 15),
(2142, 'Pedro Teixeira', 11),
(3787, 'Pedro Velho', 20),
(5522, 'Peixe', 27),
(2508, 'Peixe-Boi', 14),
(1437, 'Peixoto de Azevedo', 13),
(4159, 'Pejuçara', 23),
(4160, 'Pelotas', 23),
(835, 'Penaforte', 6),
(1268, 'Penalva', 10),
(5128, 'Penápolis', 26),
(3788, 'Pendências', 20),
(173, 'Penedo', 2),
(4603, 'Penha', 24),
(836, 'Pentecoste', 6),
(2143, 'Pequeri', 11),
(2144, 'Pequi', 11),
(5523, 'Pequizeiro', 27),
(2145, 'Perdigão', 11),
(2146, 'Perdizes', 11),
(2147, 'Perdões', 11),
(5129, 'Pereira Barreto', 26),
(5130, 'Pereiras', 26),
(837, 'Pereiro', 6),
(1269, 'Peri Mirim', 10),
(2148, 'Periquito', 11),
(4604, 'Peritiba', 24),
(1270, 'Peritoró', 10),
(3046, 'Perobal', 18),
(3047, 'Pérola', 18),
(3048, 'Pérola d`Oeste', 18),
(1062, 'Perolândia', 9),
(5131, 'Peruíbe', 26),
(2149, 'Pescador', 11),
(3307, 'Pesqueira', 16),
(3308, 'Petrolândia', 16),
(4605, 'Petrolândia', 24),
(3309, 'Petrolina', 16),
(1063, 'Petrolina de Goiás', 9),
(3645, 'Petrópolis', 19),
(174, 'Piaçabuçu', 2),
(5132, 'Piacatu', 26),
(2701, 'Piancó', 15),
(578, 'Piatã', 5),
(2150, 'Piau', 11),
(4161, 'Picada Café', 23),
(2509, 'Piçarra', 14),
(4606, 'Piçarras', 24),
(3526, 'Picos', 17),
(2702, 'Picuí', 15),
(5133, 'Piedade', 26),
(2151, 'Piedade de Caratinga', 11),
(2152, 'Piedade de Ponte Nova', 11),
(2153, 'Piedade do Rio Grande', 11),
(2154, 'Piedade dos Gerais', 11),
(3049, 'Piên', 18),
(579, 'Pilão Arcado', 5),
(175, 'Pilar', 2),
(2703, 'Pilar', 15),
(1064, 'Pilar de Goiás', 9),
(5134, 'Pilar do Sul', 26),
(2704, 'Pilões', 15),
(3789, 'Pilões', 20),
(2705, 'Pilõezinhos', 15),
(2155, 'Pimenta', 11),
(4380, 'Pimenta Bueno', 21),
(3527, 'Pimenteiras', 17),
(4381, 'Pimenteiras do Oeste', 21),
(580, 'Pindaí', 5),
(5135, 'Pindamonhangaba', 26),
(1271, 'Pindaré-Mirim', 10),
(176, 'Pindoba', 2),
(581, 'Pindobaçu', 5),
(5136, 'Pindorama', 26),
(5524, 'Pindorama do Tocantins', 27),
(838, 'Pindoretama', 6),
(2156, 'Pingo-d`Água', 11),
(3050, 'Pinhais', 18),
(4162, 'Pinhal', 23),
(4163, 'Pinhal da Serra', 23),
(3051, 'Pinhal de São Bento', 18),
(4164, 'Pinhal Grande', 23),
(3052, 'Pinhalão', 18),
(4607, 'Pinhalzinho', 24),
(5137, 'Pinhalzinho', 26),
(3053, 'Pinhão', 18),
(5401, 'Pinhão', 25),
(3646, 'Pinheiral', 19),
(4165, 'Pinheirinho do Vale', 23),
(1272, 'Pinheiro', 10),
(4166, 'Pinheiro Machado', 23),
(4608, 'Pinheiro Preto', 24),
(56, 'Pinheiros', 8),
(582, 'Pintadas', 5),
(2157, 'Pintópolis', 11),
(3528, 'Pio IX', 17),
(1273, 'Pio XII', 10),
(5138, 'Piquerobi', 26),
(839, 'Piquet Carneiro', 6),
(5139, 'Piquete', 26),
(5140, 'Piracaia', 26),
(1065, 'Piracanjuba', 9),
(2158, 'Piracema', 11),
(5141, 'Piracicaba', 26),
(3529, 'Piracuruca', 17),
(3647, 'Piraí', 19),
(583, 'Piraí do Norte', 5),
(3054, 'Piraí do Sul', 18),
(5142, 'Piraju', 26),
(2159, 'Pirajuba', 11),
(5143, 'Pirajuí', 26),
(5402, 'Pirambu', 25),
(2160, 'Piranga', 11),
(5144, 'Pirangi', 26),
(2161, 'Piranguçu', 11),
(2162, 'Piranguinho', 11),
(177, 'Piranhas', 2),
(1066, 'Piranhas', 9),
(1274, 'Pirapemas', 10),
(2163, 'Pirapetinga', 11),
(4167, 'Pirapó', 23),
(2164, 'Pirapora', 11),
(5145, 'Pirapora do Bom Jesus', 26),
(5146, 'Pirapozinho', 26),
(3055, 'Piraquara', 18),
(5525, 'Piraquê', 27),
(5147, 'Pirassununga', 26),
(4168, 'Piratini', 23),
(5148, 'Piratininga', 26),
(4609, 'Piratuba', 24),
(2165, 'Piraúba', 11),
(1067, 'Pirenópolis', 9),
(1068, 'Pires do Rio', 9),
(840, 'Pires Ferreira', 6),
(584, 'Piripá', 5),
(3530, 'Piripiri', 17),
(585, 'Piritiba', 5),
(2706, 'Pirpirituba', 15),
(3056, 'Pitanga', 18),
(3057, 'Pitangueiras', 18),
(5149, 'Pitangueiras', 26),
(2166, 'Pitangui', 11),
(2707, 'Pitimbu', 15),
(5526, 'Pium', 27),
(57, 'Piúma', 8),
(2167, 'Piumhi', 11),
(2510, 'Placas', 14),
(91, 'Plácido de Castro', 1),
(1069, 'Planaltina', 9),
(3058, 'Planaltina do Paraná', 18),
(586, 'Planaltino', 5),
(587, 'Planalto', 5),
(3059, 'Planalto', 18),
(4169, 'Planalto', 23),
(5150, 'Planalto', 26),
(4610, 'Planalto Alegre', 24),
(1438, 'Planalto da Serra', 13),
(2168, 'Planura', 11),
(5151, 'Platina', 26),
(5152, 'Poá', 26),
(3310, 'Poção', 16),
(1275, 'Poção de Pedras', 10),
(2708, 'Pocinhos', 15),
(3790, 'Poço Branco', 20),
(2709, 'Poço Dantas', 15),
(4170, 'Poço das Antas', 23),
(178, 'Poço das Trincheiras', 2),
(2710, 'Poço de José de Moura', 15),
(2169, 'Poço Fundo', 11),
(5403, 'Poço Redondo', 25),
(5404, 'Poço Verde', 25),
(588, 'Poções', 5),
(1439, 'Poconé', 13),
(2170, 'Poços de Caldas', 11),
(2171, 'Pocrane', 11),
(589, 'Pojuca', 5),
(5153, 'Poloni', 26),
(2711, 'Pombal', 15),
(3311, 'Pombos', 16),
(4611, 'Pomerode', 24),
(5154, 'Pompéia', 26),
(2172, 'Pompéu', 11),
(5155, 'Pongaí', 26),
(2511, 'Ponta de Pedras', 14),
(3060, 'Ponta Grossa', 18),
(1547, 'Ponta Porã', 12),
(5156, 'Pontal', 26),
(1440, 'Pontal do Araguaia', 13),
(3061, 'Pontal do Paraná', 18),
(1070, 'Pontalina', 9),
(5157, 'Pontalinda', 26),
(4171, 'Pontão', 23),
(4612, 'Ponte Alta', 24),
(5527, 'Ponte Alta do Bom Jesus', 27),
(4613, 'Ponte Alta do Norte', 24),
(5528, 'Ponte Alta do Tocantins', 27),
(1441, 'Ponte Branca', 13),
(2173, 'Ponte Nova', 11),
(4172, 'Ponte Preta', 23),
(4614, 'Ponte Serrada', 24),
(1442, 'Pontes e Lacerda', 13),
(5158, 'Pontes Gestal', 26),
(58, 'Ponto Belo', 8),
(2174, 'Ponto Chique', 11),
(2175, 'Ponto dos Volantes', 11),
(590, 'Ponto Novo', 5),
(5159, 'Populina', 26),
(841, 'Poranga', 6),
(5160, 'Porangaba', 26),
(1071, 'Porangatu', 9),
(3648, 'Porciúncula', 19),
(3062, 'Porecatu', 18),
(3791, 'Portalegre', 20),
(4173, 'Portão', 23),
(1072, 'Porteirão', 9),
(842, 'Porteiras', 6),
(2176, 'Porteirinha', 11),
(2512, 'Portel', 14),
(1073, 'Portelândia', 9),
(3531, 'Porto', 17),
(92, 'Porto Acre', 1),
(4174, 'Porto Alegre', 23),
(1443, 'Porto Alegre do Norte', 13),
(3532, 'Porto Alegre do Piauí', 17),
(5529, 'Porto Alegre do Tocantins', 27),
(3063, 'Porto Amazonas', 18),
(3064, 'Porto Barreiro', 18),
(4615, 'Porto Belo', 24),
(179, 'Porto Calvo', 2),
(5405, 'Porto da Folha', 25),
(2513, 'Porto de Moz', 14),
(180, 'Porto de Pedras', 2),
(3792, 'Porto do Mangue', 20),
(1444, 'Porto dos Gaúchos', 13),
(1445, 'Porto Esperidião', 13),
(1446, 'Porto Estrela', 13),
(5161, 'Porto Feliz', 26),
(5162, 'Porto Ferreira', 26),
(2177, 'Porto Firme', 11),
(1276, 'Porto Franco', 10),
(213, 'Porto Grande', 4),
(4175, 'Porto Lucena', 23),
(4176, 'Porto Mauá', 23),
(1548, 'Porto Murtinho', 12),
(5530, 'Porto Nacional', 27),
(3649, 'Porto Real', 19),
(181, 'Porto Real do Colégio', 2),
(3065, 'Porto Rico', 18),
(1277, 'Porto Rico do Maranhão', 10),
(591, 'Porto Seguro', 5),
(4616, 'Porto União', 24),
(4382, 'Porto Velho', 21),
(4177, 'Porto Vera Cruz', 23),
(3066, 'Porto Vitória', 18),
(93, 'Porto Walter', 1),
(4178, 'Porto Xavier', 23),
(1074, 'Posse', 9),
(2178, 'Poté', 11),
(843, 'Potengi', 6),
(5163, 'Potim', 26),
(592, 'Potiraguá', 5),
(5164, 'Potirendaba', 26),
(844, 'Potiretama', 6),
(2179, 'Pouso Alegre', 11),
(2180, 'Pouso Alto', 11),
(4179, 'Pouso Novo', 23),
(4617, 'Pouso Redondo', 24),
(1447, 'Poxoréo', 13),
(5165, 'Pracinha', 26),
(214, 'Pracuúba', 4),
(593, 'Prado', 5),
(3067, 'Prado Ferreira', 18),
(5166, 'Pradópolis', 26),
(2181, 'Prados', 11),
(4618, 'Praia Grande', 24),
(5167, 'Praia Grande', 26),
(5531, 'Praia Norte', 27),
(2514, 'Prainha', 14),
(3068, 'Pranchita', 18),
(2182, 'Prata', 11),
(2712, 'Prata', 15),
(3533, 'Prata do Piauí', 17),
(5168, 'Pratânia', 26),
(2183, 'Pratápolis', 11),
(2184, 'Pratinha', 11),
(5169, 'Presidente Alves', 26),
(2185, 'Presidente Bernardes', 11),
(5170, 'Presidente Bernardes', 26),
(3069, 'Presidente Castelo Branco', 18),
(4619, 'Presidente Castelo Branco', 24),
(594, 'Presidente Dutra', 5),
(1278, 'Presidente Dutra', 10),
(5171, 'Presidente Epitácio', 26),
(266, 'Presidente Figueiredo', 3),
(4620, 'Presidente Getúlio', 24),
(595, 'Presidente Jânio Quadros', 5),
(1279, 'Presidente Juscelino', 10),
(2186, 'Presidente Juscelino', 11),
(3793, 'Presidente Juscelino', 20),
(59, 'Presidente Kennedy', 8),
(5532, 'Presidente Kennedy', 27),
(2187, 'Presidente Kubitschek', 11),
(4180, 'Presidente Lucena', 23),
(1280, 'Presidente Médici', 10),
(4383, 'Presidente Médici', 21),
(4621, 'Presidente Nereu', 24),
(2188, 'Presidente Olegário', 11),
(5172, 'Presidente Prudente', 26),
(1281, 'Presidente Sarney', 10),
(596, 'Presidente Tancredo Neves', 5),
(1282, 'Presidente Vargas', 10),
(5173, 'Presidente Venceslau', 26),
(2515, 'Primavera', 14),
(3312, 'Primavera', 16),
(4384, 'Primavera de Rondônia', 21),
(1448, 'Primavera do Leste', 13),
(1283, 'Primeira Cruz', 10),
(3070, 'Primeiro de Maio', 18),
(4622, 'Princesa', 24),
(2713, 'Princesa Isabel', 15),
(1075, 'Professor Jamil', 9),
(4181, 'Progresso', 23),
(5174, 'Promissão', 26),
(5406, 'Propriá', 25),
(4182, 'Protásio Alves', 23),
(2189, 'Prudente de Morais', 11),
(3071, 'Prudentópolis', 18),
(5533, 'Pugmil', 27),
(3794, 'Pureza', 20),
(4183, 'Putinga', 23),
(2714, 'Puxinanã', 15),
(5175, 'Quadra', 26),
(4184, 'Quaraí', 23),
(2190, 'Quartel Geral', 11),
(3072, 'Quarto Centenário', 18),
(5176, 'Quatá', 26),
(3073, 'Quatiguá', 18),
(2516, 'Quatipuru', 14),
(3650, 'Quatis', 19),
(3074, 'Quatro Barras', 18),
(4185, 'Quatro Irmãos', 23),
(3075, 'Quatro Pontes', 18),
(182, 'Quebrangulo', 2),
(3076, 'Quedas do Iguaçu', 18),
(3534, 'Queimada Nova', 17),
(597, 'Queimadas', 5),
(2715, 'Queimadas', 15),
(3651, 'Queimados', 19),
(5177, 'Queiroz', 26),
(5178, 'Queluz', 26),
(2191, 'Queluzito', 11),
(1449, 'Querência', 13),
(3077, 'Querência do Norte', 18),
(4186, 'Quevedos', 23),
(598, 'Quijingue', 5),
(4623, 'Quilombo', 24),
(3078, 'Quinta do Sol', 18),
(5179, 'Quintana', 26),
(4187, 'Quinze de Novembro', 23),
(3313, 'Quipapá', 16),
(1076, 'Quirinópolis', 9),
(3652, 'Quissamã', 19),
(3079, 'Quitandinha', 18),
(845, 'Quiterianópolis', 6),
(2716, 'Quixabá', 15),
(3314, 'Quixaba', 16);
INSERT INTO `cidade` (`id`, `nome_cidade`, `id_uf`) VALUES
(599, 'Quixabeira', 5),
(846, 'Quixadá', 6),
(847, 'Quixelô', 6),
(848, 'Quixeramobim', 6),
(849, 'Quixeré', 6),
(3795, 'Rafael Fernandes', 20),
(3796, 'Rafael Godeiro', 20),
(600, 'Rafael Jambeiro', 5),
(5180, 'Rafard', 26),
(3080, 'Ramilândia', 18),
(5181, 'Rancharia', 26),
(3081, 'Rancho Alegre', 18),
(3082, 'Rancho Alegre d`Oeste', 18),
(4624, 'Rancho Queimado', 24),
(1284, 'Raposa', 10),
(2192, 'Raposos', 11),
(2193, 'Raul Soares', 11),
(3083, 'Realeza', 18),
(3084, 'Rebouças', 18),
(3315, 'Recife', 16),
(2194, 'Recreio', 11),
(5534, 'Recursolândia', 27),
(850, 'Redenção', 6),
(2517, 'Redenção', 14),
(5182, 'Redenção da Serra', 26),
(3535, 'Redenção do Gurguéia', 17),
(4188, 'Redentora', 23),
(2195, 'Reduto', 11),
(3536, 'Regeneração', 17),
(5183, 'Regente Feijó', 26),
(5184, 'Reginópolis', 26),
(5185, 'Registro', 26),
(4189, 'Relvado', 23),
(601, 'Remanso', 5),
(2717, 'Remígio', 15),
(3085, 'Renascença', 18),
(851, 'Reriutaba', 6),
(3653, 'Resende', 19),
(2196, 'Resende Costa', 11),
(3086, 'Reserva', 18),
(1450, 'Reserva do Cabaçal', 13),
(3087, 'Reserva do Iguaçu', 18),
(2197, 'Resplendor', 11),
(2198, 'Ressaquinha', 11),
(5186, 'Restinga', 26),
(4190, 'Restinga Seca', 23),
(602, 'Retirolândia', 5),
(1285, 'Riachão', 10),
(2718, 'Riachão', 15),
(603, 'Riachão das Neves', 5),
(2719, 'Riachão do Bacamarte', 15),
(5407, 'Riachão do Dantas', 25),
(604, 'Riachão do Jacuípe', 5),
(2720, 'Riachão do Poço', 15),
(2199, 'Riachinho', 11),
(5535, 'Riachinho', 27),
(3797, 'Riacho da Cruz', 20),
(3316, 'Riacho das Almas', 16),
(605, 'Riacho de Santana', 5),
(3798, 'Riacho de Santana', 20),
(2721, 'Riacho de Santo Antônio', 15),
(2722, 'Riacho dos Cavalos', 15),
(2200, 'Riacho dos Machados', 11),
(3537, 'Riacho Frio', 17),
(3799, 'Riachuelo', 20),
(5408, 'Riachuelo', 25),
(1077, 'Rialma', 9),
(1078, 'Rianápolis', 9),
(1286, 'Ribamar Fiquene', 10),
(1549, 'Ribas do Rio Pardo', 12),
(5187, 'Ribeira', 26),
(606, 'Ribeira do Amparo', 5),
(3538, 'Ribeira do Piauí', 17),
(607, 'Ribeira do Pombal', 5),
(3317, 'Ribeirão', 16),
(5188, 'Ribeirão Bonito', 26),
(5189, 'Ribeirão Branco', 26),
(1451, 'Ribeirão Cascalheira', 13),
(3088, 'Ribeirão Claro', 18),
(5190, 'Ribeirão Corrente', 26),
(2201, 'Ribeirão das Neves', 11),
(608, 'Ribeirão do Largo', 5),
(3089, 'Ribeirão do Pinhal', 18),
(5191, 'Ribeirão do Sul', 26),
(5192, 'Ribeirão dos Índios', 26),
(5193, 'Ribeirão Grande', 26),
(5194, 'Ribeirão Pires', 26),
(5195, 'Ribeirão Preto', 26),
(2202, 'Ribeirão Vermelho', 11),
(1452, 'Ribeirãozinho', 13),
(3539, 'Ribeiro Gonçalves', 17),
(5409, 'Ribeirópolis', 25),
(5196, 'Rifaina', 26),
(5197, 'Rincão', 26),
(5198, 'Rinópolis', 26),
(2203, 'Rio Acima', 11),
(3090, 'Rio Azul', 18),
(60, 'Rio Bananal', 8),
(3091, 'Rio Bom', 18),
(3654, 'Rio Bonito', 19),
(3092, 'Rio Bonito do Iguaçu', 18),
(94, 'Rio Branco', 1),
(1453, 'Rio Branco', 13),
(3093, 'Rio Branco do Ivaí', 18),
(3094, 'Rio Branco do Sul', 18),
(1550, 'Rio Brilhante', 12),
(2204, 'Rio Casca', 11),
(3655, 'Rio Claro', 19),
(5199, 'Rio Claro', 26),
(4385, 'Rio Crespo', 21),
(5536, 'Rio da Conceição', 27),
(4625, 'Rio das Antas', 24),
(3656, 'Rio das Flores', 19),
(3657, 'Rio das Ostras', 19),
(5200, 'Rio das Pedras', 26),
(609, 'Rio de Contas', 5),
(3658, 'Rio de Janeiro', 19),
(610, 'Rio do Antônio', 5),
(4626, 'Rio do Campo', 24),
(3800, 'Rio do Fogo', 20),
(4627, 'Rio do Oeste', 24),
(611, 'Rio do Pires', 5),
(2205, 'Rio do Prado', 11),
(4628, 'Rio do Sul', 24),
(2206, 'Rio Doce', 11),
(5537, 'Rio dos Bois', 27),
(4629, 'Rio dos Cedros', 24),
(4191, 'Rio dos Índios', 23),
(2207, 'Rio Espera', 11),
(3318, 'Rio Formoso', 16),
(4630, 'Rio Fortuna', 24),
(4192, 'Rio Grande', 23),
(5201, 'Rio Grande da Serra', 26),
(3540, 'Rio Grande do Piauí', 17),
(183, 'Rio Largo', 2),
(2208, 'Rio Manso', 11),
(2518, 'Rio Maria', 14),
(4631, 'Rio Negrinho', 24),
(1551, 'Rio Negro', 12),
(3095, 'Rio Negro', 18),
(2209, 'Rio Novo', 11),
(61, 'Rio Novo do Sul', 8),
(2210, 'Rio Paranaíba', 11),
(4193, 'Rio Pardo', 23),
(2211, 'Rio Pardo de Minas', 11),
(2212, 'Rio Piracicaba', 11),
(2213, 'Rio Pomba', 11),
(2214, 'Rio Preto', 11),
(267, 'Rio Preto da Eva', 3),
(1079, 'Rio Quente', 9),
(612, 'Rio Real', 5),
(4632, 'Rio Rufino', 24),
(5538, 'Rio Sono', 27),
(2723, 'Rio Tinto', 15),
(1080, 'Rio Verde', 9),
(1552, 'Rio Verde de Mato Grosso', 12),
(2215, 'Rio Vermelho', 11),
(5202, 'Riolândia', 26),
(4194, 'Riozinho', 23),
(4633, 'Riqueza', 24),
(2216, 'Ritápolis', 11),
(5203, 'Riversul', 26),
(4195, 'Roca Sales', 23),
(1553, 'Rochedo', 12),
(2217, 'Rochedo de Minas', 11),
(4634, 'Rodeio', 24),
(4196, 'Rodeio Bonito', 23),
(2218, 'Rodeiro', 11),
(613, 'Rodelas', 5),
(3801, 'Rodolfo Fernandes', 20),
(95, 'Rodrigues Alves', 1),
(4197, 'Rolador', 23),
(3096, 'Rolândia', 18),
(4198, 'Rolante', 23),
(4386, 'Rolim de Moura', 21),
(2219, 'Romaria', 11),
(4635, 'Romelândia', 24),
(3097, 'Roncador', 18),
(4199, 'Ronda Alta', 23),
(4200, 'Rondinha', 23),
(1454, 'Rondolândia', 13),
(3098, 'Rondon', 18),
(2519, 'Rondon do Pará', 14),
(1455, 'Rondonópolis', 13),
(4201, 'Roque Gonzales', 23),
(4409, 'Rorainópolis', 22),
(5204, 'Rosana', 26),
(1287, 'Rosário', 10),
(2220, 'Rosário da Limeira', 11),
(5410, 'Rosário do Catete', 25),
(3099, 'Rosário do Ivaí', 18),
(4202, 'Rosário do Sul', 23),
(1456, 'Rosário Oeste', 13),
(5205, 'Roseira', 26),
(184, 'Roteiro', 2),
(2221, 'Rubelita', 11),
(5206, 'Rubiácea', 26),
(1081, 'Rubiataba', 9),
(2222, 'Rubim', 11),
(5207, 'Rubinéia', 26),
(2520, 'Rurópolis', 14),
(852, 'Russas', 6),
(614, 'Ruy Barbosa', 5),
(3802, 'Ruy Barbosa', 20),
(2223, 'Sabará', 11),
(3100, 'Sabáudia', 18),
(5208, 'Sabino', 26),
(2224, 'Sabinópolis', 11),
(853, 'Saboeiro', 6),
(2225, 'Sacramento', 11),
(4203, 'Sagrada Família', 23),
(5209, 'Sagres', 26),
(3319, 'Sairé', 16),
(4204, 'Saldanha Marinho', 23),
(5210, 'Sales', 26),
(5211, 'Sales Oliveira', 26),
(5212, 'Salesópolis', 26),
(4636, 'Salete', 24),
(2724, 'Salgadinho', 15),
(3320, 'Salgadinho', 16),
(5411, 'Salgado', 25),
(2725, 'Salgado de São Félix', 15),
(3101, 'Salgado Filho', 18),
(3321, 'Salgueiro', 16),
(2226, 'Salinas', 11),
(615, 'Salinas da Margarida', 5),
(2521, 'Salinópolis', 14),
(854, 'Salitre', 6),
(5213, 'Salmourão', 26),
(3322, 'Saloá', 16),
(4637, 'Saltinho', 24),
(5214, 'Saltinho', 26),
(5215, 'Salto', 26),
(2227, 'Salto da Divisa', 11),
(5216, 'Salto de Pirapora', 26),
(1457, 'Salto do Céu', 13),
(3102, 'Salto do Itararé', 18),
(4205, 'Salto do Jacuí', 23),
(3103, 'Salto do Lontra', 18),
(5217, 'Salto Grande', 26),
(4638, 'Salto Veloso', 24),
(616, 'Salvador', 5),
(4206, 'Salvador das Missões', 23),
(4207, 'Salvador do Sul', 23),
(2522, 'Salvaterra', 14),
(1288, 'Sambaíba', 10),
(5539, 'Sampaio', 27),
(4208, 'Sananduva', 23),
(1082, 'Sanclerlândia', 9),
(5540, 'Sandolândia', 27),
(5218, 'Sandovalina', 26),
(4639, 'Sangão', 24),
(3323, 'Sanharó', 16),
(5219, 'Santa Adélia', 26),
(5220, 'Santa Albertina', 26),
(3104, 'Santa Amélia', 18),
(617, 'Santa Bárbara', 5),
(2228, 'Santa Bárbara', 11),
(5221, 'Santa Bárbara d`Oeste', 26),
(1083, 'Santa Bárbara de Goiás', 9),
(2229, 'Santa Bárbara do Leste', 11),
(2230, 'Santa Bárbara do Monte Verde', 11),
(2523, 'Santa Bárbara do Pará', 14),
(4209, 'Santa Bárbara do Sul', 23),
(2231, 'Santa Bárbara do Tugúrio', 11),
(5222, 'Santa Branca', 26),
(618, 'Santa Brígida', 5),
(1458, 'Santa Carmem', 13),
(2726, 'Santa Cecília', 15),
(4640, 'Santa Cecília', 24),
(3105, 'Santa Cecília do Pavão', 18),
(4210, 'Santa Cecília do Sul', 23),
(5223, 'Santa Clara d`Oeste', 26),
(4211, 'Santa Clara do Sul', 23),
(2727, 'Santa Cruz', 15),
(3324, 'Santa Cruz', 16),
(3803, 'Santa Cruz', 20),
(619, 'Santa Cruz Cabrália', 5),
(3325, 'Santa Cruz da Baixa Verde', 16),
(5224, 'Santa Cruz da Conceição', 26),
(5225, 'Santa Cruz da Esperança', 26),
(620, 'Santa Cruz da Vitória', 5),
(5226, 'Santa Cruz das Palmeiras', 26),
(1084, 'Santa Cruz de Goiás', 9),
(2232, 'Santa Cruz de Minas', 11),
(3106, 'Santa Cruz de Monte Castelo', 18),
(2233, 'Santa Cruz de Salinas', 11),
(2524, 'Santa Cruz do Arari', 14),
(3326, 'Santa Cruz do Capibaribe', 16),
(2234, 'Santa Cruz do Escalvado', 11),
(3541, 'Santa Cruz do Piauí', 17),
(5227, 'Santa Cruz do Rio Pardo', 26),
(4212, 'Santa Cruz do Sul', 23),
(1459, 'Santa Cruz do Xingu', 13),
(3542, 'Santa Cruz dos Milagres', 17),
(2235, 'Santa Efigênia de Minas', 11),
(5228, 'Santa Ernestina', 26),
(3107, 'Santa Fé', 18),
(1085, 'Santa Fé de Goiás', 9),
(2236, 'Santa Fé de Minas', 11),
(5541, 'Santa Fé do Araguaia', 27),
(5229, 'Santa Fé do Sul', 26),
(3327, 'Santa Filomena', 16),
(3543, 'Santa Filomena', 17),
(1289, 'Santa Filomena do Maranhão', 10),
(5230, 'Santa Gertrudes', 26),
(1290, 'Santa Helena', 10),
(2728, 'Santa Helena', 15),
(3108, 'Santa Helena', 18),
(4641, 'Santa Helena', 24),
(1086, 'Santa Helena de Goiás', 9),
(2237, 'Santa Helena de Minas', 11),
(621, 'Santa Inês', 5),
(1291, 'Santa Inês', 10),
(2729, 'Santa Inês', 15),
(3109, 'Santa Inês', 18),
(1087, 'Santa Isabel', 9),
(5231, 'Santa Isabel', 26),
(3110, 'Santa Isabel do Ivaí', 18),
(2525, 'Santa Isabel do Pará', 14),
(268, 'Santa Isabel do Rio Negro', 3),
(3111, 'Santa Izabel do Oeste', 18),
(2238, 'Santa Juliana', 11),
(62, 'Santa Leopoldina', 8),
(3112, 'Santa Lúcia', 18),
(5232, 'Santa Lúcia', 26),
(3544, 'Santa Luz', 17),
(622, 'Santa Luzia', 5),
(1292, 'Santa Luzia', 10),
(2239, 'Santa Luzia', 11),
(2730, 'Santa Luzia', 15),
(4387, 'Santa Luzia d`Oeste', 21),
(5412, 'Santa Luzia do Itanhy', 25),
(185, 'Santa Luzia do Norte', 2),
(2526, 'Santa Luzia do Pará', 14),
(1293, 'Santa Luzia do Paruá', 10),
(2240, 'Santa Margarida', 11),
(4213, 'Santa Margarida do Sul', 23),
(3804, 'Santa Maria', 20),
(4214, 'Santa Maria', 23),
(3328, 'Santa Maria da Boa Vista', 16),
(5233, 'Santa Maria da Serra', 26),
(623, 'Santa Maria da Vitória', 5),
(2527, 'Santa Maria das Barreiras', 14),
(2241, 'Santa Maria de Itabira', 11),
(63, 'Santa Maria de Jetibá', 8),
(3329, 'Santa Maria do Cambucá', 16),
(4215, 'Santa Maria do Herval', 23),
(3113, 'Santa Maria do Oeste', 18),
(2528, 'Santa Maria do Pará', 14),
(2242, 'Santa Maria do Salto', 11),
(2243, 'Santa Maria do Suaçuí', 11),
(5542, 'Santa Maria do Tocantins', 27),
(3659, 'Santa Maria Madalena', 19),
(3114, 'Santa Mariana', 18),
(5234, 'Santa Mercedes', 26),
(3115, 'Santa Mônica', 18),
(855, 'Santa Quitéria', 6),
(1294, 'Santa Quitéria do Maranhão', 10),
(1295, 'Santa Rita', 10),
(2731, 'Santa Rita', 15),
(5235, 'Santa Rita d`Oeste', 26),
(2244, 'Santa Rita de Caldas', 11),
(624, 'Santa Rita de Cássia', 5),
(2245, 'Santa Rita de Ibitipoca', 11),
(2246, 'Santa Rita de Jacutinga', 11),
(2247, 'Santa Rita de Minas', 11),
(1088, 'Santa Rita do Araguaia', 9),
(2248, 'Santa Rita do Itueto', 11),
(1089, 'Santa Rita do Novo Destino', 9),
(1554, 'Santa Rita do Pardo', 12),
(5236, 'Santa Rita do Passa Quatro', 26),
(2249, 'Santa Rita do Sapucaí', 11),
(5543, 'Santa Rita do Tocantins', 27),
(1460, 'Santa Rita do Trivelato', 13),
(4216, 'Santa Rosa', 23),
(2250, 'Santa Rosa da Serra', 11),
(1090, 'Santa Rosa de Goiás', 9),
(4642, 'Santa Rosa de Lima', 24),
(5413, 'Santa Rosa de Lima', 25),
(5237, 'Santa Rosa de Viterbo', 26),
(3545, 'Santa Rosa do Piauí', 17),
(96, 'Santa Rosa do Purus', 1),
(4643, 'Santa Rosa do Sul', 24),
(5544, 'Santa Rosa do Tocantins', 27),
(5238, 'Santa Salete', 26),
(64, 'Santa Teresa', 8),
(625, 'Santa Teresinha', 5),
(2732, 'Santa Teresinha', 15),
(4217, 'Santa Tereza', 23),
(1091, 'Santa Tereza de Goiás', 9),
(3116, 'Santa Tereza do Oeste', 18),
(5545, 'Santa Tereza do Tocantins', 27),
(1461, 'Santa Terezinha', 13),
(3330, 'Santa Terezinha', 16),
(4644, 'Santa Terezinha', 24),
(1092, 'Santa Terezinha de Goiás', 9),
(3117, 'Santa Terezinha de Itaipu', 18),
(4645, 'Santa Terezinha do Progresso', 24),
(5546, 'Santa Terezinha do Tocantins', 27),
(2251, 'Santa Vitória', 11),
(4218, 'Santa Vitória do Palmar', 23),
(626, 'Santaluz', 5),
(215, 'Santana', 4),
(627, 'Santana', 5),
(4219, 'Santana da Boa Vista', 23),
(5239, 'Santana da Ponte Pensa', 26),
(2252, 'Santana da Vargem', 11),
(2253, 'Santana de Cataguases', 11),
(2733, 'Santana de Mangueira', 15),
(5240, 'Santana de Parnaíba', 26),
(2254, 'Santana de Pirapama', 11),
(856, 'Santana do Acaraú', 6),
(2529, 'Santana do Araguaia', 14),
(857, 'Santana do Cariri', 6),
(2255, 'Santana do Deserto', 11),
(2256, 'Santana do Garambéu', 11),
(186, 'Santana do Ipanema', 2),
(3118, 'Santana do Itararé', 18),
(2257, 'Santana do Jacaré', 11),
(4220, 'Santana do Livramento', 23),
(2258, 'Santana do Manhuaçu', 11),
(1296, 'Santana do Maranhão', 10),
(3805, 'Santana do Matos', 20),
(187, 'Santana do Mundaú', 2),
(2259, 'Santana do Paraíso', 11),
(3546, 'Santana do Piauí', 17),
(2260, 'Santana do Riacho', 11),
(5414, 'Santana do São Francisco', 25),
(3806, 'Santana do Seridó', 20),
(2734, 'Santana dos Garrotes', 15),
(2261, 'Santana dos Montes', 11),
(628, 'Santanópolis', 5),
(2530, 'Santarém', 14),
(2735, 'Santarém', 15),
(2531, 'Santarém Novo', 14),
(4221, 'Santiago', 23),
(4646, 'Santiago do Sul', 24),
(1462, 'Santo Afonso', 13),
(629, 'Santo Amaro', 5),
(4647, 'Santo Amaro da Imperatriz', 24),
(5415, 'Santo Amaro das Brotas', 25),
(1297, 'Santo Amaro do Maranhão', 10),
(5241, 'Santo Anastácio', 26),
(2736, 'Santo André', 15),
(5242, 'Santo André', 26),
(4222, 'Santo Ângelo', 23),
(3807, 'Santo Antônio', 20),
(5243, 'Santo Antônio da Alegria', 26),
(1093, 'Santo Antônio da Barra', 9),
(4223, 'Santo Antônio da Patrulha', 23),
(3119, 'Santo Antônio da Platina', 18),
(4224, 'Santo Antônio das Missões', 23),
(1094, 'Santo Antônio de Goiás', 9),
(630, 'Santo Antônio de Jesus', 5),
(3547, 'Santo Antônio de Lisboa', 17),
(3660, 'Santo Antônio de Pádua', 19),
(5244, 'Santo Antônio de Posse', 26),
(2262, 'Santo Antônio do Amparo', 11),
(5245, 'Santo Antônio do Aracanguá', 26),
(2263, 'Santo Antônio do Aventureiro', 11),
(3120, 'Santo Antônio do Caiuá', 18),
(1095, 'Santo Antônio do Descoberto', 9),
(2264, 'Santo Antônio do Grama', 11),
(269, 'Santo Antônio do Içá', 3),
(2265, 'Santo Antônio do Itambé', 11),
(2266, 'Santo Antônio do Jacinto', 11),
(5246, 'Santo Antônio do Jardim', 26),
(1463, 'Santo Antônio do Leste', 13),
(1464, 'Santo Antônio do Leverger', 13),
(2267, 'Santo Antônio do Monte', 11),
(4225, 'Santo Antônio do Palma', 23),
(3121, 'Santo Antônio do Paraíso', 18),
(5247, 'Santo Antônio do Pinhal', 26),
(4226, 'Santo Antônio do Planalto', 23),
(2268, 'Santo Antônio do Retiro', 11),
(2269, 'Santo Antônio do Rio Abaixo', 11),
(3122, 'Santo Antônio do Sudoeste', 18),
(2532, 'Santo Antônio do Tauá', 14),
(1298, 'Santo Antônio dos Lopes', 10),
(3548, 'Santo Antônio dos Milagres', 17),
(4227, 'Santo Augusto', 23),
(4228, 'Santo Cristo', 23),
(631, 'Santo Estêvão', 5),
(5248, 'Santo Expedito', 26),
(4229, 'Santo Expedito do Sul', 23),
(2270, 'Santo Hipólito', 11),
(3123, 'Santo Inácio', 18),
(3549, 'Santo Inácio do Piauí', 17),
(5249, 'Santópolis do Aguapeí', 26),
(5250, 'Santos', 26),
(2271, 'Santos Dumont', 11),
(858, 'São Benedito', 6),
(1299, 'São Benedito do Rio Preto', 10),
(3331, 'São Benedito do Sul', 16),
(2737, 'São Bentinho', 15),
(1300, 'São Bento', 10),
(2738, 'São Bento', 15),
(2272, 'São Bento Abade', 11),
(3808, 'São Bento do Norte', 20),
(5251, 'São Bento do Sapucaí', 26),
(4648, 'São Bento do Sul', 24),
(5547, 'São Bento do Tocantins', 27),
(3809, 'São Bento do Trairí', 20),
(3332, 'São Bento do Una', 16),
(4649, 'São Bernardino', 24),
(1301, 'São Bernardo', 10),
(5252, 'São Bernardo do Campo', 26),
(4650, 'São Bonifácio', 24),
(4230, 'São Borja', 23),
(188, 'São Brás', 2),
(2273, 'São Brás do Suaçuí', 11),
(3550, 'São Braz do Piauí', 17),
(2533, 'São Caetano de Odivelas', 14),
(5253, 'São Caetano do Sul', 26),
(3333, 'São Caitano', 16),
(4651, 'São Carlos', 24),
(5254, 'São Carlos', 26),
(3124, 'São Carlos do Ivaí', 18),
(5416, 'São Cristóvão', 25),
(4652, 'São Cristovão do Sul', 24),
(632, 'São Desidério', 5),
(633, 'São Domingos', 5),
(1096, 'São Domingos', 9),
(4653, 'São Domingos', 24),
(5417, 'São Domingos', 25),
(2274, 'São Domingos das Dores', 11),
(2739, 'São Domingos de Pombal', 15),
(2534, 'São Domingos do Araguaia', 14),
(1302, 'São Domingos do Azeitão', 10),
(2535, 'São Domingos do Capim', 14),
(2740, 'São Domingos do Cariri', 15),
(1303, 'São Domingos do Maranhão', 10),
(65, 'São Domingos do Norte', 8),
(2275, 'São Domingos do Prata', 11),
(4231, 'São Domingos do Sul', 23),
(634, 'São Felipe', 5),
(4388, 'São Felipe d`Oeste', 21),
(635, 'São Félix', 5),
(1304, 'São Félix de Balsas', 10),
(2276, 'São Félix de Minas', 11),
(1465, 'São Félix do Araguaia', 13),
(636, 'São Félix do Coribe', 5),
(3551, 'São Félix do Piauí', 17),
(5548, 'São Félix do Tocantins', 27),
(2536, 'São Félix do Xingu', 14),
(3810, 'São Fernando', 20),
(3661, 'São Fidélis', 19),
(2277, 'São Francisco', 11),
(2741, 'São Francisco', 15),
(5418, 'São Francisco', 25),
(5255, 'São Francisco', 26),
(4232, 'São Francisco de Assis', 23),
(3552, 'São Francisco de Assis do Piauí', 17),
(1097, 'São Francisco de Goiás', 9),
(3662, 'São Francisco de Itabapoana', 19),
(2278, 'São Francisco de Paula', 11),
(4233, 'São Francisco de Paula', 23),
(2279, 'São Francisco de Sales', 11),
(1305, 'São Francisco do Brejão', 10),
(637, 'São Francisco do Conde', 5),
(2280, 'São Francisco do Glória', 11),
(4389, 'São Francisco do Guaporé', 21),
(1306, 'São Francisco do Maranhão', 10),
(3811, 'São Francisco do Oeste', 20),
(2537, 'São Francisco do Pará', 14),
(3553, 'São Francisco do Piauí', 17),
(4654, 'São Francisco do Sul', 24),
(638, 'São Gabriel', 5),
(4234, 'São Gabriel', 23),
(270, 'São Gabriel da Cachoeira', 3),
(66, 'São Gabriel da Palha', 8),
(1555, 'São Gabriel do Oeste', 12),
(2281, 'São Geraldo', 11),
(2282, 'São Geraldo da Piedade', 11),
(2538, 'São Geraldo do Araguaia', 14),
(2283, 'São Geraldo do Baixio', 11),
(3663, 'São Gonçalo', 19),
(2284, 'São Gonçalo do Abaeté', 11),
(859, 'São Gonçalo do Amarante', 6),
(3812, 'São Gonçalo do Amarante', 20),
(3554, 'São Gonçalo do Gurguéia', 17),
(2285, 'São Gonçalo do Pará', 11),
(3555, 'São Gonçalo do Piauí', 17),
(2286, 'São Gonçalo do Rio Abaixo', 11),
(2287, 'São Gonçalo do Rio Preto', 11),
(2288, 'São Gonçalo do Sapucaí', 11),
(639, 'São Gonçalo dos Campos', 5),
(2289, 'São Gotardo', 11),
(4235, 'São Jerônimo', 23),
(3125, 'São Jerônimo da Serra', 18),
(3334, 'São João', 16),
(3126, 'São João', 18),
(1307, 'São João Batista', 10),
(4655, 'São João Batista', 24),
(2290, 'São João Batista do Glória', 11),
(1098, 'São João d`Aliança', 9),
(4410, 'São João da Baliza', 22),
(3664, 'São João da Barra', 19),
(5256, 'São João da Boa Vista', 26),
(3556, 'São João da Canabrava', 17),
(3557, 'São João da Fronteira', 17),
(2291, 'São João da Lagoa', 11),
(2292, 'São João da Mata', 11),
(1099, 'São João da Paraúna', 9),
(2539, 'São João da Ponta', 14),
(2293, 'São João da Ponte', 11),
(3558, 'São João da Serra', 17),
(4236, 'São João da Urtiga', 23),
(3559, 'São João da Varjota', 17),
(5257, 'São João das Duas Pontes', 26),
(2294, 'São João das Missões', 11),
(5258, 'São João de Iracema', 26),
(3665, 'São João de Meriti', 19),
(2540, 'São João de Pirabas', 14),
(2295, 'São João del Rei', 11),
(2541, 'São João do Araguaia', 14),
(3560, 'São João do Arraial', 17),
(3127, 'São João do Caiuá', 18),
(2742, 'São João do Cariri', 15),
(1308, 'São João do Carú', 10),
(4656, 'São João do Itaperiú', 24),
(3128, 'São João do Ivaí', 18),
(860, 'São João do Jaguaribe', 6),
(2296, 'São João do Manhuaçu', 11),
(2297, 'São João do Manteninha', 11),
(4657, 'São João do Oeste', 24),
(2298, 'São João do Oriente', 11),
(2299, 'São João do Pacuí', 11),
(1309, 'São João do Paraíso', 10),
(2300, 'São João do Paraíso', 11),
(5259, 'São João do Pau d`Alho', 26),
(3561, 'São João do Piauí', 17),
(4237, 'São João do Polêsine', 23),
(2743, 'São João do Rio do Peixe', 15),
(3813, 'São João do Sabugi', 20),
(1310, 'São João do Soter', 10),
(4658, 'São João do Sul', 24),
(2744, 'São João do Tigre', 15),
(3129, 'São João do Triunfo', 18),
(1311, 'São João dos Patos', 10),
(2301, 'São João Evangelista', 11),
(2302, 'São João Nepomuceno', 11),
(4659, 'São Joaquim', 24),
(5260, 'São Joaquim da Barra', 26),
(2303, 'São Joaquim de Bicas', 11),
(3335, 'São Joaquim do Monte', 16),
(4238, 'São Jorge', 23),
(3130, 'São Jorge d`Oeste', 18),
(3131, 'São Jorge do Ivaí', 18),
(3132, 'São Jorge do Patrocínio', 18),
(4660, 'São José', 24),
(2304, 'São José da Barra', 11),
(5261, 'São José da Bela Vista', 26),
(3133, 'São José da Boa Vista', 18),
(3336, 'São José da Coroa Grande', 16),
(2745, 'São José da Lagoa Tapada', 15),
(189, 'São José da Laje', 2),
(2305, 'São José da Lapa', 11),
(2306, 'São José da Safira', 11),
(190, 'São José da Tapera', 2),
(2307, 'São José da Varginha', 11),
(640, 'São José da Vitória', 5),
(4239, 'São José das Missões', 23),
(3134, 'São José das Palmeiras', 18),
(2746, 'São José de Caiana', 15),
(2747, 'São José de Espinharas', 15),
(3814, 'São José de Mipibu', 20),
(2748, 'São José de Piranhas', 15),
(2749, 'São José de Princesa', 15),
(1312, 'São José de Ribamar', 10),
(3666, 'São José de Ubá', 19),
(2308, 'São José do Alegre', 11),
(5262, 'São José do Barreiro', 26),
(3337, 'São José do Belmonte', 16),
(2750, 'São José do Bonfim', 15),
(2751, 'São José do Brejo do Cruz', 15),
(67, 'São José do Calçado', 8),
(3815, 'São José do Campestre', 20),
(4661, 'São José do Cedro', 24),
(4662, 'São José do Cerrito', 24),
(2309, 'São José do Divino', 11),
(3562, 'São José do Divino', 17),
(3338, 'São José do Egito', 16),
(2310, 'São José do Goiabal', 11),
(4240, 'São José do Herval', 23),
(4241, 'São José do Hortêncio', 23),
(4242, 'São José do Inhacorá', 23),
(641, 'São José do Jacuípe', 5),
(2311, 'São José do Jacuri', 11),
(2312, 'São José do Mantimento', 11),
(4243, 'São José do Norte', 23),
(4244, 'São José do Ouro', 23),
(3563, 'São José do Peixe', 17),
(3564, 'São José do Piauí', 17),
(1466, 'São José do Povo', 13),
(1467, 'São José do Rio Claro', 13),
(5263, 'São José do Rio Pardo', 26),
(5264, 'São José do Rio Preto', 26),
(2752, 'São José do Sabugi', 15),
(3816, 'São José do Seridó', 20),
(4245, 'São José do Sul', 23),
(3667, 'São José do Vale do Rio Pret', 19),
(1468, 'São José do Xingu', 13),
(4246, 'São José dos Ausentes', 23),
(1313, 'São José dos Basílios', 10),
(5265, 'São José dos Campos', 26),
(2753, 'São José dos Cordeiros', 15),
(3135, 'São José dos Pinhais', 18),
(1469, 'São José dos Quatro Marcos', 13),
(2754, 'São José dos Ramos', 15),
(3565, 'São Julião', 17),
(4247, 'São Leopoldo', 23),
(2313, 'São Lourenço', 11),
(3339, 'São Lourenço da Mata', 16),
(5266, 'São Lourenço da Serra', 26),
(4663, 'São Lourenço do Oeste', 24),
(3566, 'São Lourenço do Piauí', 17),
(4248, 'São Lourenço do Sul', 23),
(4664, 'São Ludgero', 24),
(1314, 'São Luís', 10),
(1100, 'São Luís de Montes Belos', 9),
(861, 'São Luís do Curu', 6),
(5267, 'São Luís do Paraitinga', 26),
(3567, 'São Luis do Piauí', 17),
(191, 'São Luís do Quitunde', 2),
(1315, 'São Luís Gonzaga do Maranhão', 10),
(4411, 'São Luiz', 22),
(1101, 'São Luíz do Norte', 9),
(4249, 'São Luiz Gonzaga', 23),
(2755, 'São Mamede', 15),
(3136, 'São Manoel do Paraná', 18),
(5268, 'São Manuel', 26),
(4250, 'São Marcos', 23),
(4251, 'São Martinho', 23),
(4665, 'São Martinho', 24),
(4252, 'São Martinho da Serra', 23),
(68, 'São Mateus', 8),
(1316, 'São Mateus do Maranhão', 10),
(3137, 'São Mateus do Sul', 18),
(3817, 'São Miguel', 20),
(5269, 'São Miguel Arcanjo', 26),
(3568, 'São Miguel da Baixa Grande', 17),
(4666, 'São Miguel da Boa Vista', 24),
(642, 'São Miguel das Matas', 5),
(4253, 'São Miguel das Missões', 23),
(2756, 'São Miguel de Taipu', 15),
(5419, 'São Miguel do Aleixo', 25),
(2314, 'São Miguel do Anta', 11),
(1102, 'São Miguel do Araguaia', 9),
(3569, 'São Miguel do Fidalgo', 17),
(3818, 'São Miguel do Gostoso', 20),
(2542, 'São Miguel do Guamá', 14),
(4390, 'São Miguel do Guaporé', 21),
(3138, 'São Miguel do Iguaçu', 18),
(4667, 'São Miguel do Oeste', 24),
(1103, 'São Miguel do Passa Quatro', 9),
(3570, 'São Miguel do Tapuio', 17),
(5549, 'São Miguel do Tocantins', 27),
(192, 'São Miguel dos Campos', 2),
(193, 'São Miguel dos Milagres', 2),
(4254, 'São Nicolau', 23),
(1104, 'São Patrício', 9),
(5270, 'São Paulo', 26),
(4255, 'São Paulo das Missões', 23),
(271, 'São Paulo de Olivença', 3),
(3819, 'São Paulo do Potengi', 20),
(3820, 'São Pedro', 20),
(5271, 'São Pedro', 26),
(1317, 'São Pedro da Água Branca', 10),
(3668, 'São Pedro da Aldeia', 19),
(1470, 'São Pedro da Cipa', 13),
(4256, 'São Pedro da Serra', 23),
(2315, 'São Pedro da União', 11),
(4257, 'São Pedro das Missões', 23),
(4668, 'São Pedro de Alcântara', 24),
(4258, 'São Pedro do Butiá', 23),
(3139, 'São Pedro do Iguaçu', 18),
(3140, 'São Pedro do Ivaí', 18),
(3141, 'São Pedro do Paraná', 18),
(3571, 'São Pedro do Piauí', 17),
(2316, 'São Pedro do Suaçuí', 11),
(4259, 'São Pedro do Sul', 23),
(5272, 'São Pedro do Turvo', 26),
(1318, 'São Pedro dos Crentes', 10),
(2317, 'São Pedro dos Ferros', 11),
(3821, 'São Rafael', 20),
(1319, 'São Raimundo das Mangabeiras', 10),
(1320, 'São Raimundo do Doca Bezerra', 10),
(3572, 'São Raimundo Nonato', 17),
(1321, 'São Roberto', 10),
(2318, 'São Romão', 11),
(5273, 'São Roque', 26),
(2319, 'São Roque de Minas', 11),
(69, 'São Roque do Canaã', 8),
(5550, 'São Salvador do Tocantins', 27),
(194, 'São Sebastião', 2),
(5274, 'São Sebastião', 26),
(3142, 'São Sebastião da Amoreira', 18),
(2320, 'São Sebastião da Bela Vista', 11),
(2543, 'São Sebastião da Boa Vista', 14),
(5275, 'São Sebastião da Grama', 26),
(2321, 'São Sebastião da Vargem Alegre', 11),
(2757, 'São Sebastião de Lagoa de Roça', 15),
(3669, 'São Sebastião do Alto', 19),
(2322, 'São Sebastião do Anta', 11),
(4260, 'São Sebastião do Caí', 23),
(2323, 'São Sebastião do Maranhão', 11),
(2324, 'São Sebastião do Oeste', 11),
(2325, 'São Sebastião do Paraíso', 11),
(643, 'São Sebastião do Passé', 5),
(2326, 'São Sebastião do Rio Preto', 11),
(2327, 'São Sebastião do Rio Verde', 11),
(5551, 'São Sebastião do Tocantins', 27),
(272, 'São Sebastião do Uatumã', 3),
(2758, 'São Sebastião do Umbuzeiro', 15),
(4261, 'São Sepé', 23),
(1105, 'São Simão', 9),
(5276, 'São Simão', 26),
(2328, 'São Thomé das Letras', 11),
(2329, 'São Tiago', 11),
(2330, 'São Tomás de Aquino', 11),
(3143, 'São Tomé', 18),
(3822, 'São Tomé', 20),
(4262, 'São Valentim', 23),
(4263, 'São Valentim do Sul', 23),
(5552, 'São Valério da Natividade', 27),
(4264, 'São Valério do Sul', 23),
(4265, 'São Vendelino', 23),
(3823, 'São Vicente', 20),
(5277, 'São Vicente', 26),
(2331, 'São Vicente de Minas', 11),
(4266, 'São Vicente do Sul', 23),
(1322, 'São Vicente Ferrer', 10),
(3340, 'São Vicente Ferrer', 16),
(2759, 'Sapé', 15),
(644, 'Sapeaçu', 5),
(1471, 'Sapezal', 13),
(4267, 'Sapiranga', 23),
(3144, 'Sapopema', 18),
(2332, 'Sapucaí-Mirim', 11),
(2544, 'Sapucaia', 14),
(3670, 'Sapucaia', 19),
(4268, 'Sapucaia do Sul', 23),
(3671, 'Saquarema', 19),
(3145, 'Sarandi', 18),
(4269, 'Sarandi', 23),
(5278, 'Sarapuí', 26),
(2333, 'Sardoá', 11),
(5279, 'Sarutaiá', 26),
(2334, 'Sarzedo', 11),
(645, 'Sátiro Dias', 5),
(195, 'Satuba', 2),
(1323, 'Satubinha', 10),
(646, 'Saubara', 5),
(3146, 'Saudade do Iguaçu', 18),
(4669, 'Saudades', 24),
(647, 'Saúde', 5),
(4670, 'Schroeder', 24),
(648, 'Seabra', 5),
(4671, 'Seara', 24),
(5280, 'Sebastianópolis do Sul', 26),
(3573, 'Sebastião Barros', 17),
(649, 'Sebastião Laranjeiras', 5),
(3574, 'Sebastião Leal', 17),
(4270, 'Seberi', 23),
(4271, 'Sede Nova', 23),
(4272, 'Segredo', 23),
(4273, 'Selbach', 23),
(1556, 'Selvíria', 12),
(2335, 'Sem-Peixe', 11),
(97, 'Sena Madureira', 1),
(1324, 'Senador Alexandre Costa', 10),
(2336, 'Senador Amaral', 11),
(1106, 'Senador Canedo', 9),
(2337, 'Senador Cortes', 11),
(3824, 'Senador Elói de Souza', 20),
(2338, 'Senador Firmino', 11),
(3825, 'Senador Georgino Avelino', 20),
(98, 'Senador Guiomard', 1),
(2339, 'Senador José Bento', 11),
(2545, 'Senador José Porfírio', 14),
(1325, 'Senador La Rocque', 10),
(2340, 'Senador Modestino Gonçalves', 11),
(862, 'Senador Pompeu', 6),
(196, 'Senador Rui Palmeira', 2),
(863, 'Senador Sá', 6),
(4274, 'Senador Salgado Filho', 23),
(3147, 'Sengés', 18),
(650, 'Senhor do Bonfim', 5),
(2341, 'Senhora de Oliveira', 11),
(2342, 'Senhora do Porto', 11),
(2343, 'Senhora dos Remédios', 11),
(4275, 'Sentinela do Sul', 23),
(651, 'Sento Sé', 5),
(4276, 'Serafina Corrêa', 23),
(2344, 'Sericita', 11),
(2760, 'Seridó', 15),
(4391, 'Seringueiras', 21),
(4277, 'Sério', 23),
(2345, 'Seritinga', 11),
(3672, 'Seropédica', 19),
(70, 'Serra', 8),
(4672, 'Serra Alta', 24),
(5281, 'Serra Azul', 26),
(2346, 'Serra Azul de Minas', 11),
(2761, 'Serra Branca', 15),
(2762, 'Serra da Raiz', 15),
(2347, 'Serra da Saudade', 11),
(3826, 'Serra de São Bento', 20),
(3827, 'Serra do Mel', 20),
(216, 'Serra do Navio', 4),
(652, 'Serra do Ramalho', 5),
(2348, 'Serra do Salitre', 11),
(2349, 'Serra dos Aimorés', 11),
(653, 'Serra Dourada', 5),
(2763, 'Serra Grande', 15),
(5282, 'Serra Negra', 26),
(3828, 'Serra Negra do Norte', 20),
(1472, 'Serra Nova Dourada', 13),
(654, 'Serra Preta', 5),
(2764, 'Serra Redonda', 15),
(3341, 'Serra Talhada', 16),
(5283, 'Serrana', 26),
(2350, 'Serrania', 11),
(1326, 'Serrano do Maranhão', 10),
(1107, 'Serranópolis', 9),
(2351, 'Serranópolis de Minas', 11),
(3148, 'Serranópolis do Iguaçu', 18),
(2352, 'Serranos', 11),
(2765, 'Serraria', 15),
(655, 'Serrinha', 5),
(3829, 'Serrinha', 20),
(3830, 'Serrinha dos Pintos', 20),
(3342, 'Serrita', 16),
(2353, 'Serro', 11),
(656, 'Serrolândia', 5),
(3149, 'Sertaneja', 18),
(3343, 'Sertânia', 16),
(3150, 'Sertanópolis', 18),
(4278, 'Sertão', 23),
(4279, 'Sertão Santana', 23),
(2766, 'Sertãozinho', 15),
(5284, 'Sertãozinho', 26),
(5285, 'Sete Barras', 26),
(4280, 'Sete de Setembro', 23),
(2354, 'Sete Lagoas', 11),
(1557, 'Sete Quedas', 12),
(2355, 'Setubinha', 11),
(4281, 'Severiano de Almeida', 23),
(3831, 'Severiano Melo', 20),
(5286, 'Severínia', 26),
(4673, 'Siderópolis', 24),
(1558, 'Sidrolândia', 12),
(3575, 'Sigefredo Pacheco', 17),
(3673, 'Silva Jardim', 19),
(1108, 'Silvânia', 9),
(5553, 'Silvanópolis', 27),
(4282, 'Silveira Martins', 23),
(2356, 'Silveirânia', 11),
(5287, 'Silveiras', 26),
(273, 'Silves', 3),
(2357, 'Silvianópolis', 11),
(5420, 'Simão Dias', 25),
(2358, 'Simão Pereira', 11),
(3576, 'Simões', 17),
(657, 'Simões Filho', 5),
(1109, 'Simolândia', 9),
(2359, 'Simonésia', 11),
(3577, 'Simplício Mendes', 17),
(4283, 'Sinimbu', 23),
(1473, 'Sinop', 13),
(3151, 'Siqueira Campos', 18),
(3344, 'Sirinhaém', 16),
(5421, 'Siriri', 25),
(1110, 'Sítio d`Abadia', 9),
(658, 'Sítio do Mato', 5),
(659, 'Sítio do Quinto', 5),
(1327, 'Sítio Novo', 10),
(3832, 'Sítio Novo', 20),
(5554, 'Sítio Novo do Tocantins', 27),
(660, 'Sobradinho', 5),
(4284, 'Sobradinho', 23),
(2767, 'Sobrado', 15),
(864, 'Sobral', 6),
(2360, 'Sobrália', 11),
(5288, 'Socorro', 26),
(3578, 'Socorro do Piauí', 17),
(2768, 'Solânea', 15),
(2769, 'Soledade', 15),
(4285, 'Soledade', 23),
(2361, 'Soledade de Minas', 11),
(3345, 'Solidão', 16),
(865, 'Solonópole', 6),
(4674, 'Sombrio', 24),
(1559, 'Sonora', 12),
(71, 'Sooretama', 8),
(5289, 'Sorocaba', 26),
(1474, 'Sorriso', 13),
(2770, 'Sossêgo', 15),
(2546, 'Soure', 14),
(2771, 'Sousa', 15),
(661, 'Souto Soares', 5),
(5555, 'Sucupira', 27),
(1328, 'Sucupira do Norte', 10),
(1329, 'Sucupira do Riachão', 10),
(5290, 'Sud Mennucci', 26),
(4675, 'Sul Brasil', 24),
(3152, 'Sulina', 18),
(5291, 'Sumaré', 26),
(2772, 'Sumé', 15),
(3674, 'Sumidouro', 19),
(3346, 'Surubim', 16),
(3579, 'Sussuapara', 17),
(5292, 'Suzanápolis', 26),
(5293, 'Suzano', 26),
(4286, 'Tabaí', 23),
(1475, 'Tabaporã', 13),
(5294, 'Tabapuã', 26),
(274, 'Tabatinga', 3),
(5295, 'Tabatinga', 26),
(3347, 'Tabira', 16),
(5296, 'Taboão da Serra', 26),
(662, 'Tabocas do Brejo Velho', 5),
(3833, 'Taboleiro Grande', 20),
(2362, 'Tabuleiro', 11),
(866, 'Tabuleiro do Norte', 6),
(3348, 'Tacaimbó', 16),
(3349, 'Tacaratu', 16),
(5297, 'Taciba', 26),
(1560, 'Tacuru', 12),
(5298, 'Taguaí', 26),
(5556, 'Taguatinga', 27),
(5299, 'Taiaçu', 26),
(2547, 'Tailândia', 14),
(4676, 'Taió', 24),
(2363, 'Taiobeiras', 11),
(5557, 'Taipas do Tocantins', 27),
(3834, 'Taipu', 20),
(5300, 'Taiúva', 26),
(5558, 'Talismã', 27),
(3350, 'Tamandaré', 16),
(3153, 'Tamarana', 18),
(5301, 'Tambaú', 26),
(3154, 'Tamboara', 18),
(867, 'Tamboril', 6),
(3580, 'Tamboril do Piauí', 17),
(5302, 'Tanabi', 26),
(3835, 'Tangará', 20),
(4677, 'Tangará', 24),
(1476, 'Tangará da Serra', 13),
(3675, 'Tanguá', 19),
(663, 'Tanhaçu', 5),
(197, 'Tanque d`Arca', 2),
(3581, 'Tanque do Piauí', 17),
(664, 'Tanque Novo', 5),
(665, 'Tanquinho', 5),
(2364, 'Taparuba', 11),
(275, 'Tapauá', 3),
(3155, 'Tapejara', 18),
(4287, 'Tapejara', 23),
(4288, 'Tapera', 23),
(666, 'Taperoá', 5),
(2773, 'Taperoá', 15),
(4289, 'Tapes', 23),
(2365, 'Tapira', 11),
(3156, 'Tapira', 18),
(2366, 'Tapiraí', 11),
(5303, 'Tapiraí', 26),
(667, 'Tapiramutá', 5),
(5304, 'Tapiratiba', 26),
(1477, 'Tapurah', 13),
(4290, 'Taquara', 23),
(2367, 'Taquaraçu de Minas', 11),
(5305, 'Taquaral', 26),
(1111, 'Taquaral de Goiás', 9),
(198, 'Taquarana', 2),
(4291, 'Taquari', 23),
(5306, 'Taquaritinga', 26),
(3351, 'Taquaritinga do Norte', 16),
(5307, 'Taquarituba', 26),
(5308, 'Taquarivaí', 26),
(4292, 'Taquaruçu do Sul', 23),
(1561, 'Taquarussu', 12),
(5309, 'Tarabai', 26),
(99, 'Tarauacá', 1),
(868, 'Tarrafas', 6),
(217, 'Tartarugalzinho', 4),
(5310, 'Tarumã', 26),
(2368, 'Tarumirim', 11),
(1330, 'Tasso Fragoso', 10),
(5311, 'Tatuí', 26),
(869, 'Tauá', 6),
(5312, 'Taubaté', 26),
(2774, 'Tavares', 15),
(4293, 'Tavares', 23),
(276, 'Tefé', 3),
(2775, 'Teixeira', 15),
(668, 'Teixeira de Freitas', 5),
(3157, 'Teixeira Soares', 18),
(2369, 'Teixeiras', 11),
(4392, 'Teixeirópolis', 21),
(870, 'Tejuçuoca', 6),
(5313, 'Tejupá', 26),
(3158, 'Telêmaco Borba', 18),
(5422, 'Telha', 25),
(3836, 'Tenente Ananias', 20),
(3837, 'Tenente Laurentino Cruz', 20),
(4294, 'Tenente Portela', 23),
(2776, 'Tenório', 15),
(669, 'Teodoro Sampaio', 5),
(5314, 'Teodoro Sampaio', 26),
(670, 'Teofilândia', 5),
(2370, 'Teófilo Otoni', 11),
(671, 'Teolândia', 5),
(199, 'Teotônio Vilela', 2),
(1562, 'Terenos', 12),
(3582, 'Teresina', 17),
(1112, 'Teresina de Goiás', 9),
(3676, 'Teresópolis', 19),
(3352, 'Terezinha', 16),
(1113, 'Terezópolis de Goiás', 9),
(2548, 'Terra Alta', 14),
(3159, 'Terra Boa', 18),
(4295, 'Terra de Areia', 23),
(672, 'Terra Nova', 5),
(3353, 'Terra Nova', 16),
(1478, 'Terra Nova do Norte', 13),
(3160, 'Terra Rica', 18),
(3161, 'Terra Roxa', 18),
(5315, 'Terra Roxa', 26),
(2549, 'Terra Santa', 14),
(1479, 'Tesouro', 13),
(4296, 'Teutônia', 23),
(4393, 'Theobroma', 21),
(871, 'Tianguá', 6),
(3162, 'Tibagi', 18),
(3838, 'Tibau', 20),
(3839, 'Tibau do Sul', 20),
(5316, 'Tietê', 26),
(4678, 'Tigrinhos', 24),
(4679, 'Tijucas', 24),
(3163, 'Tijucas do Sul', 18),
(3354, 'Timbaúba', 16),
(3840, 'Timbaúba dos Batistas', 20),
(4680, 'Timbé do Sul', 24),
(1331, 'Timbiras', 10),
(4681, 'Timbó', 24),
(4682, 'Timbó Grande', 24),
(5317, 'Timburi', 26),
(1332, 'Timon', 10),
(2371, 'Timóteo', 11),
(4297, 'Tio Hugo', 23),
(2372, 'Tiradentes', 11),
(4298, 'Tiradentes do Sul', 23),
(2373, 'Tiros', 11),
(5423, 'Tobias Barreto', 25),
(5559, 'Tocantínia', 27),
(5560, 'Tocantinópolis', 27),
(2374, 'Tocantins', 11),
(2375, 'Tocos do Moji', 11),
(2376, 'Toledo', 11),
(3164, 'Toledo', 18),
(5424, 'Tomar do Geru', 25),
(3165, 'Tomazina', 18),
(2377, 'Tombos', 11),
(2550, 'Tomé-Açu', 14),
(277, 'Tonantins', 3),
(3355, 'Toritama', 16),
(1480, 'Torixoréu', 13),
(4299, 'Toropi', 23),
(5318, 'Torre de Pedra', 26),
(4300, 'Torres', 23),
(5319, 'Torrinha', 26),
(3841, 'Touros', 20),
(5320, 'Trabiju', 26),
(2551, 'Tracuateua', 14),
(3356, 'Tracunhaém', 16),
(200, 'Traipu', 2),
(2552, 'Trairão', 14),
(872, 'Trairi', 6),
(3677, 'Trajano de Morais', 19),
(4301, 'Tramandaí', 23),
(4302, 'Travesseiro', 23),
(673, 'Tremedal', 5),
(5321, 'Tremembé', 26),
(4303, 'Três Arroios', 23),
(4683, 'Três Barras', 24),
(3166, 'Três Barras do Paraná', 18),
(4304, 'Três Cachoeiras', 23),
(2378, 'Três Corações', 11),
(4305, 'Três Coroas', 23),
(4306, 'Três de Maio', 23),
(4307, 'Três Forquilhas', 23),
(5322, 'Três Fronteiras', 26),
(1563, 'Três Lagoas', 12),
(2379, 'Três Marias', 11),
(4308, 'Três Palmeiras', 23),
(4309, 'Três Passos', 23),
(2380, 'Três Pontas', 11),
(1114, 'Três Ranchos', 9),
(3678, 'Três Rios', 19),
(4684, 'Treviso', 24),
(4685, 'Treze de Maio', 24),
(4686, 'Treze Tílias', 24),
(1115, 'Trindade', 9),
(3357, 'Trindade', 16),
(4310, 'Trindade do Sul', 23),
(2777, 'Triunfo', 15),
(3358, 'Triunfo', 16),
(4311, 'Triunfo', 23),
(3842, 'Triunfo Potiguar', 20),
(1333, 'Trizidela do Vale', 10),
(1116, 'Trombas', 9),
(4687, 'Trombudo Central', 24),
(4688, 'Tubarão', 24),
(674, 'Tucano', 5),
(2553, 'Tucumã', 14),
(4312, 'Tucunduva', 23),
(2554, 'Tucuruí', 14),
(1334, 'Tufilândia', 10),
(5323, 'Tuiuti', 26),
(2381, 'Tumiritinga', 11),
(4689, 'Tunápolis', 24),
(4313, 'Tunas', 23),
(3167, 'Tunas do Paraná', 18),
(3168, 'Tuneiras do Oeste', 18),
(1335, 'Tuntum', 10),
(5324, 'Tupã', 26),
(2382, 'Tupaciguara', 11),
(3359, 'Tupanatinga', 16),
(4314, 'Tupanci do Sul', 23),
(4315, 'Tupanciretã', 23),
(4316, 'Tupandi', 23),
(4317, 'Tuparendi', 23),
(3360, 'Tuparetama', 16),
(3169, 'Tupãssi', 18),
(5325, 'Tupi Paulista', 26),
(5561, 'Tupirama', 27),
(5562, 'Tupiratins', 27),
(1336, 'Turiaçu', 10),
(1337, 'Turilândia', 10),
(5326, 'Turiúba', 26),
(2383, 'Turmalina', 11),
(5327, 'Turmalina', 26),
(4318, 'Turuçu', 23),
(873, 'Tururu', 6),
(1117, 'Turvânia', 9),
(1118, 'Turvelândia', 9),
(3170, 'Turvo', 18),
(4690, 'Turvo', 24),
(2384, 'Turvolândia', 11),
(1338, 'Tutóia', 10),
(278, 'Uarini', 3),
(675, 'Uauá', 5),
(2385, 'Ubá', 11),
(2386, 'Ubaí', 11),
(676, 'Ubaíra', 5),
(677, 'Ubaitaba', 5),
(874, 'Ubajara', 6),
(2387, 'Ubaporanga', 11),
(5328, 'Ubarana', 26),
(678, 'Ubatã', 5),
(5329, 'Ubatuba', 26),
(2388, 'Uberaba', 11),
(2389, 'Uberlândia', 11),
(5330, 'Ubirajara', 26),
(3171, 'Ubiratã', 18),
(4319, 'Ubiretama', 23),
(5331, 'Uchoa', 26),
(679, 'Uibaí', 5),
(4412, 'Uiramutã', 22),
(1119, 'Uirapuru', 9),
(2778, 'Uiraúna', 15),
(2555, 'Ulianópolis', 14),
(875, 'Umari', 6),
(3843, 'Umarizal', 20),
(5425, 'Umbaúba', 25),
(680, 'Umburanas', 5),
(2390, 'Umburatiba', 11),
(2779, 'Umbuzeiro', 15),
(876, 'Umirim', 6),
(3172, 'Umuarama', 18),
(681, 'Una', 5),
(2391, 'Unaí', 11),
(3583, 'União', 17),
(4320, 'União da Serra', 23),
(3173, 'União da Vitória', 18),
(2392, 'União de Minas', 11),
(4691, 'União do Oeste', 24),
(1481, 'União do Sul', 13),
(201, 'União dos Palmares', 2),
(5332, 'União Paulista', 26),
(3174, 'Uniflor', 18),
(4321, 'Unistalda', 23),
(3844, 'Upanema', 20),
(3175, 'Uraí', 18),
(682, 'Urandi', 5),
(5333, 'Urânia', 26),
(1339, 'Urbano Santos', 10),
(5334, 'Uru', 26),
(1120, 'Uruaçu', 9),
(1121, 'Uruana', 9),
(2393, 'Uruana de Minas', 11),
(2556, 'Uruará', 14),
(4692, 'Urubici', 24),
(877, 'Uruburetama', 6),
(2394, 'Urucânia', 11),
(279, 'Urucará', 3),
(683, 'Uruçuca', 5),
(3584, 'Uruçuí', 17),
(2395, 'Urucuia', 11),
(280, 'Urucurituba', 3),
(4322, 'Uruguaiana', 23),
(878, 'Uruoca', 6),
(4394, 'Urupá', 21),
(4693, 'Urupema', 24),
(5335, 'Urupês', 26),
(4694, 'Urussanga', 24),
(1122, 'Urutaí', 9),
(684, 'Utinga', 5),
(4323, 'Vacaria', 23),
(1482, 'Vale de São Domingos', 13),
(4395, 'Vale do Anari', 21),
(4396, 'Vale do Paraíso', 21),
(4324, 'Vale do Sol', 23),
(4325, 'Vale Real', 23),
(4326, 'Vale Verde', 23),
(685, 'Valença', 5),
(3679, 'Valença', 19),
(3585, 'Valença do Piauí', 17),
(686, 'Valente', 5),
(5336, 'Valentim Gentil', 26),
(5337, 'Valinhos', 26),
(5338, 'Valparaíso', 26),
(1123, 'Valparaíso de Goiás', 9),
(4327, 'Vanini', 23),
(4695, 'Vargeão', 24),
(4696, 'Vargem', 24),
(5339, 'Vargem', 26),
(2396, 'Vargem Alegre', 11),
(72, 'Vargem Alta', 8),
(2397, 'Vargem Bonita', 11),
(4697, 'Vargem Bonita', 24),
(1340, 'Vargem Grande', 10),
(2398, 'Vargem Grande do Rio Pardo', 11),
(5340, 'Vargem Grande do Sul', 26),
(5341, 'Vargem Grande Paulista', 26),
(2399, 'Varginha', 11),
(1124, 'Varjão', 9),
(2400, 'Varjão de Minas', 11),
(879, 'Varjota', 6),
(3680, 'Varre-Sai', 19),
(2780, 'Várzea', 15),
(3845, 'Várzea', 20),
(880, 'Várzea Alegre', 6),
(3586, 'Várzea Branca', 17),
(2401, 'Várzea da Palma', 11),
(687, 'Várzea da Roça', 5),
(688, 'Várzea do Poço', 5),
(1483, 'Várzea Grande', 13),
(3587, 'Várzea Grande', 17),
(689, 'Várzea Nova', 5),
(5342, 'Várzea Paulista', 26),
(690, 'Varzedo', 5),
(2402, 'Varzelândia', 11),
(3681, 'Vassouras', 19),
(2403, 'Vazante', 11),
(4328, 'Venâncio Aires', 23),
(73, 'Venda Nova do Imigrante', 8),
(3846, 'Venha-Ver', 20),
(3176, 'Ventania', 18),
(3361, 'Venturosa', 16),
(1484, 'Vera', 13),
(691, 'Vera Cruz', 5),
(3847, 'Vera Cruz', 20),
(4329, 'Vera Cruz', 23),
(5343, 'Vera Cruz', 26),
(3177, 'Vera Cruz do Oeste', 18),
(3588, 'Vera Mendes', 17),
(4330, 'Veranópolis', 23),
(3362, 'Verdejante', 16),
(2404, 'Verdelândia', 11),
(3178, 'Verê', 18),
(692, 'Vereda', 5),
(2405, 'Veredinha', 11),
(2406, 'Veríssimo', 11),
(2407, 'Vermelho Novo', 11),
(3363, 'Vertente do Lério', 16),
(3364, 'Vertentes', 16),
(2408, 'Vespasiano', 11),
(4331, 'Vespasiano Correa', 23),
(4332, 'Viadutos', 23),
(4333, 'Viamão', 23),
(74, 'Viana', 8),
(1341, 'Viana', 10),
(1125, 'Vianópolis', 9),
(3365, 'Vicência', 16),
(4334, 'Vicente Dutra', 23),
(1564, 'Vicentina', 12),
(1126, 'Vicentinópolis', 9),
(202, 'Viçosa', 2),
(2409, 'Viçosa', 11),
(3848, 'Viçosa', 20),
(881, 'Viçosa do Ceará', 6),
(4335, 'Victor Graeff', 23),
(4698, 'Vidal Ramos', 24),
(4699, 'Videira', 24),
(2410, 'Vieiras', 11),
(2781, 'Vieirópolis', 15),
(2557, 'Vigia', 14),
(1485, 'Vila Bela da Santíssima Trindade', 13),
(1127, 'Vila Boa', 9),
(3849, 'Vila Flor', 20),
(4336, 'Vila Flores', 23),
(4337, 'Vila Lângaro', 23),
(4338, 'Vila Maria', 23),
(3589, 'Vila Nova do Piauí', 17),
(4339, 'Vila Nova do Sul', 23),
(1342, 'Vila Nova dos Martírios', 10),
(75, 'Vila Pavão', 8),
(1128, 'Vila Propício', 9),
(1486, 'Vila Rica', 13),
(76, 'Vila Valério', 8),
(77, 'Vila Velha', 8),
(4397, 'Vilhena', 21),
(5344, 'Vinhedo', 26),
(5345, 'Viradouro', 26),
(2411, 'Virgem da Lapa', 11),
(2412, 'Virgínia', 11),
(2413, 'Virginópolis', 11),
(2414, 'Virgolândia', 11),
(3179, 'Virmond', 18),
(2415, 'Visconde do Rio Branco', 11),
(2558, 'Viseu', 14),
(4340, 'Vista Alegre', 23),
(5346, 'Vista Alegre do Alto', 26),
(4341, 'Vista Alegre do Prata', 23),
(4342, 'Vista Gaúcha', 23),
(2782, 'Vista Serrana', 15),
(4700, 'Vitor Meireles', 24),
(78, 'Vitória', 8),
(5347, 'Vitória Brasil', 26),
(693, 'Vitória da Conquista', 5),
(4343, 'Vitória das Missões', 23),
(3366, 'Vitória de Santo Antão', 16),
(218, 'Vitória do Jari', 4),
(1343, 'Vitória do Mearim', 10),
(2559, 'Vitória do Xingu', 14),
(3180, 'Vitorino', 18),
(1344, 'Vitorino Freire', 10),
(2416, 'Volta Grande', 11),
(3682, 'Volta Redonda', 19),
(5348, 'Votorantim', 26),
(5349, 'Votuporanga', 26),
(694, 'Wagner', 5),
(3590, 'Wall Ferraz', 17),
(5563, 'Wanderlândia', 27),
(695, 'Wanderley', 5),
(2417, 'Wenceslau Braz', 11),
(3181, 'Wenceslau Braz', 18),
(696, 'Wenceslau Guimarães', 5),
(4344, 'Westfália', 23),
(4701, 'Witmarsum', 24),
(5564, 'Xambioá', 27),
(3182, 'Xambrê', 18),
(4345, 'Xangri-lá', 23),
(4702, 'Xanxerê', 24),
(100, 'Xapuri', 1),
(4703, 'Xavantina', 24),
(4704, 'Xaxim', 24),
(3367, 'Xexéu', 16),
(2560, 'Xinguara', 14),
(697, 'Xique-Xique', 5),
(2783, 'Zabelê', 15),
(5350, 'Zacarias', 26),
(1345, 'Zé Doca', 10),
(4705, 'Zortéa', 24);

-- --------------------------------------------------------

--
-- Estrutura para tabela `cliente`
--

CREATE TABLE `cliente` (
  `id` int(10) UNSIGNED NOT NULL,
  `nome` varchar(150) NOT NULL,
  `data_nascimento` date DEFAULT NULL,
  `apelido` varchar(50) DEFAULT NULL,
  `rg` varchar(20) DEFAULT NULL,
  `orgao_emissor` varchar(10) DEFAULT NULL,
  `cpf` varchar(11) DEFAULT NULL,
  `conjugue` varchar(150) DEFAULT NULL,
  `nome_pai` varchar(150) DEFAULT NULL,
  `nome_mae` varchar(150) DEFAULT NULL,
  `endereco` varchar(100) NOT NULL,
  `numero` varchar(10) NOT NULL,
  `bairro` varchar(100) NOT NULL,
  `referencia` varchar(100) DEFAULT NULL,
  `casa_propria` tinyint(1) DEFAULT NULL,
  `tempo_casa_propria` varchar(10) DEFAULT NULL,
  `observacao` text DEFAULT NULL,
  `renda_mensal` float DEFAULT NULL,
  `id_localidade` smallint(5) UNSIGNED NOT NULL,
  `bloqueado` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `cliente_telefone`
--

CREATE TABLE `cliente_telefone` (
  `telefone` varchar(12) NOT NULL,
  `id_cliente` int(10) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `consulta`
--

CREATE TABLE `consulta` (
  `id` int(10) UNSIGNED NOT NULL,
  `id_venda` int(10) UNSIGNED NOT NULL,
  `nome_paciente` varchar(100) NOT NULL,
  `esferico_od` float NOT NULL,
  `esferico_oe` float NOT NULL,
  `cilindrico_od` float NOT NULL,
  `cilindrico_eq` float NOT NULL,
  `eixo_od` float NOT NULL,
  `eixo_oe` float NOT NULL,
  `dnp_od` float NOT NULL,
  `dnp_oe` float NOT NULL,
  `dp` float DEFAULT NULL,
  `adicao` float DEFAULT NULL,
  `altura` float DEFAULT NULL,
  `co` float DEFAULT NULL,
  `cor` varchar(50) DEFAULT NULL,
  `lente` varchar(50) DEFAULT NULL,
  `observacao` text DEFAULT NULL,
  `id_funcionario_oculista` smallint(5) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `dados_boleto_loja`
--

CREATE TABLE `dados_boleto_loja` (
  `id_loja` tinyint(3) UNSIGNED NOT NULL,
  `agencia` varchar(8) NOT NULL,
  `conta` varchar(8) NOT NULL,
  `conta_digito` varchar(1) NOT NULL,
  `conta_cedente` varchar(8) NOT NULL,
  `conta_cedente_digito` varchar(1) NOT NULL,
  `carteira` varchar(8) NOT NULL,
  `padrao` tinyint(1) NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `despesa_caixa`
--

CREATE TABLE `despesa_caixa` (
  `id` int(11) UNSIGNED NOT NULL,
  `valor` float NOT NULL DEFAULT 0,
  `id_natureza` int(10) UNSIGNED NOT NULL,
  `id_entidade` int(11) NOT NULL,
  `observacao` text DEFAULT NULL,
  `id_caixa` int(10) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `despesa_combustivel`
--

CREATE TABLE `despesa_combustivel` (
  `id` int(10) UNSIGNED NOT NULL,
  `litros` float NOT NULL,
  `preco` float NOT NULL,
  `km_inicial` float NOT NULL,
  `km_final` float NOT NULL,
  `id_despesa` int(10) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `equipe_venda`
--

CREATE TABLE `equipe_venda` (
  `id` int(10) UNSIGNED NOT NULL,
  `nome` varchar(128) NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL,
  `id_funcionario_lider` smallint(5) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `equipe_venda_funcionario`
--

CREATE TABLE `equipe_venda_funcionario` (
  `id_equipe` int(10) UNSIGNED NOT NULL,
  `id_funcionario` smallint(5) UNSIGNED NOT NULL,
  `data_entrada` date NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `funcionario`
--

CREATE TABLE `funcionario` (
  `id` smallint(5) UNSIGNED NOT NULL,
  `nome_funcionario` varchar(255) NOT NULL,
  `login_sistema` varchar(20) DEFAULT NULL,
  `cidade` smallint(5) UNSIGNED DEFAULT NULL,
  `senha` varchar(100) DEFAULT NULL,
  `data_nascimento` date DEFAULT NULL,
  `data_admissao` date DEFAULT NULL,
  `data_demissao` date DEFAULT NULL,
  `rua` varchar(100) DEFAULT NULL,
  `numero` varchar(10) DEFAULT NULL,
  `bairro` varchar(100) DEFAULT NULL,
  `cep` varchar(8) DEFAULT NULL,
  `cpf` varchar(11) NOT NULL,
  `rg` varchar(20) NOT NULL,
  `cpt` varchar(100) DEFAULT NULL,
  `referencia` varchar(100) DEFAULT NULL,
  `banco` varchar(100) DEFAULT NULL,
  `agencia` varchar(100) DEFAULT NULL,
  `conta` varchar(100) DEFAULT NULL,
  `id_funcionario_cargo` tinyint(3) UNSIGNED NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL,
  `id_perfil_sistema` tinyint(3) UNSIGNED DEFAULT NULL,
  `email` varchar(100) DEFAULT NULL,
  `status` tinyint(1) DEFAULT 1,
  `inativo_para_caixa` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `funcionario`
--

INSERT INTO `funcionario` (`id`, `nome_funcionario`, `login_sistema`, `cidade`, `senha`, `data_nascimento`, `data_admissao`, `data_demissao`, `rua`, `numero`, `bairro`, `cep`, `cpf`, `rg`, `cpt`, `referencia`, `banco`, `agencia`, `conta`, `id_funcionario_cargo`, `id_loja`, `id_perfil_sistema`, `email`, `status`, `inativo_para_caixa`) VALUES
(5, 'JOAO PAUL G', 'JOAO', 59, '9a286406c252a3d14218228974e1f567', '1979-12-09', '2013-09-02', NULL, 'RIO NEGRO, BL-C2, APTO 912', '98', 'TABAPUA', '', '91690617349', '331115898', '', '', 'CAIXA', '0748-013', '567760', 4, 5, 2, 'VALBERITAPIPOCA2013@GMAIL.COM', 0, 0),
(6, 'EDSON RAFAEL FURTADO RODRIGUES', 'RAFAEL', 88, 'e10adc3949ba59abbe56e057f20f883e', '1979-10-02', '2009-07-01', NULL, 'ANA LUIZA TABOSA BRAGA', '1588', 'MADALENAS', '62500000', '84829427353', '20078740732', '9999999', '', '', '', '', 5, 10, 1, 'EDSONRODRIGUES_ITA@YAHOO.COM.BR', 1, 0),
(43, 'JOAO PAULO ALVES AUGUSTO', 'JOAOPAULO', 88, '71f909349719be125db63cef657bd106', '1985-08-09', '2013-11-25', NULL, 'JOSE MARIA DA SILVEIRA', '1335', 'MARANHAO', '62500000', '01901421384', '2003098045151', '', 'NA RUA DA IGREJA ELUZAI', 'CAIXA', '0748-013', '45545-4', 5, 4, 1, 'JOAOAUGUSTO1985@GMAIL.COM', 1, 0),
(241, 'ELIAS PEREIRA DE ALMEIDA', 'ELIASALMEIDA', 184, '827ccb0eea8a706c4c34a16891f84e7b', '2001-05-31', '2024-02-01', NULL, 'RUA POVOADO', 'oo1', 'INTERIOR', '62300000', '63300565367', '20180014808', '00000011', 'ENTRE VICOSA E TIANGUA', '', '', '', 4, 10, 4, 'ANGELAFARIAS2005@YAHOO.COM.BR', 1, 0),
(242, 'FREDSON MORAIS ARRUDA', NULL, 184, 'd41d8cd98f00b204e9800998ecf8427e', '2004-07-13', NULL, NULL, 'RUA MANHOSO', 's/n', 'MANHOSO', '62300000', '09301525305', '20192210526', '', 'FILHO DO HOMEM MAIS RICO DO MANHOSO', '', '', '', 2, 10, NULL, 'EDSONRODRIGUESOPTO@GMAIL.COM', 1, 0),
(244, 'FRANCISCO GERLANDO PEREIRA FELIX ARAUJO', NULL, 184, 'd41d8cd98f00b204e9800998ecf8427e', '1995-04-21', NULL, NULL, 'RUA MANHOSO', 's/n', 'MANHOSO', '62300000', '05949760301', '20075913075', '', 'O MAIS VALENTE DO MANHOSO', '', '', '', 6, 10, NULL, 'GERLANDOFELIX@GMAIL.COM', 1, 0),
(245, 'FRANCILDA RODRIGUES DE MOURA', 'FRANCILDA', 184, 'c33367701511b4f6020ec61ded352059', '1993-08-21', '2024-02-01', NULL, 'RUA  SITIO VAMBIRA', 's/n', 'SITIO VAMBIRA', '62300000', '05412211321', '2009098053293', '', 'POVOADO ENTRE VICOSA E TIANGUA', '', '', '', 3, 10, 2, 'FRANCILDAMOURA372@GMAIL.COM', 1, 0);

-- --------------------------------------------------------

--
-- Estrutura para tabela `funcionario_cargo`
--

CREATE TABLE `funcionario_cargo` (
  `id` tinyint(3) UNSIGNED NOT NULL,
  `nome_cargo` varchar(45) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `funcionario_cargo`
--

INSERT INTO `funcionario_cargo` (`id`, `nome_cargo`) VALUES
(6, 'Agente Vendas'),
(3, 'Auxiliar Escritório'),
(2, 'Cobrador'),
(5, 'Diretor'),
(4, 'Gerente'),
(7, 'Lider de Equipe'),
(8, 'Optometrista'),
(1, 'Vendedor');

-- --------------------------------------------------------

--
-- Estrutura para tabela `funcionario_telefone`
--

CREATE TABLE `funcionario_telefone` (
  `id_funcionario` smallint(5) UNSIGNED NOT NULL,
  `telefone` varchar(12) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `funcionario_telefone`
--

INSERT INTO `funcionario_telefone` (`id_funcionario`, `telefone`) VALUES
(245, '88992993701'),
(241, '88993458409'),
(242, '88994599379'),
(244, '88999892402');

-- --------------------------------------------------------

--
-- Estrutura para tabela `laboratorio`
--

CREATE TABLE `laboratorio` (
  `id` tinyint(3) UNSIGNED NOT NULL,
  `nome` varchar(100) NOT NULL,
  `telefone` varchar(100) DEFAULT NULL,
  `cnpj` varchar(20) DEFAULT NULL,
  `principal` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `localidade`
--

CREATE TABLE `localidade` (
  `id` smallint(5) UNSIGNED NOT NULL,
  `nome_localidade` varchar(100) NOT NULL,
  `id_cidade` smallint(5) UNSIGNED NOT NULL,
  `seq_rota` smallint(5) UNSIGNED DEFAULT NULL,
  `id_rota` tinyint(3) UNSIGNED DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `loja`
--

CREATE TABLE `loja` (
  `id` tinyint(3) UNSIGNED NOT NULL,
  `sigla_loja` varchar(4) NOT NULL,
  `rua` varchar(100) NOT NULL,
  `numero` varchar(10) DEFAULT NULL,
  `bairro` varchar(100) NOT NULL,
  `cep` varchar(8) NOT NULL,
  `cnpj` varchar(20) NOT NULL,
  `id_cidade` smallint(5) UNSIGNED NOT NULL,
  `cgc` varchar(20) NOT NULL,
  `id_funcionario_gerente` smallint(5) UNSIGNED DEFAULT NULL,
  `inativo_para_caixa` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `loja`
--

INSERT INTO `loja` (`id`, `sigla_loja`, `rua`, `numero`, `bairro`, `cep`, `cnpj`, `id_cidade`, `cgc`, `id_funcionario_gerente`, `inativo_para_caixa`) VALUES
(1, 'Loja', '', NULL, '', '', '', 1, '', 6, 0),
(10, 'MAIS', 'RUA POVOADO', '001', 'INTERIOR', '62300000', '12519825000195', 184, '0000111122', 241, 0),
(11, 'ONGS', 'RUA CHICO VIRIATO', '854', 'AREA NOBRE', '62507055', '32958725000183', 88, '0000111122', 241, 0);

-- --------------------------------------------------------

--
-- Estrutura para tabela `loja_produto`
--

CREATE TABLE `loja_produto` (
  `id` int(11) NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL,
  `id_produto` int(10) UNSIGNED NOT NULL,
  `quantidade` int(10) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `loja_telefone`
--

CREATE TABLE `loja_telefone` (
  `telefone` varchar(12) NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `loja_telefone`
--

INSERT INTO `loja_telefone` (`telefone`, `id_loja`) VALUES
('88994358409', 10),
('88994361991', 11);

-- --------------------------------------------------------

--
-- Estrutura para tabela `marca_produto`
--

CREATE TABLE `marca_produto` (
  `id_marca_produto` tinyint(3) UNSIGNED NOT NULL,
  `nome_marca_produto` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `marca_produto`
--

INSERT INTO `marca_produto` (`id_marca_produto`, `nome_marca_produto`) VALUES
(15, 'BIFOCAL'),
(12, 'DVIP'),
(13, 'MULTIFOCAL'),
(16, 'TESTE'),
(14, 'VISAO SIMPLES');

-- --------------------------------------------------------

--
-- Estrutura para tabela `natureza_despesa`
--

CREATE TABLE `natureza_despesa` (
  `id` int(10) UNSIGNED NOT NULL,
  `nome` varchar(128) NOT NULL,
  `tipo` smallint(6) NOT NULL,
  `entrada` tinyint(1) NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `natureza_despesa`
--

INSERT INTO `natureza_despesa` (`id`, `nome`, `tipo`, `entrada`) VALUES
(86, 'ALIMENTAÇAO', 3, 0),
(87, 'COMBUSTIVEL', 2, 0),
(88, 'ADIANTAMENTO', 1, 0),
(89, 'HOSPEDAGEM', 3, 0);

-- --------------------------------------------------------

--
-- Estrutura para tabela `ordem_servico`
--

CREATE TABLE `ordem_servico` (
  `id` int(10) UNSIGNED NOT NULL,
  `numero` varchar(15) NOT NULL,
  `data_envio_lab` date NOT NULL,
  `data_recebimento_lab` date DEFAULT NULL,
  `armacao_loja` tinyint(1) NOT NULL DEFAULT 0,
  `valor` float DEFAULT NULL,
  `id_laboratorio` tinyint(3) UNSIGNED NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL,
  `cancelada` tinyint(1) DEFAULT 0,
  `func_autor` smallint(5) UNSIGNED DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `ordem_servico_repasse`
--

CREATE TABLE `ordem_servico_repasse` (
  `id` int(11) UNSIGNED NOT NULL,
  `dt_chegada` date NOT NULL,
  `dt_envio_conserto` date DEFAULT NULL,
  `dt_recebimento_conserto` date DEFAULT NULL,
  `dt_envio_cliente` date DEFAULT NULL,
  `observacao` text DEFAULT NULL,
  `id_funcionario_cobrador` smallint(5) UNSIGNED NOT NULL,
  `id_venda` int(10) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `perfil_sistema`
--

CREATE TABLE `perfil_sistema` (
  `id` tinyint(3) UNSIGNED NOT NULL,
  `nome_perfil` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `perfil_sistema`
--

INSERT INTO `perfil_sistema` (`id`, `nome_perfil`) VALUES
(1, 'Administrador'),
(2, 'Operador'),
(3, 'Vendedor'),
(4, 'Gerente de Loja'),
(5, 'Controlador Estoque');

-- --------------------------------------------------------

--
-- Estrutura para tabela `prestacao_conta`
--

CREATE TABLE `prestacao_conta` (
  `id` int(10) UNSIGNED NOT NULL,
  `id_cobrador` smallint(5) UNSIGNED NOT NULL,
  `status` tinyint(1) NOT NULL DEFAULT 0,
  `dt_inicial_arrecadacao` date NOT NULL,
  `dt_final_arrecadacao` date NOT NULL,
  `seq` int(10) UNSIGNED NOT NULL DEFAULT 0,
  `cancelada` tinyint(1) DEFAULT 0,
  `auditada` tinyint(1) NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `prestacao_conta_item`
--

CREATE TABLE `prestacao_conta_item` (
  `id` int(10) UNSIGNED NOT NULL,
  `valor` float NOT NULL,
  `tipo_pagamento` smallint(6) UNSIGNED NOT NULL,
  `data` date DEFAULT NULL,
  `id_prest_conta` int(10) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `produto`
--

CREATE TABLE `produto` (
  `id` int(10) UNSIGNED NOT NULL,
  `cod_produto` int(11) NOT NULL,
  `descricao` varchar(100) NOT NULL,
  `preco_compra` float DEFAULT NULL,
  `preco_venda` float DEFAULT NULL,
  `preco_venda_min` float DEFAULT NULL,
  `id_tipo_produto` tinyint(3) UNSIGNED NOT NULL,
  `id_marca_produto` tinyint(3) UNSIGNED NOT NULL,
  `categoria` smallint(6) NOT NULL DEFAULT 0 COMMENT '0: produto (gen?rico), 1: lente'
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `produto`
--

INSERT INTO `produto` (`id`, `cod_produto`, `descricao`, `preco_compra`, `preco_venda`, `preco_venda_min`, `id_tipo_produto`, `id_marca_produto`, `categoria`) VALUES
(1000029, 1000, 'TESTE', 100, 200, 150, 31, 15, 0),
(1000030, 1001, 'LENTES 2', 0, 100, 150, 33, 12, 0),
(1000032, 1010, 'PARAFUSADA', 100, 500, 900, 33, 14, 0);

-- --------------------------------------------------------

--
-- Estrutura para tabela `produto_venda`
--

CREATE TABLE `produto_venda` (
  `id` int(10) UNSIGNED NOT NULL,
  `id_produto` int(10) UNSIGNED NOT NULL,
  `id_venda` int(10) UNSIGNED NOT NULL,
  `valor_venda_produto` float NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `regiao`
--

CREATE TABLE `regiao` (
  `id` smallint(5) UNSIGNED NOT NULL,
  `nome_regiao` varchar(100) CHARACTER SET latin1 COLLATE latin1_swedish_ci NOT NULL,
  `id_funcionario_cobrador` smallint(5) UNSIGNED NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `regiao`
--

INSERT INTO `regiao` (`id`, `nome_regiao`, `id_funcionario_cobrador`, `id_loja`) VALUES
(24, 'GRAJAU', 242, 10),
(25, 'LAGOA GRANDE', 242, 10);

-- --------------------------------------------------------

--
-- Estrutura para tabela `rota`
--

CREATE TABLE `rota` (
  `id` tinyint(3) UNSIGNED NOT NULL,
  `nome_rota` varchar(100) NOT NULL,
  `id_regiao` smallint(5) UNSIGNED NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `rota`
--

INSERT INTO `rota` (`id`, `nome_rota`, `id_regiao`) VALUES
(53, 'ROTA 01 LGM', 25),
(54, 'ROTA01', 24),
(55, 'ROTA01', 25);

-- --------------------------------------------------------

--
-- Estrutura para tabela `tipo_pagamento`
--

CREATE TABLE `tipo_pagamento` (
  `id` smallint(6) UNSIGNED NOT NULL,
  `nome` varchar(50) NOT NULL,
  `observacao` text DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `tipo_pagamento`
--

INSERT INTO `tipo_pagamento` (`id`, `nome`, `observacao`) VALUES
(8, 'PIX', ''),
(9, 'DEPOSITO', ''),
(10, 'LOJA', '');

-- --------------------------------------------------------

--
-- Estrutura para tabela `tipo_produto`
--

CREATE TABLE `tipo_produto` (
  `id` tinyint(3) UNSIGNED NOT NULL,
  `nome_tipo_produto` varchar(100) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `tipo_produto`
--

INSERT INTO `tipo_produto` (`id`, `nome_tipo_produto`) VALUES
(31, 'ACETATO'),
(34, 'ARM DE METAL'),
(33, 'LENTES'),
(32, 'NYLON');

-- --------------------------------------------------------

--
-- Estrutura para tabela `uf`
--

CREATE TABLE `uf` (
  `id_uf` tinyint(3) UNSIGNED NOT NULL,
  `sigla_uf` varchar(2) NOT NULL,
  `nome_uf` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `uf`
--

INSERT INTO `uf` (`id_uf`, `sigla_uf`, `nome_uf`) VALUES
(1, 'AC', 'Acre'),
(2, 'AL', 'Alagoas'),
(3, 'AM', 'Amazonas'),
(4, 'AP', 'Amapá'),
(5, 'BA', 'Bahia'),
(6, 'CE', 'Ceará'),
(7, 'DF', 'Distrito Federal'),
(8, 'ES', 'Espírito Santo'),
(9, 'GO', 'Goiás'),
(10, 'MA', 'Maranhão'),
(11, 'MG', 'Minas Gerais'),
(12, 'MS', 'Mato Grosso do Sul'),
(13, 'MT', 'Mato Grosso'),
(14, 'PA', 'Pará'),
(15, 'PB', 'Paraíba'),
(16, 'PE', 'Pernambuco'),
(17, 'PI', 'Piauí'),
(18, 'PR', 'Paraná'),
(19, 'RJ', 'Rio de Janeiro'),
(20, 'RN', 'Rio Grande do Norte'),
(21, 'RO', 'Rondônia'),
(22, 'RR', 'Roraima'),
(23, 'RS', 'Rio Grande do Sul'),
(24, 'SC', 'Santa Catarina'),
(25, 'SE', 'Sergipe'),
(26, 'SP', 'São Paulo'),
(27, 'TO', 'Tocantins');

-- --------------------------------------------------------

--
-- Estrutura para tabela `veiculo`
--

CREATE TABLE `veiculo` (
  `id` int(11) NOT NULL,
  `nome` varchar(128) NOT NULL,
  `placa` char(8) NOT NULL,
  `id_funcionario_motorista` smallint(5) UNSIGNED NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL,
  `inativo_para_caixa` tinyint(1) DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Despejando dados para a tabela `veiculo`
--

INSERT INTO `veiculo` (`id`, `nome`, `placa`, `id_funcionario_motorista`, `id_loja`, `inativo_para_caixa`) VALUES
(1, 'ONIX PRATA', 'HVN0056', 241, 10, 0);

-- --------------------------------------------------------

--
-- Estrutura para tabela `venda`
--

CREATE TABLE `venda` (
  `id` int(10) UNSIGNED NOT NULL,
  `data_venda` date NOT NULL,
  `data_previsao_entrega` date NOT NULL,
  `data_entrega` date DEFAULT NULL,
  `id_cliente` int(10) UNSIGNED NOT NULL,
  `id_loja` tinyint(3) UNSIGNED NOT NULL,
  `id_func_vendedor` smallint(5) UNSIGNED NOT NULL,
  `id_func_agente_vendas` smallint(5) UNSIGNED NOT NULL,
  `id_ordem_servico` int(10) UNSIGNED DEFAULT NULL,
  `status` smallint(5) DEFAULT 1,
  `venda_antiga` int(10) UNSIGNED DEFAULT NULL,
  `id_equipe` int(10) UNSIGNED DEFAULT NULL,
  `lider_equipe` smallint(5) UNSIGNED DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `venda_cancelamento`
--

CREATE TABLE `venda_cancelamento` (
  `venda_id` int(10) UNSIGNED NOT NULL,
  `funcionario_id_autor` smallint(5) UNSIGNED NOT NULL,
  `funcionario_id_autorizador` smallint(5) UNSIGNED NOT NULL,
  `data_cancelamento` date NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_general_ci;

-- --------------------------------------------------------

--
-- Estrutura para tabela `venda_parcela`
--

CREATE TABLE `venda_parcela` (
  `numero_parcela` tinyint(3) UNSIGNED NOT NULL,
  `data_parcela` date NOT NULL,
  `valor_parcela` float NOT NULL,
  `status_parcela` tinyint(1) DEFAULT 0,
  `id_venda` int(10) UNSIGNED NOT NULL,
  `data_remarcacao` date DEFAULT NULL,
  `por_boleto` int(11) NOT NULL DEFAULT 1,
  `cancelada` tinyint(1) NOT NULL DEFAULT 0
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb3 COLLATE=utf8mb3_unicode_ci;

--
-- Índices para tabelas despejadas
--

--
-- Índices de tabela `caixa_diario`
--
ALTER TABLE `caixa_diario`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `data` (`data`,`loja`),
  ADD UNIQUE KEY `data_2` (`data`,`loja`),
  ADD KEY `loja` (`loja`),
  ADD KEY `loja_2` (`loja`);

--
-- Índices de tabela `central_estoque`
--
ALTER TABLE `central_estoque`
  ADD PRIMARY KEY (`id`),
  ADD KEY `id_usuario_estoque` (`id_usuario_estoque`),
  ADD KEY `id_usuario_confirmacao` (`id_usuario_confirmacao_entrada`),
  ADD KEY `id_loja_origem` (`id_loja_origem`),
  ADD KEY `id_loja_destino` (`id_loja_destino`),
  ADD KEY `id_usuario_confirmacao_saida` (`id_usuario_confirmacao_saida`);

--
-- Índices de tabela `central_estoque_produto`
--
ALTER TABLE `central_estoque_produto`
  ADD KEY `id_produto` (`id_produto`),
  ADD KEY `id_central_estoque` (`id_central_estoque`);

--
-- Índices de tabela `cidade`
--
ALTER TABLE `cidade`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `nome_cidade_UNIQUE` (`nome_cidade`,`id_uf`),
  ADD KEY `fk_cidade_uf1_idx` (`id_uf`);

--
-- Índices de tabela `cliente`
--
ALTER TABLE `cliente`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `cpf_UNIQUE` (`cpf`),
  ADD KEY `fk_id_cidade_idx` (`id_localidade`);

--
-- Índices de tabela `cliente_telefone`
--
ALTER TABLE `cliente_telefone`
  ADD PRIMARY KEY (`telefone`,`id_cliente`),
  ADD KEY `fk_cliente_telefone_cliente1_idx` (`id_cliente`);

--
-- Índices de tabela `consulta`
--
ALTER TABLE `consulta`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_consulta_funcionario1_idx` (`id_funcionario_oculista`),
  ADD KEY `fk_consulta_venda1_idx` (`id_venda`);

--
-- Índices de tabela `dados_boleto_loja`
--
ALTER TABLE `dados_boleto_loja`
  ADD PRIMARY KEY (`id_loja`);

--
-- Índices de tabela `despesa_caixa`
--
ALTER TABLE `despesa_caixa`
  ADD PRIMARY KEY (`id`),
  ADD KEY `id_natureza` (`id_natureza`,`id_caixa`),
  ADD KEY `id_caixa` (`id_caixa`),
  ADD KEY `id_caixa_2` (`id_caixa`);

--
-- Índices de tabela `despesa_combustivel`
--
ALTER TABLE `despesa_combustivel`
  ADD PRIMARY KEY (`id`),
  ADD KEY `id_despesa` (`id_despesa`);

--
-- Índices de tabela `equipe_venda`
--
ALTER TABLE `equipe_venda`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_equipe_venda` (`id_funcionario_lider`),
  ADD KEY `id_funcionario_lider` (`id_funcionario_lider`),
  ADD KEY `id_loja` (`id_loja`);

--
-- Índices de tabela `equipe_venda_funcionario`
--
ALTER TABLE `equipe_venda_funcionario`
  ADD KEY `id_funcionario` (`id_funcionario`),
  ADD KEY `id_equipe` (`id_equipe`),
  ADD KEY `id_funcionario_2` (`id_funcionario`);

--
-- Índices de tabela `funcionario`
--
ALTER TABLE `funcionario`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `cpf` (`cpf`),
  ADD UNIQUE KEY `login_UNIQUE` (`login_sistema`),
  ADD UNIQUE KEY `email_UNIQUE` (`email`),
  ADD KEY `fk_funcionario_funcionario_cargo_idx` (`id_funcionario_cargo`),
  ADD KEY `fk_funcionario_loja1_idx` (`id_loja`),
  ADD KEY `fk_perfil_sistema_idx` (`id_perfil_sistema`),
  ADD KEY `fk_funcionario_cidade1_idx` (`cidade`);

--
-- Índices de tabela `funcionario_cargo`
--
ALTER TABLE `funcionario_cargo`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `nome_cargo_UNIQUE` (`nome_cargo`);

--
-- Índices de tabela `funcionario_telefone`
--
ALTER TABLE `funcionario_telefone`
  ADD PRIMARY KEY (`telefone`,`id_funcionario`),
  ADD KEY `fk_funcionario_telefone_funcionario1_idx` (`id_funcionario`);

--
-- Índices de tabela `laboratorio`
--
ALTER TABLE `laboratorio`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `cnpj_UNIQUE` (`cnpj`);

--
-- Índices de tabela `localidade`
--
ALTER TABLE `localidade`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `id_localidade_cidade` (`nome_localidade`,`id_cidade`),
  ADD UNIQUE KEY `id_rota` (`id_rota`,`seq_rota`),
  ADD KEY `fk_localidade_cidade1_idx` (`id_cidade`),
  ADD KEY `fk_localidade_regiao1_idx` (`id_rota`);

--
-- Índices de tabela `loja`
--
ALTER TABLE `loja`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `cnpj_UNIQUE` (`cnpj`),
  ADD UNIQUE KEY `sigla_loja_UNIQUE` (`sigla_loja`),
  ADD KEY `fk_id_cidade_idx` (`id_cidade`),
  ADD KEY `fk_loja_funcionario1_idx` (`id_funcionario_gerente`),
  ADD KEY `id` (`id`,`sigla_loja`,`rua`,`numero`,`bairro`,`cep`,`cnpj`,`id_cidade`,`cgc`,`id_funcionario_gerente`,`inativo_para_caixa`);

--
-- Índices de tabela `loja_produto`
--
ALTER TABLE `loja_produto`
  ADD PRIMARY KEY (`id`),
  ADD KEY `id_loja` (`id_loja`),
  ADD KEY `id_produto` (`id_produto`);

--
-- Índices de tabela `loja_telefone`
--
ALTER TABLE `loja_telefone`
  ADD PRIMARY KEY (`telefone`,`id_loja`),
  ADD KEY `fk_loja_telefone_loja1_idx` (`id_loja`);

--
-- Índices de tabela `marca_produto`
--
ALTER TABLE `marca_produto`
  ADD PRIMARY KEY (`id_marca_produto`),
  ADD UNIQUE KEY `nome_marcar_produto_UNIQUE` (`nome_marca_produto`);

--
-- Índices de tabela `natureza_despesa`
--
ALTER TABLE `natureza_despesa`
  ADD PRIMARY KEY (`id`);

--
-- Índices de tabela `ordem_servico`
--
ALTER TABLE `ordem_servico`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `unique_numero_loja` (`numero`,`id_loja`),
  ADD KEY `fk_venda_ordem_servico_laboratorio1_idx` (`id_laboratorio`),
  ADD KEY `fk_loja_ordem_servico_idx` (`id_loja`),
  ADD KEY `fk_autor_os_idx` (`func_autor`);

--
-- Índices de tabela `ordem_servico_repasse`
--
ALTER TABLE `ordem_servico_repasse`
  ADD PRIMARY KEY (`id`),
  ADD KEY `id_funcionario_cobrador` (`id_funcionario_cobrador`,`id_venda`),
  ADD KEY `id_venda` (`id_venda`);

--
-- Índices de tabela `perfil_sistema`
--
ALTER TABLE `perfil_sistema`
  ADD PRIMARY KEY (`id`);

--
-- Índices de tabela `prestacao_conta`
--
ALTER TABLE `prestacao_conta`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `id_cobrador_2` (`id_cobrador`,`seq`),
  ADD KEY `id_cobrador` (`id_cobrador`);

--
-- Índices de tabela `prestacao_conta_item`
--
ALTER TABLE `prestacao_conta_item`
  ADD PRIMARY KEY (`id`),
  ADD KEY `tipo_pagamento` (`tipo_pagamento`),
  ADD KEY `id_prest_conta` (`id_prest_conta`);

--
-- Índices de tabela `produto`
--
ALTER TABLE `produto`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `cod_produto_UNIQUE` (`cod_produto`),
  ADD KEY `fk_produto_tipo_produto1_idx` (`id_tipo_produto`),
  ADD KEY `fk_produto_marca_produto1_idx` (`id_marca_produto`);

--
-- Índices de tabela `produto_venda`
--
ALTER TABLE `produto_venda`
  ADD PRIMARY KEY (`id`),
  ADD KEY `fk_produto_has_venda_item_produto1_idx` (`id_produto`),
  ADD KEY `fk_produto_venda_venda1_idx` (`id_venda`);

--
-- Índices de tabela `regiao`
--
ALTER TABLE `regiao`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `nome_rota_UNIQUE` (`nome_regiao`),
  ADD KEY `fk_rota_funcionario1_idx` (`id_funcionario_cobrador`),
  ADD KEY `fk_rota_id_loja_idx` (`id_loja`);

--
-- Índices de tabela `rota`
--
ALTER TABLE `rota`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `nome_regiao_UNIQUE` (`nome_rota`,`id_regiao`),
  ADD KEY `fk_regiao_rota1_idx` (`id_regiao`);

--
-- Índices de tabela `tipo_pagamento`
--
ALTER TABLE `tipo_pagamento`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `nome` (`nome`);

--
-- Índices de tabela `tipo_produto`
--
ALTER TABLE `tipo_produto`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `nome_tipo_produto_UNIQUE` (`nome_tipo_produto`);

--
-- Índices de tabela `uf`
--
ALTER TABLE `uf`
  ADD PRIMARY KEY (`id_uf`),
  ADD UNIQUE KEY `nome_uf_UNIQUE` (`nome_uf`);

--
-- Índices de tabela `veiculo`
--
ALTER TABLE `veiculo`
  ADD PRIMARY KEY (`id`);

--
-- Índices de tabela `venda`
--
ALTER TABLE `venda`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `id_ordem_servico_UNIQUE` (`id_ordem_servico`),
  ADD KEY `fk_venda_cliente1_idx` (`id_cliente`),
  ADD KEY `fk_venda_loja1_idx` (`id_loja`),
  ADD KEY `fk_venda_funcionario1_idx` (`id_func_vendedor`),
  ADD KEY `fk_venda_funcionario2_idx` (`id_func_agente_vendas`),
  ADD KEY `fk_venda_ordem_servico1_idx` (`id_ordem_servico`),
  ADD KEY `venda_antiga` (`venda_antiga`),
  ADD KEY `id_equipe` (`id_equipe`),
  ADD KEY `id_equipe_2` (`id_equipe`),
  ADD KEY `lider_equipe` (`lider_equipe`);

--
-- Índices de tabela `venda_cancelamento`
--
ALTER TABLE `venda_cancelamento`
  ADD PRIMARY KEY (`venda_id`),
  ADD KEY `funcionario_id_autor` (`funcionario_id_autor`),
  ADD KEY `funcionario_id_autorizador` (`funcionario_id_autorizador`);

--
-- Índices de tabela `venda_parcela`
--
ALTER TABLE `venda_parcela`
  ADD KEY `fk_venda_parcelas_venda1_idx` (`id_venda`),
  ADD KEY `primary_key_parcela` (`id_venda`,`numero_parcela`),
  ADD KEY `numero_parcela_index` (`numero_parcela`);

--
-- AUTO_INCREMENT para tabelas despejadas
--

--
-- AUTO_INCREMENT de tabela `caixa_diario`
--
ALTER TABLE `caixa_diario`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=656;

--
-- AUTO_INCREMENT de tabela `central_estoque`
--
ALTER TABLE `central_estoque`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=289;

--
-- AUTO_INCREMENT de tabela `cidade`
--
ALTER TABLE `cidade`
  MODIFY `id` smallint(5) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5565;

--
-- AUTO_INCREMENT de tabela `cliente`
--
ALTER TABLE `cliente`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17773;

--
-- AUTO_INCREMENT de tabela `consulta`
--
ALTER TABLE `consulta`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `despesa_caixa`
--
ALTER TABLE `despesa_caixa`
  MODIFY `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11884;

--
-- AUTO_INCREMENT de tabela `despesa_combustivel`
--
ALTER TABLE `despesa_combustivel`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `equipe_venda`
--
ALTER TABLE `equipe_venda`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT de tabela `funcionario`
--
ALTER TABLE `funcionario`
  MODIFY `id` smallint(5) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=246;

--
-- AUTO_INCREMENT de tabela `laboratorio`
--
ALTER TABLE `laboratorio`
  MODIFY `id` tinyint(3) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=3;

--
-- AUTO_INCREMENT de tabela `localidade`
--
ALTER TABLE `localidade`
  MODIFY `id` smallint(5) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2268;

--
-- AUTO_INCREMENT de tabela `loja`
--
ALTER TABLE `loja`
  MODIFY `id` tinyint(3) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=12;

--
-- AUTO_INCREMENT de tabela `loja_produto`
--
ALTER TABLE `loja_produto`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=287;

--
-- AUTO_INCREMENT de tabela `marca_produto`
--
ALTER TABLE `marca_produto`
  MODIFY `id_marca_produto` tinyint(3) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17;

--
-- AUTO_INCREMENT de tabela `natureza_despesa`
--
ALTER TABLE `natureza_despesa`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=90;

--
-- AUTO_INCREMENT de tabela `ordem_servico`
--
ALTER TABLE `ordem_servico`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20704;

--
-- AUTO_INCREMENT de tabela `ordem_servico_repasse`
--
ALTER TABLE `ordem_servico_repasse`
  MODIFY `id` int(11) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=637;

--
-- AUTO_INCREMENT de tabela `perfil_sistema`
--
ALTER TABLE `perfil_sistema`
  MODIFY `id` tinyint(3) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=6;

--
-- AUTO_INCREMENT de tabela `prestacao_conta`
--
ALTER TABLE `prestacao_conta`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2526;

--
-- AUTO_INCREMENT de tabela `prestacao_conta_item`
--
ALTER TABLE `prestacao_conta_item`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=14859;

--
-- AUTO_INCREMENT de tabela `produto`
--
ALTER TABLE `produto`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1000034;

--
-- AUTO_INCREMENT de tabela `produto_venda`
--
ALTER TABLE `produto_venda`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT de tabela `regiao`
--
ALTER TABLE `regiao`
  MODIFY `id` smallint(5) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=27;

--
-- AUTO_INCREMENT de tabela `rota`
--
ALTER TABLE `rota`
  MODIFY `id` tinyint(3) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=56;

--
-- AUTO_INCREMENT de tabela `tipo_pagamento`
--
ALTER TABLE `tipo_pagamento`
  MODIFY `id` smallint(6) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=11;

--
-- AUTO_INCREMENT de tabela `tipo_produto`
--
ALTER TABLE `tipo_produto`
  MODIFY `id` tinyint(3) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=35;

--
-- AUTO_INCREMENT de tabela `uf`
--
ALTER TABLE `uf`
  MODIFY `id_uf` tinyint(3) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=28;

--
-- AUTO_INCREMENT de tabela `veiculo`
--
ALTER TABLE `veiculo`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=2;

--
-- AUTO_INCREMENT de tabela `venda`
--
ALTER TABLE `venda`
  MODIFY `id` int(10) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=17670;

--
-- Restrições para tabelas despejadas
--

--
-- Restrições para tabelas `caixa_diario`
--
ALTER TABLE `caixa_diario`
  ADD CONSTRAINT `caixa_diario_ibfk_1` FOREIGN KEY (`loja`) REFERENCES `loja` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `caixa_diario_ibfk_2` FOREIGN KEY (`loja`) REFERENCES `loja` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `central_estoque`
--
ALTER TABLE `central_estoque`
  ADD CONSTRAINT `central_estoque_ibfk_1` FOREIGN KEY (`id_usuario_estoque`) REFERENCES `funcionario` (`id`),
  ADD CONSTRAINT `central_estoque_ibfk_3` FOREIGN KEY (`id_loja_origem`) REFERENCES `loja` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `central_estoque_ibfk_4` FOREIGN KEY (`id_loja_destino`) REFERENCES `loja` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `central_estoque_ibfk_5` FOREIGN KEY (`id_usuario_confirmacao_entrada`) REFERENCES `funcionario` (`id`),
  ADD CONSTRAINT `central_estoque_ibfk_6` FOREIGN KEY (`id_usuario_confirmacao_saida`) REFERENCES `funcionario` (`id`);

--
-- Restrições para tabelas `central_estoque_produto`
--
ALTER TABLE `central_estoque_produto`
  ADD CONSTRAINT `central_estoque_produto_ibfk_1` FOREIGN KEY (`id_produto`) REFERENCES `produto` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `central_estoque_produto_ibfk_2` FOREIGN KEY (`id_central_estoque`) REFERENCES `central_estoque` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `cidade`
--
ALTER TABLE `cidade`
  ADD CONSTRAINT `fk_cidade_uf1` FOREIGN KEY (`id_uf`) REFERENCES `uf` (`id_uf`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `cliente`
--
ALTER TABLE `cliente`
  ADD CONSTRAINT `fk_id_localidade` FOREIGN KEY (`id_localidade`) REFERENCES `localidade` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `cliente_telefone`
--
ALTER TABLE `cliente_telefone`
  ADD CONSTRAINT `fk_cliente_telefone_cliente1` FOREIGN KEY (`id_cliente`) REFERENCES `cliente` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `consulta`
--
ALTER TABLE `consulta`
  ADD CONSTRAINT `fk_consulta_funcionario1` FOREIGN KEY (`id_funcionario_oculista`) REFERENCES `funcionario` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_consulta_venda1` FOREIGN KEY (`id_venda`) REFERENCES `venda` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `dados_boleto_loja`
--
ALTER TABLE `dados_boleto_loja`
  ADD CONSTRAINT `dados_boleto_loja_ibfk_1` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`);

--
-- Restrições para tabelas `despesa_caixa`
--
ALTER TABLE `despesa_caixa`
  ADD CONSTRAINT `despesa_caixa_ibfk_1` FOREIGN KEY (`id_caixa`) REFERENCES `caixa_diario` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `despesa_combustivel`
--
ALTER TABLE `despesa_combustivel`
  ADD CONSTRAINT `despesa_combustivel_ibfk_1` FOREIGN KEY (`id_despesa`) REFERENCES `despesa_caixa` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `equipe_venda`
--
ALTER TABLE `equipe_venda`
  ADD CONSTRAINT `equipe_venda_ibfk_1` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `equipe_venda_ibfk_2` FOREIGN KEY (`id_funcionario_lider`) REFERENCES `funcionario` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `equipe_venda_funcionario`
--
ALTER TABLE `equipe_venda_funcionario`
  ADD CONSTRAINT `equipe_venda_funcionario_ibfk_1` FOREIGN KEY (`id_equipe`) REFERENCES `equipe_venda` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `equipe_venda_funcionario_ibfk_2` FOREIGN KEY (`id_funcionario`) REFERENCES `funcionario` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `funcionario`
--
ALTER TABLE `funcionario`
  ADD CONSTRAINT `fk_funcionario_cidade1` FOREIGN KEY (`cidade`) REFERENCES `cidade` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_funcionario_funcionario_cargo` FOREIGN KEY (`id_funcionario_cargo`) REFERENCES `funcionario_cargo` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_funcionario_loja1` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_perfil_sistema` FOREIGN KEY (`id_perfil_sistema`) REFERENCES `perfil_sistema` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `funcionario_telefone`
--
ALTER TABLE `funcionario_telefone`
  ADD CONSTRAINT `fk_funcionario_telefone_funcionario1` FOREIGN KEY (`id_funcionario`) REFERENCES `funcionario` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `localidade`
--
ALTER TABLE `localidade`
  ADD CONSTRAINT `fk_localidade_cidade1` FOREIGN KEY (`id_cidade`) REFERENCES `cidade` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_localidade_regiao1` FOREIGN KEY (`id_rota`) REFERENCES `rota` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `loja`
--
ALTER TABLE `loja`
  ADD CONSTRAINT `fk_id_cidade` FOREIGN KEY (`id_cidade`) REFERENCES `cidade` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_loja_funcionario1` FOREIGN KEY (`id_funcionario_gerente`) REFERENCES `funcionario` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `loja_produto`
--
ALTER TABLE `loja_produto`
  ADD CONSTRAINT `loja_produto_ibfk_1` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `loja_produto_ibfk_2` FOREIGN KEY (`id_produto`) REFERENCES `produto` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `loja_telefone`
--
ALTER TABLE `loja_telefone`
  ADD CONSTRAINT `fk_loja_telefone_loja1` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Restrições para tabelas `ordem_servico`
--
ALTER TABLE `ordem_servico`
  ADD CONSTRAINT `fk_autor_os` FOREIGN KEY (`func_autor`) REFERENCES `funcionario` (`id`) ON DELETE SET NULL ON UPDATE SET NULL,
  ADD CONSTRAINT `fk_loja_ordem_servico` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_venda_ordem_servico_laboratorio1` FOREIGN KEY (`id_laboratorio`) REFERENCES `laboratorio` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `ordem_servico_repasse`
--
ALTER TABLE `ordem_servico_repasse`
  ADD CONSTRAINT `ordem_servico_repasse_ibfk_1` FOREIGN KEY (`id_venda`) REFERENCES `venda` (`id`),
  ADD CONSTRAINT `ordem_servico_repasse_ibfk_2` FOREIGN KEY (`id_funcionario_cobrador`) REFERENCES `funcionario` (`id`),
  ADD CONSTRAINT `ordem_servico_repasse_ibfk_3` FOREIGN KEY (`id_funcionario_cobrador`) REFERENCES `funcionario` (`id`),
  ADD CONSTRAINT `ordem_servico_repasse_ibfk_4` FOREIGN KEY (`id_venda`) REFERENCES `venda` (`id`);

--
-- Restrições para tabelas `prestacao_conta`
--
ALTER TABLE `prestacao_conta`
  ADD CONSTRAINT `prestacao_conta_ibfk_1` FOREIGN KEY (`id_cobrador`) REFERENCES `funcionario` (`id`);

--
-- Restrições para tabelas `prestacao_conta_item`
--
ALTER TABLE `prestacao_conta_item`
  ADD CONSTRAINT `prestacao_conta_item_ibfk_1` FOREIGN KEY (`id_prest_conta`) REFERENCES `prestacao_conta` (`id`),
  ADD CONSTRAINT `prestacao_conta_item_ibfk_2` FOREIGN KEY (`tipo_pagamento`) REFERENCES `tipo_pagamento` (`id`);

--
-- Restrições para tabelas `produto`
--
ALTER TABLE `produto`
  ADD CONSTRAINT `fk_produto_marca_produto1` FOREIGN KEY (`id_marca_produto`) REFERENCES `marca_produto` (`id_marca_produto`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_produto_tipo_produto1` FOREIGN KEY (`id_tipo_produto`) REFERENCES `tipo_produto` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `produto_venda`
--
ALTER TABLE `produto_venda`
  ADD CONSTRAINT `fk_produto_venda_venda1` FOREIGN KEY (`id_venda`) REFERENCES `venda` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `produto_venda_ibfk_1` FOREIGN KEY (`id_produto`) REFERENCES `produto` (`id`);

--
-- Restrições para tabelas `regiao`
--
ALTER TABLE `regiao`
  ADD CONSTRAINT `fk_rota_funcionario1` FOREIGN KEY (`id_funcionario_cobrador`) REFERENCES `funcionario` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_rota_id_loja` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `rota`
--
ALTER TABLE `rota`
  ADD CONSTRAINT `fk_regiao_rota1` FOREIGN KEY (`id_regiao`) REFERENCES `regiao` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `venda`
--
ALTER TABLE `venda`
  ADD CONSTRAINT `fk_venda_cliente1` FOREIGN KEY (`id_cliente`) REFERENCES `cliente` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_venda_funcionario1` FOREIGN KEY (`id_func_vendedor`) REFERENCES `funcionario` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_venda_funcionario2` FOREIGN KEY (`id_func_agente_vendas`) REFERENCES `funcionario` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_venda_loja1` FOREIGN KEY (`id_loja`) REFERENCES `loja` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `fk_venda_ordem_servico1` FOREIGN KEY (`id_ordem_servico`) REFERENCES `ordem_servico` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `venda_ibfk_1` FOREIGN KEY (`venda_antiga`) REFERENCES `venda` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `venda_ibfk_2` FOREIGN KEY (`id_equipe`) REFERENCES `equipe_venda` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
  ADD CONSTRAINT `venda_ibfk_3` FOREIGN KEY (`lider_equipe`) REFERENCES `funcionario` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;

--
-- Restrições para tabelas `venda_cancelamento`
--
ALTER TABLE `venda_cancelamento`
  ADD CONSTRAINT `venda_cancelamento_ibfk_1` FOREIGN KEY (`funcionario_id_autor`) REFERENCES `funcionario` (`id`),
  ADD CONSTRAINT `venda_cancelamento_ibfk_2` FOREIGN KEY (`funcionario_id_autorizador`) REFERENCES `funcionario` (`id`);

--
-- Restrições para tabelas `venda_parcela`
--
ALTER TABLE `venda_parcela`
  ADD CONSTRAINT `fk_venda_parcelas_venda1` FOREIGN KEY (`id_venda`) REFERENCES `venda` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;

/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
